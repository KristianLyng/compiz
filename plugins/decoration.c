...
computeQuadBox(decor_quad_t * q,
	       ...
	       int *return_y1,
	       int *return_x2,
	       int *return_y2, float *return_sx, float *return_sy)
{
	int x1, y1, x2, y2;
	float sx = 1.0f;
	float sy = 1.0f;
...
static Decoration *decorCreateDecoration(CompScreen * screen,
					 Window id, Atom decorAtom)
{
	...
	nQuad = decor_property_to_quads(prop,
					...
					&pixmap,
					&input,
					&maxInput, &minWidth, &minHeight, quad);
...
static void decorReleaseDecoration(CompScreen * screen, Decoration * decoration)
{
	decoration->refCount--;
...
static void destroyWindowDecoration(CompScreen * screen, WindowDecoration * wd)
{
...
static int decorWindowShiftX(CompWindow * w)
{
...
static int decorWindowShiftY(CompWindow * w)
{
	switch (w->sizeHints.win_gravity) {
	case NorthGravity:
	case NorthWestGravity:
...
static Bool decorWindowUpdate(CompWindow * w, Bool allowDecoration)
{
	WindowDecoration *wd;
...
static unsigned int
decorAddSupportedAtoms(CompScreen * s, Atom * atoms, unsigned int size)
{
	unsigned int count;
...
static CompBool decorStartDecorator(void *closure)
{
...
static CompOption *decorGetDisplayOptions(CompPlugin * plugin,
	...
	case DECOR_DISPLAY_OPTION_SHADOW_MATCH:
		{
			...
			if (matchString) {
				if (!strstr(matchString, "rgba=")) {
					...
					matchInit(&rgbaMatch);
					matchAddFromString(&rgbaMatch,
							   "rgba=0");
					matchAddGroup(&value->match,
						      MATCH_OP_AND_MASK,
						      &rgbaMatch);
					matchFini(&rgbaMatch);
				}
...
static Bool decorResizeUpdateTimeout(void *closure)
{
...
static void decorWindowStateChangeNotify(CompWindow * w, unsigned int lastState)
{
	DECOR_SCREEN(w->screen);
	DECOR_WINDOW(w);
	...
	if (!decorWindowUpdate(w, TRUE)) {
		if (dw->wd && dw->wd->decor) {
			if ((w->state & MAXIMIZE_STATE) == MAXIMIZE_STATE)
				setWindowFrameExtents(w,
						      &dw->wd->decor->maxInput);
			else
...
static void decorMatchPropertyChanged(CompDisplay * d, CompWindow * w)
{
	DECOR_DISPLAY(d);
...
static void decorWindowAdd(CompScreen * s, CompWindow * w)
{
	if (w->shaded || w->attrib.map_state == IsViewable)
		decorWindowUpdate(w, TRUE);
}

static void decorWindowRemove(CompScreen * s, CompWindow * w)
{
	if (!w->destroyed)
		decorWindowUpdate(w, FALSE);
}

static void decorObjectAdd(CompObject * parent, CompObject * object)
{
...
static void decorObjectRemove(CompObject * parent, CompObject * object)
{
...
static Bool decorInitCore(CompPlugin * p, CompCore * c)
{
	DecorCore *dc;
...
static void decorFiniCore(CompPlugin * p, CompCore * c)
{
	DECOR_CORE(c);
...
static Bool decorInitDisplay(CompPlugin * p, CompDisplay * d)
{
	...
	dd->winDecorAtom = XInternAtom(d->display, DECOR_WINDOW_ATOM_NAME, 0);
...
static CompBool decorInitObject(CompPlugin * p, CompObject * o)
{
...
static void decorFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *decorGetObjectOptions(CompPlugin * plugin,
...
static CompMetadata *decorGetMetadata(CompPlugin * plugin)
{
