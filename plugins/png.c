...
writePng(unsigned char *buffer,
	 png_rw_ptr writeFunc, void *closure, int width, int height, int stride)
{
	png_struct *png;
	png_info *info;
	png_byte **rows;
	       ...
	       int width, int height, int stride, void *data)
{
	...
	if (!strcasecmp(format, "png")) {
		...
		if (fp) {
			status =
			    writePng(data, stdioWriteFunc, fp, width, height,
				     stride);
			fclose(fp);
		}
...
static CompBool pngInitObject(CompPlugin * p, CompObject * o)
{
...
static void pngFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompMetadata *pngGetMetadata(CompPlugin * plugin)
{
	return &pngMetadata;
}
