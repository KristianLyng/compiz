		     ...
		     CompOutput * output, CompTransform * transform)
{
	...
	matrixRotate(transform, sAttrib->xRotate, 0.0f, 1.0f, 0.0f);
	matrixRotate(transform,
		     sAttrib->vRotate,
		     cosf(sAttrib->xRotate * DEG2RAD),
		     0.0f, sinf(sAttrib->xRotate * DEG2RAD));
	matrixRotate(transform, sAttrib->yRotate, 0.0f, 1.0f, 0.0f);
}
		       ...
		       CompOutput * output, float z, CompTransform * transform)
{
	...
	matrixScale(transform,
		    1.0f / output->width, -1.0f / output->height, 1.0f);
	matrixTranslate(transform,
			-output->region.extents.x1,
			-output->region.extents.y2, 0.0f);
}

void prepareXCoords(CompScreen * screen, CompOutput * output, float z)
{
	glTranslatef(-0.5f, -0.5f, z);
	glScalef(1.0f / output->width, -1.0f / output->height, 1.0f);
...
static void paintBackground(CompScreen * s, Region region, Bool transformed)
{
		  ...
		  Region region, CompOutput * output, unsigned int mask)
{
	...
	if (!(mask & PAINT_SCREEN_NO_OCCLUSION_DETECTION_MASK)) {
		...
		for (w = (*walk.last) (screen); w; w = (*walk.prev) (w)) {
			...
			if ((screen->windowOffsetX != 0
			     || screen->windowOffsetY != 0)
			...
			if (status) {
				...
				if (count == 0 &&
				    screen->
				    opt[COMP_SCREEN_OPTION_UNREDIRECT_FS].value.
				    b) {
					if (XEqualRegion
					    (w->region, &screen->region)
		       ...
		       Region region, CompOutput * output, unsigned int mask)
{
	...
	if ((mask & CLIP_PLANE_MASK) == CLIP_PLANE_MASK) {
		screen->enableOutputClipping(screen, &sTransform, region,
					     output);
	...
	if (mask & PAINT_SCREEN_REGION_MASK) {
		if (mask & PAINT_SCREEN_TRANSFORMED_MASK) {
			if (mask & PAINT_SCREEN_FULL_MASK) {
				...
				(*screen->paintTransformedOutput) (screen,
								   sAttrib,
								   &sTransform,
								   region,
								   output,
...
static void drawWindowGeometry(CompWindow * w)
{
	...
	if (full.x1 < full.x2 && full.y1 < full.y2) {
		...
		while (nBox--) {
			...
			if (x1 < x2 && y1 < y2) {
				...
				if (nClip == 1) {
					if (rect) {
						ADD_RECT(d, matrix, nMatrix, x1,
							 y1, x2, y2);
					} else {
						ADD_QUAD(d, matrix, nMatrix, x1,
							 y1, x2, y2);
					}
		  ...
		  const FragmentAttrib * attrib, unsigned int mask)
{
	...
	if ((!attrib->nFunction && (!w->screen->lighting ||
				    ...
				    || attrib->saturation == 0))
	    || !enableFragmentProgramAndDrawGeometry(w, texture, attrib, filter,
						     mask)) {
		enableFragmentOperationsAndDrawGeometry(w, texture, attrib,
							filter, mask);
	}
}
	    ...
	    const CompTransform * transform, Region region, unsigned int mask)
{
	...
	status =
	    (*w->screen->drawWindow) (w, transform, &fragment, region, mask);
