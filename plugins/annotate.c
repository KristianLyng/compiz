...
static void annoCairoClear(CompScreen * s, cairo_t * cr)
{
	ANNO_SCREEN(s);
...
static cairo_t *annoCairoContext(CompScreen * s)
{
	ANNO_SCREEN(s);
	 ...
	 CompActionState state, CompOption * option, int nOption)
{
	...
	if (s) {
		...
		if (cr) {
			...
			fillColor =
			    getColorOptionNamed(option, nOption, "fill_color",
						fillColor);

			strokeColor =
			    ad->opt[ANNO_DISPLAY_OPTION_STROKE_COLOR].value.c;
			strokeColor =
			    getColorOptionNamed(option, nOption, "stroke_color",
						strokeColor);
			...
			lineWidth =
			    getFloatOptionNamed(option, nOption, "line_width",
						lineWidth);

			if (strcasecmp(tool, "rectangle") == 0) {
				double x, y, w, h;

				x = getFloatOptionNamed(option, nOption, "x",
							0);
				y = getFloatOptionNamed(option, nOption, "y",
							0);
				w = getFloatOptionNamed(option, nOption, "w",
							100);
				h = getFloatOptionNamed(option, nOption, "h",
							100);

				annoDrawRectangle(s, x, y, w, h, fillColor,
						  strokeColor, strokeWidth);
			} else if (strcasecmp(tool, "circle") == 0) {
				double xc, yc, r;

				xc = getFloatOptionNamed(option, nOption, "xc",
							 0);
				yc = getFloatOptionNamed(option, nOption, "yc",
							 0);
				r = getFloatOptionNamed(option, nOption,
							"radius", 100);

				annoDrawCircle(s, xc, yc, r, fillColor,
					       strokeColor, strokeWidth);
			} else if (strcasecmp(tool, "line") == 0) {
				double x1, y1, x2, y2;

				x1 = getFloatOptionNamed(option, nOption, "x1",
							 0);
				y1 = getFloatOptionNamed(option, nOption, "y1",
							 0);
				x2 = getFloatOptionNamed(option, nOption, "x2",
							 100);
				y2 = getFloatOptionNamed(option, nOption, "y2",
							 100);

				annoDrawLine(s, x1, y1, x2, y2, lineWidth,
					     fillColor);
				...
				str =
				    getStringOptionNamed(option, nOption,
							 "slant", "");
				...
				family =
				    getStringOptionNamed(option, nOption,
							 "family", "Sans");
				size =
				    getFloatOptionNamed(option, nOption, "size",
							36.0);

				annoDrawText(s, x, y, text, family, size, slant,
					     weight, fillColor, strokeColor,
					     strokeWidth);
			}
		}
	}
	...
	if (status && as->content && region->numRects) {
		...
		while (nBox--) {
			glTexCoord2f(COMP_TEX_COORD_X
				     (&as->texture.matrix, pBox->x1),
				     COMP_TEX_COORD_Y(&as->texture.matrix,
						      pBox->y2));
			glVertex2i(pBox->x1, pBox->y2);
			glTexCoord2f(COMP_TEX_COORD_X
				     (&as->texture.matrix, pBox->x2),
				     COMP_TEX_COORD_Y(&as->texture.matrix,
						      pBox->y2));
	...
	if (as->grabIndex) {
		if (as->eraseMode) {
			...
			annoDrawLine(s,
				     annoLastPointerX, annoLastPointerY,
				     xRoot, yRoot,
				     ad->opt[ANNO_DISPLAY_OPTION_LINE_WIDTH].
				     value.f,
				     ad->opt[ANNO_DISPLAY_OPTION_FILL_COLOR].
				     value.c);
		}
...
static CompOption *annoGetDisplayOptions(CompPlugin * plugin,
...
static CompBool annoInitObject(CompPlugin * p, CompObject * o)
{
...
static void annoFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *annoGetObjectOptions(CompPlugin * plugin,
...
static CompMetadata *annoGetMetadata(CompPlugin * plugin)
{
	return &annoMetadata;
}
