...
static void cubeLoadImg(CompScreen * s, int n)
{
	...
	if (!readImageToTexture(s, &cs->texture,
				imgFiles[cs->imgCurFile].s, &width, &height)) {
...
static CompOption *cubeGetScreenOptions(CompPlugin * plugin,
...
static void cubeUpdateSkyDomeTexture(CompScreen * screen)
{
	...
	if (strlen(cs->opt[CUBE_SCREEN_OPTION_SKYDOME_IMG].value.s) == 0 ||
		...
		GLfloat fRStart = (GLfloat)
		    cs->opt[CUBE_SCREEN_OPTION_SKYDOME_GRAD_START].value.c[0] /
		    0xffff;
		GLfloat fGStart = (GLfloat)
		    cs->opt[CUBE_SCREEN_OPTION_SKYDOME_GRAD_START].value.c[1] /
		    0xffff;
		GLfloat fBStart = (GLfloat)
		    cs->opt[CUBE_SCREEN_OPTION_SKYDOME_GRAD_START].value.c[2] /
		    0xffff;
		GLfloat fREnd = (GLfloat)
		    cs->opt[CUBE_SCREEN_OPTION_SKYDOME_GRAD_END].value.c[0] /
		    0xffff;
		GLfloat fGEnd = (GLfloat)
		    cs->opt[CUBE_SCREEN_OPTION_SKYDOME_GRAD_END].value.c[1] /
		    0xffff;
		GLfloat fBEnd = (GLfloat)
		    cs->opt[CUBE_SCREEN_OPTION_SKYDOME_GRAD_END].value.c[2] /
		    0xffff;
		...
		glTexParameteri(cs->sky.target, GL_TEXTURE_MIN_FILTER,
				GL_LINEAR);
		glTexParameteri(cs->sky.target, GL_TEXTURE_MAG_FILTER,
				GL_LINEAR);

		glTexParameteri(cs->sky.target, GL_TEXTURE_WRAP_S,
				GL_CLAMP_TO_EDGE);
		glTexParameteri(cs->sky.target, GL_TEXTURE_WRAP_T,
				GL_CLAMP_TO_EDGE);

		glTexImage2D(cs->sky.target,
			     0,
			     GL_RGB,
			     128, 128, 0, GL_RGB, GL_FLOAT, aaafTextureData);
	...
	for (i = iStacksStart; i < iStacksEnd; i++) {
		...
		for (j = iSlicesStart; j < iSlicesEnd; j++) {
			...
			glTexCoord2f(COMP_TEX_COORD_X
				     (&cs->sky.matrix,
				      afTexCoordX[3] * cs->skyW),
				     COMP_TEX_COORD_Y(&cs->sky.matrix,
						      afTexCoordY[3] *
						      cs->skyH));
			glVertex3f(x * r * fRadius, y * r * fRadius,
				   z * fRadius);
...
static int adjustVelocity(CubeScreen * cs)
{
	...
	adjust =
	    unfold * 0.02f * cs->opt[CUBE_SCREEN_OPTION_ACCELERATION].value.f;
	amount = fabs(unfold);
	if (amount < 1.0f)
		amount = 1.0f;
	else if (amount > 3.0f)
...
static void cubePreparePaintScreen(CompScreen * s, int msSinceLastPaint)
{
	...
	if (cs->grabIndex) {
		...
		steps =
		    amount / (0.5f *
			      cs->opt[CUBE_SCREEN_OPTION_TIMESTEP].value.f);
		if (!steps)
			steps = 1;
	...
	if (cs->rotationState == RotationManual ||
		...
		opt = cs->lastOpacityIndex =
		    CUBE_SCREEN_OPTION_INACTIVE_OPACITY;
	} else {
...
cubeCheckOrientation(CompScreen * s,
		     const ScreenPaintAttrib * sAttrib,
		     const CompTransform * transform,
		     CompOutput * outputPtr, CompVector * points)
{
	...
	matrixTranslate(&sTransform, cs->outputXOffset, -cs->outputYOffset,
			0.0f);
...
cubeShouldPaintViewport(CompScreen * s,
			const ScreenPaintAttrib * sAttrib,
			const CompTransform * transform,
			CompOutput * outputPtr, PaintOrder order)
{
	Bool ftb;
	float pointZ;
	...
	ftb =
	    (*cs->checkOrientation) (s, sAttrib, transform, outputPtr, vPoints);
...
cubeMoveViewportAndPaint(CompScreen * s,
			 ...
			 unsigned int mask, PaintOrder paintOrder, int dx)
{
	int output;
	...
	if (!(*cs->shouldPaintViewport) (s,
					 sAttrib,
					 transform, outputPtr, paintOrder))
		return;
	...
	if (cs->nOutput > 1) {
		...
		(*cs->paintViewport) (s, sAttrib, transform, region, outputPtr,
				      mask);
...
cubePaintAllViewports(CompScreen * s,
		      ...
		      CompOutput * outputPtr,
		      unsigned int mask,
		      int xMove, float size, int hsize, PaintOrder paintOrder)
{
...
static void cubeGetRotation(CompScreen * s, float *x, float *v, float *progress)
{
	*x = 0.0f;
	*v = 0.0f;
	*progress = 0.0f;
...
static void cubeClearTargetOutput(CompScreen * s, float xRotate, float vRotate)
{
	CUBE_SCREEN(s);
...
cubePaintTop(CompScreen * s,
	     const ScreenPaintAttrib * sAttrib,
	     const CompTransform * transform, CompOutput * output, int size)
{
	ScreenPaintAttrib sa = *sAttrib;
	CompTransform sTransform = *transform;
			 ...
			 Region region, CompOutput * output)
{
	...
	if (cs->rotationState != RotationNone) {
		...
		if (cs->invert == 1) {
			GLdouble clipPlane0[] =
			    { 1.0, 0.0, 0.5 / cs->distance, 0.0 };
			GLdouble clipPlane1[] =
			    { -1.0, 0.0, 0.5 / cs->distance, 0.0 };
			GLdouble clipPlane2[] =
			    { 0.0, -1.0, 0.5 / cs->distance, 0.0 };
			GLdouble clipPlane3[] =
			    { 0.0, 1.0, 0.5 / cs->distance, 0.0 };
...
cubePaintTransformedOutput(CompScreen * s,
			   ...
			   const CompTransform * transform,
			   Region region,
			   CompOutput * outputPtr, unsigned int mask)
{
	ScreenPaintAttrib sa = *sAttrib;
	...
	if (paintCaps) {
		...
		topDir =
		    (*cs->checkOrientation) (s, &sa, transform, outputPtr, top);
		bottomDir =
		    (*cs->checkOrientation) (s, &sa, transform, outputPtr,
					     bottom);
		...
		if (topDir && bottomDir) {
			...
			glNormal3f(0.0f, 1.0f, 0.0f);
			if (allCaps) {
				(*cs->paintTop) (s, &sa, transform, outputPtr,
						 hsize);
				glNormal3f(0.0f, 0.0f, -1.0f);
				(*cs->paintInside) (s, &sa, transform,
						    outputPtr, hsize);
			...
			(*cs->paintBottom) (s, &sa, transform, outputPtr,
					    hsize);
		} else if (allCaps) {
			glNormal3f(0.0f, 1.0f, 0.0f);
			(*cs->paintTop) (s, &sa, transform, outputPtr, hsize);
			...
			(*cs->paintBottom) (s, &sa, transform, outputPtr,
					    hsize);
			glNormal3f(0.0f, 0.0f, -1.0f);
			(*cs->paintInside) (s, &sa, transform, outputPtr,
					    hsize);
		}
		glNormal3f(0.0f, 0.0f, -1.0f);
	}
		...
		Region region, unsigned int mask)
{
	...
	if ((w->type & CompWindowTypeDesktopMask) &&
		...
		status =
		    (*s->paintWindow) (w, &wAttrib, transform, region, mask);
...
static void cubeInitWindowWalker(CompScreen * s, CompWalker * walker)
{
	CUBE_SCREEN(s);
	   ...
	   CompActionState state, CompOption * option, int nOption)
{
	...
	if (s) {
		...
		if (!cs->grabIndex)
			cs->grabIndex =
			    pushScreenGrab(s, s->invisibleCursor, "cube");
	      ...
	      CompActionState state, CompOption * option, int nOption)
{
	...
	if (s) {
		...
		if (imgNFile) {
			cubeLoadImg(s,
				    (cs->imgCurFile - 1 + imgNFile) % imgNFile);
			damageScreen(s);
		}
	}
...
static CompOption *cubeGetDisplayOptions(CompPlugin * plugin,
...
static Bool cubeInitCore(CompPlugin * p, CompCore * c)
{
	CubeCore *cc;
...
static void cubeFiniCore(CompPlugin * p, CompCore * c)
{
	CUBE_CORE(c);
...
static CompBool cubeInitObject(CompPlugin * p, CompObject * o)
{
...
static void cubeFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *cubeGetObjectOptions(CompPlugin * plugin,
...
static CompMetadata *cubeGetMetadata(CompPlugin * plugin)
{
	return &cubeMetadata;
}
