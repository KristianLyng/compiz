...
int gconf_value_compare(const GConfValue * value_a, const GConfValue * value_b);
...
static gchar *gconfGetKey(CompObject * object,
			  const gchar * plugin, const gchar * option)
{
	const gchar *type;
	...
	if (strcmp(plugin, "core") == 0)
		...
		key =
		    g_strjoin("/", "/apps", APP_NAME, "plugins", plugin,
			      objectName, "options", option, NULL);
...
static GConfValueType gconfTypeFromCompType(CompOptionType type)
{
	switch (type) {
...
gconfSetValue(CompObject * object,
	      CompOptionValue * value, CompOptionType type, GConfValue * gvalue)
{
	switch (type) {
	case CompOptionTypeBool:
	...
	case CompOptionTypeKey:{
			...
			action =
			    keyActionToString(GET_CORE_DISPLAY(object),
					      &value->action);
...
static void
gconfSetOption(CompObject * object, CompOption * o, const gchar * plugin)
{
	...
	if (o->type == CompOptionTypeList) {
		...
		if (!existingValue
		    || gconf_value_compare(existingValue, gvalue))
...
gconfGetValue(CompObject * object,
	      CompOptionValue * value, CompOptionType type, GConfValue * gvalue)
{
	if (type == CompOptionTypeBool && gvalue->type == GCONF_VALUE_BOOL) {
		value->b = gconf_value_get_bool(gvalue);
		return TRUE;
	} else if (type == CompOptionTypeInt && gvalue->type == GCONF_VALUE_INT) {
		...
		stringToKeyAction(GET_CORE_DISPLAY(object), action,
				  &value->action);
		return TRUE;
		     ...
		     CompOption * o, CompOptionValue * value)
{
	...
	if (o->type == CompOptionTypeList && gvalue->type == GCONF_VALUE_LIST) {
		GConfValueType type;
		GSList *list;
		int i, n;
...
static void
gconfGetOption(CompObject * object, CompOption * o, const char *plugin)
{
	GConfEntry *entry;
	gchar *key;
	...
	if (entry) {
		...
		if (gconfReadOptionValue(object, entry, o, &value)) {
			(*core.setOptionForPlugin) (object, plugin, o->name,
						    &value);
			compFiniOptionValue(&value, o->type);
		} else {
...
static CompBool gconfReloadObjectTree(CompObject * object, void *closure);
...
static Bool gconfReload(void *closure)
{
	CompPlugin *p;
...
static CompBool gconfInitPluginForObject(CompPlugin * p, CompObject * o)
{
	CompBool status;
		...
		guint cnxn_id, GConfEntry * entry, gpointer user_data)
{
	...
	if (strcmp(token[0], "") != 0 ||
	    strcmp(token[1], "apps") != 0 || strcmp(token[2], APP_NAME) != 0) {
		g_strfreev(token);
		return;
	}
	...
	if (strcmp(token[3], "general") == 0) {
		...
		if (strcmp(token[3], "plugins") != 0
		    || g_strv_length(token) < 8) {
			g_strfreev(token);
			return;
		}
...
static void gconfSendGLibNotify(CompScreen * s)
{
...
static Bool gconfInitCore(CompPlugin * p, CompCore * c)
{
	GConfCore *gc;
...
static void gconfFiniCore(CompPlugin * p, CompCore * c)
{
	GCONF_CORE(c);
...
static Bool gconfInitScreen(CompPlugin * p, CompScreen * s)
{
...
static CompBool gconfInitObject(CompPlugin * p, CompObject * o)
{
...
static void gconfFiniObject(CompPlugin * p, CompObject * o)
{
...
static Bool gconfInit(CompPlugin * p)
{
...
static void gconfFini(CompPlugin * p)
{
	freeCorePrivateIndex(corePrivateIndex);
	compFiniMetadata(&gconfMetadata);
}

static CompMetadata *gconfGetMetadata(CompPlugin * plugin)
{
