...
static void kdecompatStopCloseAnimation(CompWindow * w)
{
	KDECOMPAT_WINDOW(w);
...
static void kdecompatSendSlideEvent(CompWindow * w, Bool start)
{
	CompOption o[2];
...
static void kdecompatStartSlideAnimation(CompWindow * w, Bool appearing)
{
	KDECOMPAT_WINDOW(w);
	...
	if (kw->slideData) {
		...
		if (appearing) {
			if (data->slideInTime >= 0)
				...
				data->duration =
				    kdecompatGetSlideInDuration(w->screen);
		} else {
			if (data->slideOutTime >= 0)
				data->duration = data->slideOutTime;
			else
				data->duration =
				    kdecompatGetSlideOutDuration(w->screen);
		}
...
static void kdecompatEndSlideAnimation(CompWindow * w)
{
	KDECOMPAT_WINDOW(w);
...
static void kdecompatPreparePaintScreen(CompScreen * s, int msSinceLastPaint)
{
	KDECOMPAT_SCREEN(s);
...
static void kdecompatDonePaintScreen(CompScreen * s)
{
	KDECOMPAT_SCREEN(s);
		     ...
		     Region region, unsigned int mask)
{
	...
	if (!status ||
	    ...
	    !kw->nPreviews ||
	    !w->mapNum || (mask & PAINT_WINDOW_OCCLUSION_DETECTION_MASK)) {
		return status;
	}
	...
	for (i = 0; i < kw->nPreviews; i++) {
		...
		if (tw->texture->pixmap) {
			...
			width =
			    tw->attrib.width + tw->input.left + tw->input.right;
			height =
			    tw->attrib.height + tw->input.top +
			    tw->input.bottom;
			...
			if (icon)
				if (!icon->texture.name
				    && !iconToTexture(s, icon))
					icon = NULL;
			...
			if (icon) {
				...
				if (icon->width >= rect->width
					...
					xScale =
					    (float)rect->width / icon->width;
					yScale =
					    (float)rect->height / icon->height;
				...
				iconReg.extents.y2 =
				    tw->attrib.y + icon->height;

				matrix = icon->texture.matrix;
				matrix.x0 -=
				    (tw->attrib.x * icon->texture.matrix.xx);
				matrix.y0 -=
				    (tw->attrib.y * icon->texture.matrix.yy);
		...
		if (tw->texture->pixmap || icon) {
			...
			matrixTranslate(&wTransform, tw->attrib.x, tw->attrib.y,
					0.0f);
...
static void kdecompatUpdatePreviews(CompWindow * w)
{
	CompWindow *cw;
	CompScreen *s = w->screen;
			...
			if (n == (6 * nPreview + 1)) {
				...
				if (kw->previews) {
					while (kw->nPreviews < nPreview) {
						...
						kw->previews[kw->nPreviews].id =
						    *data++;

						kw->previews[kw->nPreviews].
						    thumb.x = *data++;
						kw->previews[kw->nPreviews].
						    thumb.y = *data++;
						kw->previews[kw->nPreviews].
...
static void kdecompatUpdateSlidePosition(CompWindow * w)
{
	CompDisplay *d = w->screen->display;
	Atom actual;
	int result, format;
		...
		if (format == 32 && actual == kd->kdeSlideAtom && n >= 2) {
			...
			if (kw->slideData) {
				...
				if (kw->slideData->start < 0) {
					...
					getWorkareaForOutput(w->screen, output,
							     &rect);
...
static void kdecompatHandleWindowClose(CompWindow * w, Bool destroy)
{
	KDECOMPAT_WINDOW(w);
...
static CompAction *kdecompatGetScaleAction(CompDisplay * d, const char *name)
{
	CompObject *object;
	CompOption *option;
	int nOption;
...
static Bool kdecompatScaleActivate(void *closure)
{
	...
	if (w && !kd->scaleActive) {
		...
		for (i = 0; i < data->nWindows; i++) {
			snprintf(buffer, sizeof(buffer), "xid=%ld",
				 data->windows[i]);
...
static void kdecompatFreeScaleTimeout(KdeCompatDisplay * kd)
{
...
static void kdecompatPresentWindowGroup(CompWindow * w)
{
	CompScreen *s = w->screen;
	CompDisplay *d = s->display;
	Atom actual;
	...
	result =
	    XGetWindowProperty(d->display, w->id, kd->kdePresentGroupAtom, 0,
			       32768, FALSE, AnyPropertyType, &actual, &format,
			       &n, &left, &propData);
...
static void kdecompatUpdateDesktopLayoutFromViewports(CompScreen * s)
{
	CompDisplay *d = s->display;
...
static void kdecompatUpdateBlurProperty(CompWindow * w)
{
	CompScreen *s = w->screen;
	CompDisplay *d = s->display;
	Atom actual;
	...
	if (result == Success && propData) {
		if (format == 32 && actual == XA_CARDINAL &&
			...
			XChangeProperty(d->display, w->id,
					kd->compizWindowBlurAtom, XA_INTEGER,
					32, PropModeReplace,
...
static void kdecompatHandleEvent(CompDisplay * d, XEvent * event)
{
	CompWindow *w;
...
static Bool kdecompatDamageWindowRect(CompWindow * w, Bool initial, BoxPtr rect)
{
	Bool status;
	CompScreen *s = w->screen;
	...
	if (kw->isPreview && kdecompatGetPlasmaThumbnails(s)) {
		...
		for (cw = s->windows; cw; cw = cw->next) {
			...
			for (i = 0; i < kcw->nPreviews; i++) {
				...
				reg.extents.x1 =
				    kcw->previews[i].thumb.x + cw->attrib.x;
				reg.extents.x2 =
				    reg.extents.x1 +
				    kcw->previews[i].thumb.width;
				reg.extents.y1 =
				    kcw->previews[i].thumb.y + cw->attrib.y;
				reg.extents.y2 =
				    reg.extents.y1 +
				    kcw->previews[i].thumb.height;
...
static void kdecompatAdvertiseSupport(CompScreen * s, Atom atom, Bool enable)
{
	if (enable) {
...
kdecompatScreenOptionChanged(CompScreen * s,
			     CompOption * opt, KdecompatScreenOptions num)
{
...
static Bool kdecompatInitCore(CompPlugin * p, CompCore * c)
{
	KdeCompatCore *kc;
...
static void kdecompatFiniCore(CompPlugin * p, CompCore * c)
{
	KDECOMPAT_CORE(c);
...
static Bool kdecompatInitDisplay(CompPlugin * p, CompDisplay * d)
{
	KdeCompatDisplay *kd;
...
static void kdecompatFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	KDECOMPAT_DISPLAY(d);
...
static Bool kdecompatInitScreen(CompPlugin * p, CompScreen * s)
{
	KdeCompatScreen *ks;
...
static void kdecompatFiniScreen(CompPlugin * p, CompScreen * s)
{
	KDECOMPAT_SCREEN(s);
	KDECOMPAT_DISPLAY(s->display);
...
static Bool kdecompatInitWindow(CompPlugin * p, CompWindow * w)
{
	KdeCompatWindow *kw;
...
static void kdecompatFiniWindow(CompPlugin * p, CompWindow * w)
{
	KDECOMPAT_WINDOW(w);
	KDECOMPAT_SCREEN(w->screen);
...
static CompBool kdecompatInitObject(CompPlugin * p, CompObject * o)
{
...
static void kdecompatFiniObject(CompPlugin * p, CompObject * o)
{
...
static Bool kdecompatInit(CompPlugin * p)
{
...
static void kdecompatFini(CompPlugin * p)
{
