...
setStringListProperty(SmcConn connection,
		      const char *name, const char **values, int nValues)
{
	SmProp prop, *pProp;
	int i;
...
static void setCloneRestartCommands(SmcConn connection)
{
	const char **args;
	int i, count = 0;
...
static void setProgramInfo(SmcConn connection, pid_t pid, uid_t uid)
{
...
saveYourselfCallback(SmcConn connection,
		     ...
		     int saveType, Bool shutdown, int interact_Style, Bool fast)
{
	CompOption args[4];
...
static void shutdownCancelledCallback(SmcConn connection, SmPointer clientData)
{
	(*core.sessionEvent) (&core, CompSessionEventShutdownCancelled, NULL,
			      0);
}

void initSession(char *prevClientId)
{
