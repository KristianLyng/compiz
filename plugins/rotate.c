...
static CompOption *rotateGetScreenOptions(CompPlugin * plugin,
...
static int adjustVelocity(RotateScreen * rs, int size, int invert)
{
	...
	adjust =
	    -xrot * 0.05f * rs->opt[ROTATE_SCREEN_OPTION_ACCELERATION].value.f;
	amount = fabs(xrot);
	if (amount < 10.0f)
		amount = 10.0f;
	...
	adjust =
	    -yrot * 0.05f * rs->opt[ROTATE_SCREEN_OPTION_ACCELERATION].value.f;
	amount = fabs(rs->yrot);
	if (amount < 10.0f)
		amount = 10.0f;
	else if (amount > 30.0f)
...
static void rotatePreparePaintScreen(CompScreen * s, int msSinceLastPaint)
{
	...
	if (rs->moving) {
		if (fabs(rs->xrot + rs->baseXrot + rs->moveTo) <=
			...
			rs->progress =
			    fabs(rs->xrot + rs->baseXrot +
				 rs->moveTo) / (360.0 / (s->hsize * 2.0));
		} else if (fabs(rs->xrot + rs->baseXrot) <=
			   (360.0 / (s->hsize * 2.0))) {
			rs->progress =
			    fabs(rs->xrot +
				 rs->baseXrot) / (360.0 / (s->hsize * 2.0));
		} else {
			rs->progress +=
			    fabs(rs->xrot + rs->baseXrot -
				 oldXrot) / (360.0 / (s->hsize * 2.0));
		...
		while (steps--) {
			...
			rs->progressVelocity =
			    (tamount * rs->progressVelocity +
			     adjust) / (tamount + 1.0f);

			rs->progress += rs->progressVelocity * chunk;

			if (fabs(dt) < 0.01f
			    && fabs(rs->progressVelocity) < 0.0001f) {
				if (rs->grabbed)
					rs->progress = 1.0f;
				else
					rs->progress = 0.0f;
...
static void
rotateGetRotation(CompScreen * s, float *x, float *v, float *progress)
{
	CUBE_SCREEN(s);
	ROTATE_SCREEN(s);
	       ...
	       CompActionState state, CompOption * option, int nOption)
{
	...
	if (s) {
		...
		if (!rs->grabIndex) {
			rs->grabIndex =
			    pushScreenGrab(s, s->invisibleCursor, "rotate");
			if (rs->grabIndex) {
				int x, y;
		...
		if (rs->grabIndex) {
			...
			rs->grabbed = TRUE;
			rs->snapTop =
			    rs->opt[ROTATE_SCREEN_OPTION_SNAP_TOP].value.b;
			rs->snapBottom =
       ...
       CompActionState state, CompOption * option, int nOption)
{
	...
	if (s) {
		...
		if (!rs->grabIndex) {
			...
			o[0].name = "x";
			o[0].value.i =
			    getIntOptionNamed(option, nOption, "x", 0);

			o[1].type = CompOptionTypeInt;
			o[1].name = "y";
			o[1].value.i =
			    getIntOptionNamed(option, nOption, "y", 0);
		 ...
		 CompActionState state, CompOption * option, int nOption)
{
	...
	if (s) {
		Bool raise =
		    rd->opt[ROTATE_DISPLAY_OPTION_RAISE_ON_ROTATE].value.b;
		int direction;
		   ...
		   CompActionState state, CompOption * option, int nOption)
{
	...
	s = findScreenAtDisplay(d, xid);
	if (s)
		rotateEdgeFlip(s, SCREEN_EDGE_LEFT, action, state, option,
			       nOption);
		    ...
		    CompActionState state, CompOption * option, int nOption)
{
	...
	s = findScreenAtDisplay(d, xid);
	if (s)
		rotateEdgeFlip(s, SCREEN_EDGE_RIGHT, action, state, option,
			       nOption);
	 ...
	 CompActionState state, CompOption * option, int nOption)
{
	...
	if (s) {
		...
		o[0].name = "x";
		o[0].value.i =
		    getIntOptionNamed(option, nOption, "x", pointerX);

		o[1].type = CompOptionTypeInt;
		o[1].name = "y";
		o[1].value.i =
		    getIntOptionNamed(option, nOption, "y", pointerY);
		   ...
		   CompActionState state, CompOption * option, int nOption)
{
	...
	if (s) {
		...
		while (i <= ROTATE_DISPLAY_OPTION_TO_12_WINDOW_KEY) {
			if (action == &rd->opt[i].value.action) {
				face =
				    i - ROTATE_DISPLAY_OPTION_TO_1_WINDOW_KEY;
				break;
			}
		...
		if (s) {
			...
			if (rs->grabIndex) {
				if (rs->grabbed) {
					...
					if (rs->
					    opt
					    [ROTATE_SCREEN_OPTION_POINTER_INVERT_Y].
					    value.b)
						pointerDy = -pointerDy;

					if (rs->
					    opt
					    [ROTATE_SCREEN_OPTION_POINTER_INVERT_X].
					    value.b)
						pointerDx = -pointerDx;

					rs->xVelocity +=
					    pointerDx * rs->pointerSensitivity *
					    cs->invert;
					rs->yVelocity +=
					    pointerDy * rs->pointerSensitivity;
...
static void rotateActivateWindow(CompWindow * w)
{
...
static CompOption *rotateGetDisplayOptions(CompPlugin * plugin,
...
static CompBool rotateInitObject(CompPlugin * p, CompObject * o)
{
...
static void rotateFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *rotateGetObjectOptions(CompPlugin * plugin,
...
static CompMetadata *rotateGetMetadata(CompPlugin * plugin)
{
