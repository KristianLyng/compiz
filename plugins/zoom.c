...
static void zoomInEvent(CompScreen * s)
{
	CompOption o[6];
...
static void zoomOutEvent(CompScreen * s)
{
	CompOption o[2];
...
static void zoomPreparePaintScreen(CompScreen * s, int msSinceLastPaint)
{
	...
	if (zs->adjust) {
		...
		steps =
		    amount / (0.5f *
			      zs->opt[ZOOM_SCREEN_OPTION_TIMESTEP].value.f);
		if (!steps)
			steps = 1;
		...
		while (steps--) {
			if (adjustZoomVelocity(zs)) {
				BoxPtr pBox =
				    &s->outputDev[zs->zoomOutput].region.
				    extents;
				...
				if (zs->current[zs->zoomOutput].x1 == pBox->x1
				    && zs->current[zs->zoomOutput].y1 ==
				    pBox->y1
				    && zs->current[zs->zoomOutput].x2 ==
				    pBox->x2
				    && zs->current[zs->zoomOutput].y2 ==
				    pBox->y2) {
...
static void zoomGetCurrentZoom(CompScreen * s, int output, ZoomBox * pBox)
{
	ZOOM_SCREEN(s);
		...
		Region region, CompOutput * output, unsigned int mask)
{
	...
	if (output->id != ~0 && (zs->zoomed & (1 << output->id))) {
		...
		if ((zs->zoomOutput != output->id || !zs->adjust)
		    && scale > 3.9f
		...
		status =
		    (*s->paintOutput) (s, sAttrib, &zTransform, region, output,
				       mask);
...
static void zoomInitiateForSelection(CompScreen * s, int output)
{
	int tmp;
	...
	if (zs->x1 < zs->x2 && zs->y1 < zs->y2) {
		...
		if (box.x1 < s->outputDev[output].region.extents.x1) {
			box.x2 +=
			    s->outputDev[output].region.extents.x1 - box.x1;
		...
		if (box.y1 < s->outputDev[output].region.extents.y1) {
			box.y2 +=
			    s->outputDev[output].region.extents.y1 - box.y1;
       ...
       CompActionState state, CompOption * option, int nOption)
{
	...
	if (s) {
		...
		x0 = (pointerX -
		      s->outputDev[output].region.extents.x1) / (float)
		    s->outputDev[output].width;
		y0 = (pointerY -
		      s->outputDev[output].region.extents.y1) / (float)
		...
		CompActionState state, CompOption * option, int nOption)
{
	...
	if (s) {
		...
		if (!zs->panGrabIndex)
			zs->panGrabIndex =
			    pushScreenGrab(s, zs->panCursor, "zoom-pan");
...
static void zoomHandleMotionEvent(CompScreen * s, int xRoot, int yRoot)
{
	ZOOM_SCREEN(s);
