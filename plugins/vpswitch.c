		   ...
		   CompActionState state, CompOption * option, int nOption)
{
	...
	tOption =
	    (*plugin->vTable->getObjectOptions) (plugin, object, &nTOption);
	while (nTOption--) {
		if (tOption->type == CompOptionTypeAction ||
			...
			if (strcmp(tOption->name, vpswitchGetInitAction(d)) == 0
				...
				rv = (tOption->value.action.initiate)
				    (d, &tOption->value.action, state, option,
				     nOption);
				break;
			}
...
static void vpswitchHandleEvent(CompDisplay * d, XEvent * event)
{
	CompScreen *s;
		 ...
		 CompActionState state, CompOption * option, int nOption)
{
	...
	for (i = VpswitchDisplayOptionSwitchTo1Key;
		...
		if (action == &vpswitchGetDisplayOption(d, i)->value.action) {
			...
			vd->activeScreen = findScreenAtDisplay(d, xid);
			vd->destination =
			    i - VpswitchDisplayOptionSwitchTo1Key + 1;
			break;
		}
	}
...
static void vpswitchFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	VPSWITCH_DISPLAY(d);
...
static CompBool vpswitchInitObject(CompPlugin * p, CompObject * o)
{
...
static void vpswitchFiniObject(CompPlugin * p, CompObject * o)
{
...
static Bool vpswitchInit(CompPlugin * p)
{
...
static void vpswitchFini(CompPlugin * p)
{
