...
static int reallocScreenPrivate(int size, void *closure)
{
	...
	for (s = d->screens; s; s = s->next) {
		privates =
		    realloc(s->base.privates, size * sizeof(CompPrivate));
		if (!privates)
			return FALSE;
...
CompBool
forEachScreenObject(CompObject * parent, ObjectCallBackProc proc, void *closure)
{
...
static void updateOutputDevices(CompScreen * s)
{
	...
	for (i = 0; i < list->nValue; i++) {
		...
		bits =
		    XParseGeometry(list->value[i].s, &x, &y, &width, &height);
	...
	if (region) {
		...
		for (i = 0; i < s->nOutputDev; i++)
			XSubtractRegion(region, &s->outputDev[i].region,
					region);
...
static void detectOutputDevices(CompScreen * s)
{
	if (!noDetection && s->opt[COMP_SCREEN_OPTION_DETECT_OUTPUTS].value.b) {
		...
		if (s->display->nScreenInfo) {
			...
			snprintf(output, size, "%dx%d+%d+%d", s->width,
				 s->height, 0, 0);
		...
		CompScreen * screen, const char *name, CompOptionValue * value)
{
	...
	case COMP_SCREEN_OPTION_VSIZE:
		if (compSetIntOption(o, value)) {
			...
			setVirtualScreenSize(screen, hsize->value.i,
					     o->value.i);
			return TRUE;
		}
		break;
...
const CompMetadataOptionInfo coreScreenOptionInfo[COMP_SCREEN_OPTION_NUM] = {
	...
	{"focus_prevention_level", "int",
	 RESTOSTRING(0, FOCUS_PREVENTION_LEVEL_LAST) "<default>0</default>", 0,
	 0},
...
static void addSequence(CompScreen * screen, SnStartupSequence * sequence)
{
	...
	if (!screen->startupSequenceTimeoutHandle)
		screen->startupSequenceTimeoutHandle =
		    compAddTimeout(1000, 1500, startupSequenceTimeout, screen);
...
static void reshape(CompScreen * s, int w, int h)
{
	...
	if (s->display->xineramaExtension) {
		...
		d->nScreenInfo = 0;
		d->screenInfo =
		    XineramaQueryScreens(d->display, &d->nScreenInfo);
	}
...
void updateScreenBackground(CompScreen * screen, CompTexture * texture)
{
	...
	if (!texture->name && backgroundImage)
		readImageToTexture(screen, texture, backgroundImage, &width,
				   &height);
...
{
	if (!noDetection
	    && s->opt[COMP_SCREEN_OPTION_DETECT_REFRESH_RATE].value.b) {
		char *name;
		CompOptionValue value;
		...
		s->redrawTime =
		    1000 / s->opt[COMP_SCREEN_OPTION_REFRESH_RATE].value.i;
...
static unsigned int
addSupportedAtoms(CompScreen * s, Atom * atoms, unsigned int size)
{
	CompDisplay *d = s->display;
...
static void getDesktopHints(CompScreen * s)
{
	...
	if (useDesktopHints) {
		result = XGetWindowProperty(s->display->display, s->root,
					    d->numberOfDesktopsAtom, 0L, 1L,
					    FALSE, XA_CARDINAL, &actual,
...
void showOutputWindow(CompScreen * s)
{
	...
	if (useCow) {
		...
		XFixesSetWindowShapeRegion(dpy,
					   s->output, ShapeBounding, 0, 0, 0);
		XFixesSetWindowShapeRegion(dpy,
					   s->output, ShapeInput, 0, 0, region);
...
void updateOutputWindow(CompScreen * s)
{
	...
	if (useCow) {
		...
		for (w = s->reverseWindows; w; w = w->prev)
			if (w->overlayWindow) {
				XSubtractRegion(tmpRegion, w->region,
						tmpRegion);
			}
...
static void makeOutputWindow(CompScreen * s)
{
	...
	if (useCow) {
		s->overlay =
		    XCompositeGetOverlayWindow(s->display->display, s->root);
		s->output = s->overlay;
...
static void enterShowDesktopMode(CompScreen * s)
{
	CompDisplay *d = s->display;
	CompWindow *w;
	...
	for (w = s->windows; w; w = w->next) {
		if ((s->showingDesktopMask & w->wmType) &&
		    (!(w->state & CompWindowStateSkipTaskbarMask)
		     || st->value.b)) {
			if (!w->inShowDesktopMode && !w->grabbed && w->managed
			    && (*s->focusWindow) (w)) {
...
static void leaveShowDesktopMode(CompScreen * s, CompWindow * window)
{
	CompWindow *w;
...
static CompWindow *walkFirst(CompScreen * s)
{
	return s->windows;
}

static CompWindow *walkLast(CompScreen * s)
{
	return s->reverseWindows;
}

static CompWindow *walkNext(CompWindow * w)
{
	return w->next;
}

static CompWindow *walkPrev(CompWindow * w)
{
	return w->prev;
}

static void initWindowWalker(CompScreen * screen, CompWalker * walker)
{
	walker->fini = NULL;
...
static void freeScreen(CompScreen * s)
{
	int i, j;
	...
	for (i = 0; i < 2; i++)
		for (j = 0; j < 64; j++)
			if (s->saturateFunction[i][j])
				destroyFragmentFunction(s,
							s->
							saturateFunction[i][j]);
	  ...
	  Window wmSnSelectionWindow, Atom wmSnAtom, Time wmSnTimestamp)
{
	...
	if (strstr(glExtensions, "GL_ARB_fragment_program")) {
		...
		if (s->genPrograms &&
		    ...
		    s->programLocalParameter4f && s->getProgramiv)
			s->fragmentProgram = 1;
	}
	...
	if (strstr(glExtensions, "GL_EXT_framebuffer_object")) {
		...
		if (s->genFramebuffers &&
		    ...
		    s->framebufferTexture2D && s->generateMipmap)
			s->fbo = 1;
	}
	...
	for (i = 0; i < SCREEN_EDGE_NUM; i++) {
		...
		XChangeProperty(dpy, s->screenEdge[i].id,
				...
				PropModeReplace, (unsigned char *)&xdndVersion,
				1);
...
int pushScreenGrab(CompScreen * s, Cursor cursor, const char *name)
{
	if (s->maxGrab == 0) {
		...
		if (status == GrabSuccess) {
			status = XGrabKeyboard(s->display->display,
					       ...
					       CurrentTime);
			if (status != GrabSuccess) {
				XUngrabPointer(s->display->display,
					       CurrentTime);
				return 0;
			}
		} else
	...
	if (s->grabSize <= s->maxGrab) {
		s->grabs =
		    realloc(s->grabs, sizeof(CompGrab) * (s->maxGrab + 1));
		if (!s->grabs)
			return 0;
...
static void
grabUngrabOneKey(CompScreen * s, unsigned int modifiers, int keycode, Bool grab)
{
	if (grab) {
		XGrabKey(s->display->display,
			 keycode,
			 modifiers,
			 s->root, TRUE, GrabModeAsync, GrabModeAsync);
	} else {
...
static void removePassiveButtonGrab(CompScreen * s, CompButtonBinding * button)
{
	int i;
	...
	for (i = 0; i < s->nButtonGrab; i++) {
		if (button->button == s->buttonGrab[i].button &&
			...
			memmove(s->buttonGrab + i, s->buttonGrab + i + 1,
				(s->nButtonGrab -
				 (i + 1)) * sizeof(CompButtonGrab));

			s->nButtonGrab--;
			s->buttonGrab = realloc(s->buttonGrab,
						sizeof(CompButtonGrab) *
						s->nButtonGrab);
		}
	}
}
...
static void
computeWorkareaForBox(CompScreen * s, BoxPtr pBox, XRectangle * area)
{
	CompWindow *w;
	Region region;
	REGION r;
...
void updateWorkareaForScreen(CompScreen * s)
{
	...
	for (i = 0; i < s->nOutputDev; i++) {
		computeWorkareaForBox(s, &s->outputDev[i].region.extents,
				      &workArea);
		if (memcmp
...
static Bool isClientListWindow(CompWindow * w)
{
...
static void countClientListWindow(CompWindow * w, void *closure)
{
...
toolkitAction(CompScreen * s,
	      Atom toolkitAction,
	      Time eventTime, Window window, long data0, long data1, long data2)
{
	XEvent ev;
	...
	XSendEvent(s->display->display, s->root, FALSE, StructureNotifyMask,
		   &ev);
}
...
void getCurrentOutputExtents(CompScreen * s, int *x1, int *y1, int *x2, int *y2)
{
	if (x1)
...
void setDefaultViewport(CompScreen * s)
{
	...
	glViewport(s->lastViewport.x,
		   s->lastViewport.y,
		   s->lastViewport.width, s->lastViewport.height);
}
...
viewportForGeometry(CompScreen * s,
		    ...
		    int y,
		    int width,
		    int height, int borderWidth, int *viewportX, int *viewportY)
{
	int centerX;
	int centerY;
	...
	if (viewportY) {
		...
		if (centerY < 0)
			*viewportY =
			    s->y + ((centerY / s->height) - 1) % s->vsize;
		else
			*viewportY = s->y + (centerY / s->height) % s->vsize;
	}
}

static int rectangleOverlapArea(BOX * rect1, BOX * rect2)
{
			...
			int x, int y, int width, int height, int borderWidth)
{
	...
	for (i = 0; i < s->nOutputDev; i++)
		overlapAreas[i] =
		    rectangleOverlapArea(&s->outputDev[i].region.extents,
					 &geomRect);
	...
	if (seen > 1) {
		...
		searchLargest =
		    (strategy != OUTPUT_OVERLAP_MODE_PREFER_SMALLER);
		if (searchLargest)
			bestOutputSize = 0;
		else
