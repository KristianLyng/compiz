...
static void shiftActivateEvent(CompScreen * s, Bool activating)
{
	CompOption o[2];
	...
	(*s->display->handleCompizEvent) (s->display, "shift", "activate", o,
					  2);
}

static Bool isShiftWin(CompWindow * w)
{
...
static void shiftFreeWindowTitle(CompScreen * s)
{
	SHIFT_SCREEN(s);
...
static void shiftRenderWindowTitle(CompScreen * s)
{
	CompTextAttrib tA;
	int ox1, ox2, oy1, oy2;
...
static void shiftDrawWindowTitle(CompScreen * s)
{
		 ...
		 Region region, unsigned int mask)
{
	...
	if (ss->state != ShiftStateNone && !ss->paintingAbove) {
		...
		if (sw->opacity > 0.01 && (ss->activeSlot == NULL)) {
			sAttrib.brightness =
			    sAttrib.brightness * sw->brightness;
		...
		if (scaled && w->texture->pixmap) {
			...
			if (slot->primary)
				sscale =
				    (ss->anim * slot->scale) + (1 - ss->anim);
			else
				sscale = ss->anim * slot->scale;

			if (slot->primary && !ss->reflectActive)
				sopacity =
				    (ss->anim * slot->opacity) + (1 - ss->anim);
			else
			...
			matrixTranslate(&wTransform,
					w->attrib.x +
					(w->attrib.width * sscale / 2),
					w->attrib.y +
					(w->attrib.height * sscale / 2.0),
					0.0f);
		...
		if (scaled && ((shiftGetOverlayIcon(s) != OverlayIconNone) ||
			...
			if (icon
			    && (icon->texture.name
				|| iconToTexture(w->screen, icon))) {
				REGION iconReg;
				float scale;
				float x, y;
				int width, height;
				int scaledWinWidth, scaledWinHeight;
				ShiftOverlayIconEnum iconOverlay =
				    shiftGetOverlayIcon(s);
				ShiftSlot *slot = ss->activeSlot->slot;
				...
				if (slot->primary)
					sscale =
					    (ss->anim * slot->scale) + (1 -
									ss->
									anim);
				else
					sscale =
					    ss->anim * ss->anim * slot->scale;
				...
				if (w->vCount) {
					...
					fragment.opacity =
					    (float)fragment.opacity * sopacity;
					fragment.brightness =
					    (float)fragment.brightness *
					    ss->reflectBrightness;

					matrixTranslate(&wTransform, sx, sy,
							sz);
					...
					matrixScale(&wTransform,
						    ss->output->width,
						    -ss->output->height, 1.0f);

					matrixRotate(&wTransform, srot, 0.0,
						     1.0, 0.0);

					matrixScale(&wTransform,
						    1.0f / ss->output->width,
						    -1.0f / ss->output->height,
						    1.0f);
					...
					matrixTranslate(&wTransform, x -
							...
							(w->attrib.height *
							 sscale / 2.0), 0.0f);
					matrixScale(&wTransform, scale, scale,
						    1.0f);
...
static void switchActivateEvent(CompScreen * s, Bool activating)
{
	...
	(*s->display->handleCompizEvent) (s->display, "switcher", "activate", o,
					  2);
}
...
static int compareShiftWindowDistance(const void *elem1, const void *elem2)
{
...
static Bool layoutThumbsCover(CompScreen * s)
{
	SHIFT_SCREEN(s);
	CompWindow *w;
	int index;
	...
	for (index = 0; index < ss->nWindows; index++) {
		...
		for (i = 0; i < 2; i++) {
			...
			if (fabs(distance) < 1.0) {
				sw->slots[i].x =
				    centerX + (sin(pos * PI * 0.5) * space);
				sw->slots[i].z = fabs(distance);
				sw->slots[i].z *=
				    -(ss->maxThumbWidth / (2.0 * (ox2 - ox1)));

				sw->slots[i].rotation =
				    sin(pos * PI * 0.5) * -60;
			} else {
				float rad =
				    (space / (ox2 - ox1)) / sin(PI / 6.0);
...
static Bool layoutThumbsFlip(CompScreen * s)
{
	SHIFT_SCREEN(s);
	CompWindow *w;
	int index;
	...
	for (index = 0; index < ss->nWindows; index++) {
		...
		for (i = 0; i < 2; i++) {
			...
			if (distance > 0.0)
				sw->slots[i].opacity =
				    MAX(0.0, 1.0 - (distance * 1.0));
			else {
				if (distance < -(ss->nWindows - 1))
					sw->slots[i].opacity =
					    MAX(0.0, ss->nWindows + distance);
				else
			...
			sw->slots[i].x =
			    sin(angle) * distance * (ss->maxThumbWidth / 2);
			if (distance > 0 && FALSE)
				sw->slots[i].x *= 1.5;
			sw->slots[i].x += centerX;
...
static void shiftAddWindowToList(CompScreen * s, CompWindow * w)
{
	SHIFT_SCREEN(s);

	if (ss->windowsSize <= ss->nWindows) {
		ss->windows = realloc(ss->windows,
				      sizeof(CompWindow *) * (ss->nWindows +
							      32));
		if (!ss->windows)
			return;
...
static Bool shiftUpdateWindowList(CompScreen * s)
{
	SHIFT_SCREEN(s);
...
static Bool shiftCreateWindowList(CompScreen * s)
{
	CompWindow *w;
	SHIFT_SCREEN(s);
...
static int shiftCountWindows(CompScreen * s)
{
	CompWindow *w;
	int count = 0;
...
static Bool adjustShiftWindowAttribs(CompWindow * w, float chunk)
{
...
static Bool adjustShiftAnimationAttribs(CompScreen * s, float chunk)
{
		 ...
		 Region region, CompOutput * output, unsigned int mask)
{
	...
	if (ss->state != ShiftStateNone &&
		...
		if (shiftGetReflection(s)) {
			...
			matrixTranslate(&rTransform, 0.0,
					oy1 + oy2 + maxThumbHeight, 0.0);
			...
			if (shiftGetGroundSize(s) > 0.0) {
				...
				color[3] =
				    (float)shiftGetGroundColor1(s)[3] *
				    ss->anim;
				glColor4usv(color);
				glVertex2f(-0.5, -0.5);
				glVertex2f(0.5, -0.5);
				...
				color[3] =
				    (float)shiftGetGroundColor2(s)[3] *
				    ss->anim;
				glColor4usv(color);
...
static void shiftPreparePaintScreen(CompScreen * s, int msSinceLastPaint)
{
	SHIFT_SCREEN(s);

	if (ss->state != ShiftStateNone && (ss->moreAdjust || ss->moveAdjust)) {
		CompWindow *w;
		int steps;
		float amount, chunk;
		int i;
...
static Bool canStackRelativeTo(CompWindow * w)
{
	if (w->attrib.override_redirect)
		return FALSE;
...
static void shiftDonePaintScreen(CompScreen * s)
{
	SHIFT_SCREEN(s);
...
static void shiftTerm(CompScreen * s, Bool cancel)
{
	SHIFT_SCREEN(s);
	...
	if (ss->state != ShiftStateNone &&
	    ss->state != ShiftStateFinish && ss->state != ShiftStateIn) {
...
shiftDoSwitch(CompDisplay * d,
	      ...
	      CompOption * option, int nOption, Bool nextWindow, ShiftType type)
{
	CompScreen *s;
	Window xid;
	Bool ret = TRUE;
	...
	if (s) {
		...
		if ((ss->state == ShiftStateNone)
			...
			if (type == ShiftTypeGroup) {
				...
				ss->type = type;
				ret =
				    shiftInitiateScreen(s, action, state,
							option, nOption);
			}
	      ...
	      CompActionState state, CompOption * option, int nOption)
{
	...
	if (s) {
		...
		if ((ss->state == ShiftStateNone) || (ss->state == ShiftStateIn)
		    || (ss->state == ShiftStateFinish))
			ret =
			    shiftInitiateScreen(s, action, state, option,
						nOption);
		else
...
static void shiftWindowRemove(CompDisplay * d, CompWindow * w)
{
	if (w) {
		Bool inList = FALSE;
		int j, i = 0;
...
static void shiftHandleEvent(CompDisplay * d, XEvent * event)
{
	SHIFT_DISPLAY(d);
	CompScreen *s;
	CompWindow *w = NULL;
	...
	case MotionNotify:
		...
		if (s) {
			...
			if (ss->state == ShiftStateSwitching
				...
				if (ss->buttonPressed) {
					int ox1 =
					    s->outputDev[ss->usedOutput].region.
					    extents.x1;
					int ox2 =
					    s->outputDev[ss->usedOutput].region.
					    extents.x2;
					...
					if (event->xmotion.x_root < 50)
						wx = 50;
					if (s->width - event->xmotion.x_root <
					    50)
						wx = -50;
					if (event->xmotion.y_root < 50)
						wy = 50;
					if (s->height - event->xmotion.y_root <
					    50)
						wy = -50;
					if (wx != 0 || wy != 0) {
						warpPointer(s, wx, wy);
...
static Bool shiftDamageWindowRect(CompWindow * w, Bool initial, BoxPtr rect)
{
	Bool status = FALSE;
...
static Bool shiftInitDisplay(CompPlugin * p, CompDisplay * d)
{
	ShiftDisplay *sd;
	int index;
...
static void shiftFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	SHIFT_DISPLAY(d);
...
static Bool shiftInitScreen(CompPlugin * p, CompScreen * s)
{
	ShiftScreen *ss;
...
static void shiftFiniScreen(CompPlugin * p, CompScreen * s)
{
	SHIFT_SCREEN(s);
...
static Bool shiftInitWindow(CompPlugin * p, CompWindow * w)
{
	ShiftWindow *sw;
...
static void shiftFiniWindow(CompPlugin * p, CompWindow * w)
{
	SHIFT_WINDOW(w);
...
static Bool shiftInit(CompPlugin * p)
{
...
static void shiftFini(CompPlugin * p)
{
	if (displayPrivateIndex >= 0)
		freeDisplayPrivateIndex(displayPrivateIndex);
}

static CompBool shiftInitObject(CompPlugin * p, CompObject * o)
{
...
static void shiftFiniObject(CompPlugin * p, CompObject * o)
{
