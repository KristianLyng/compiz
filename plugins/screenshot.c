...
static int shotSort(const struct dirent **a, const struct dirent **b)
{
...
static char *shotGetXDGDesktopDir(void)
{
	int nPrinted;
	FILE *userDirsFile;
	...
	while (fgets(line, MAX_LINE_LENGTH, userDirsFile) != NULL) {
		...
		if (varStart) {	/* if found */
			...
			size_t valueStartPos =
			    (varStart - line) + varLength + 2;

			/* Ignore the " at the end */
			size_t valueSrcLength =
			    strlen(line) - valueStartPos - 1;
	...
	if (ss->grab) {
		...
		if (!ss->grabIndex) {
			...
			if (w && h) {
				GLubyte *buffer;
				char *dir =
				    sd->opt[SHOT_DISPLAY_OPTION_DIR].value.s;
				...
				if (buffer) {
					...
					n = scandir(dir, &namelist, shotFilter,
						    shotSort);
					if (n >= 0) {
						char name[256];
						char *app;
						...
						app =
						    sd->
						    opt
						    [SHOT_DISPLAY_OPTION_LAUNCH_APP].
						    value.s;
						/* FIXME:
...
static CompOption *shotGetDisplayOptions(CompPlugin * plugin,
...
static CompBool shotInitObject(CompPlugin * p, CompObject * o)
{
...
static void shotFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *shotGetObjectOptions(CompPlugin * plugin,
...
static CompMetadata *shotGetMetadata(CompPlugin * plugin)
{
	return &shotMetadata;
}
