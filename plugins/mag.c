...
static const char *fisheyeFpString =
    ...
    "MAD t1, t1, p1, p2;" "TEX result.color, t1, texture[0], %s;" "END";

static void magCleanup(CompScreen * s)
{
	MAG_SCREEN(s);
...
static Bool loadFragmentProgram(CompScreen * s)
{
	char buffer[1024];
	GLint errorPos;
	...
	if (glGetError() != GL_NO_ERROR || errorPos != -1) {
		compWarn("failed to fisheye program");
...
static Bool loadImages(CompScreen * s)
{
	MAG_SCREEN(s);
	...
	if (!ms->overlay.loaded) {
		compWarn("Could not load magnifier overlay image \"%s\"!",
			 magGetOverlay(s));
		return FALSE;
	}
	...
	ms->mask.loaded = readImageToTexture(s, &ms->mask.tex,
					     magGetMask(s),
					     &ms->mask.width, &ms->mask.height);

	if (!ms->mask.loaded) {
		compWarn("Could not load magnifier mask image \"%s\"!",
			 magGetOverlay(s));
		ms->overlay.loaded = FALSE;
		finiTexture(s, &ms->overlay.tex);
		initTexture(s, &ms->overlay.tex);
	...
	if (ms->overlay.width != ms->mask.width ||
		...
		compWarn("Image dimensions do not match!");
		ms->overlay.loaded = FALSE;
		finiTexture(s, &ms->overlay.tex);
		initTexture(s, &ms->overlay.tex);
		ms->mask.loaded = FALSE;
...
static void
magOptionsChanged(CompScreen * s, CompOption * opt, MagScreenOptions num)
{
	MAG_SCREEN(s);
...
static void damageRegion(CompScreen * s)
{
	REGION r;
	...
	case ModeSimple:
		{
			...
			r.extents.x1 =
			    MAX(0, MIN(ms->posX - (w / 2), s->width - w));
			r.extents.x2 = r.extents.x1 + w;
			r.extents.y1 =
			    MAX(0, MIN(ms->posY - (h / 2), s->height - h));
...
static int adjustZoom(CompScreen * s, float chunk)
{
...
static void magPreparePaintScreen(CompScreen * s, int time)
{
	MAG_SCREEN(s);
	...
	if (ms->zoom != 1.0) {
		if (!ms->pollHandle) {
			(*md->mpFunc->getCurrentPosition) (s, &ms->posX,
							   &ms->posY);
			ms->pollHandle =
			    (*md->mpFunc->addPositionPolling) (s,
							       positionUpdate);
		}
		damageRegion(s);
	}
...
static void magDonePaintScreen(CompScreen * s)
{
	MAG_SCREEN(s);
	MAG_DISPLAY(s->display);
...
static void magPaintSimple(CompScreen * s)
{
	float pw, ph, bw, bh;
	int x1, x2, y1, y2;
	float vc[4];
...
static void magPaintImage(CompScreen * s)
{
	float pw, ph;
	int x1, x2, y1, y2;
	float vc[4];
...
static void magPaintFisheye(CompScreen * s)
{
	float pw, ph;
...
static Bool magInitScreen(CompPlugin * p, CompScreen * s)
{
	MagScreen *ms;
	...
	if (!s->fragmentProgram)
		compWarn("GL_ARB_fragment_program not supported. "
			 "Fisheye mode will not work.");

	return TRUE;
}

static void magFiniScreen(CompPlugin * p, CompScreen * s)
{
	MAG_SCREEN(s);
	MAG_DISPLAY(s->display);
...
static Bool magInitDisplay(CompPlugin * p, CompDisplay * d)
{
...
static void magFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	MAG_DISPLAY(d);
...
static Bool magInit(CompPlugin * p)
{
...
static void magFini(CompPlugin * p)
{
	freeDisplayPrivateIndex(displayPrivateIndex);
}

static CompBool magInitObject(CompPlugin * p, CompObject * o)
{
...
static void magFiniObject(CompPlugin * p, CompObject * o)
{
