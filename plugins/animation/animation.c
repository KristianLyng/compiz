...
static void updateEventEffects(CompScreen * s, AnimEvent e, Bool forRandom)
{
	ANIM_SCREEN(s);
...
static void updateAllEventEffects(CompScreen * s)
{
	AnimEvent e;
...
static void freeAllEffects(AnimScreen * as)
{
	AnimEvent e;
...
static void
animAddExtension(CompScreen * s, ExtensionPluginInfo * extensionPluginInfo)
{
	ANIM_SCREEN(s);
	...
	for (j = 0; j < nPluginEffects; j++) {
		...
		for (e = 0; e < AnimEventNum; e++) {
			if (effect->usedForEvents[e]) {
				as->eventEffectsAllowed[e][as->
							   nEventEffectsAllowed
							   [e]++] = effect;
...
static void
animRemoveExtension(CompScreen * s, ExtensionPluginInfo * extensionPluginInfo)
{
	ANIM_SCREEN(s);
	...
	for (e = 0; e < AnimEventNum; e++) {
		...
		for (nUpto = 0; nUpto < n; nUpto++) {
			...
			if (0 ==
			    strncmp(pluginName,
				    eventEffectsAllowed[nUpto]->name,
				    pluginNameLen))
				break;
		}
...
Bool defaultAnimInit(CompWindow * w)
{
	...
	aw->com.timestep = (w->screen->slowAnimations ? 2 :	// For smooth slow-mo (refer to display.c)
			    as->opt[ANIM_SCREEN_OPTION_TIME_STEP].value.i);

	return TRUE;
}

Bool animZoomToIcon(CompWindow * w)
{
	ANIM_WINDOW(w);
...
static Bool defaultMinimizeAnimInit(CompWindow * w)
{
	ANIM_WINDOW(w);
...
static Bool animWithTransformInit(CompWindow * w)
{
	ANIM_WINDOW(w);
...
static inline Bool returnTrue(CompWindow * w)
{
	return TRUE;
}
...
static AnimEffect
getMatchingAnimSelection(CompWindow * w, AnimEvent e, int *duration)
{
	ANIM_SCREEN(w->screen);
	ANIM_WINDOW(w);
...
animGetAnimEffect(AnimScreen * as, AnimEffect effect, AnimEvent animEvent)
{
	...
	if ((effect == AnimEffectRandom) || allRandom) {
		if (nRandomEffects == 0)	// no random animation selected, assume "all"
		{
			...
			nRandomEffects =
			    as->nEventEffectsAllowed[animEvent] - 2;
		}
		unsigned int index;
		index =
...
AnimDirection getActualAnimDirection(CompWindow * w,
				     AnimDirection dir, Bool openDir)
{
	ANIM_WINDOW(w);
	...
	if (dir == AnimDirectionRandom) {
		...
		float relDiffX =
		    ((float)centerX - aw->com.icon.x) / BORDER_W(w);
		float relDiffY =
		    ((float)centerY - aw->com.icon.y) / BORDER_H(w);
		...
		if (openDir) {
			if (aw->com.curWindowEvent == WindowEventMinimize ||
				...
				dir =
				    ...
				    w->screen->height -
				    aw->com.icon.
				    y ? AnimDirectionDown : AnimDirectionUp;
			else if (fabs(relDiffY) > fabs(relDiffX))
				dir =
				    relDiffY >
				    0 ? AnimDirectionDown : AnimDirectionUp;
			else
				dir =
				    relDiffX >
				    0 ? AnimDirectionRight : AnimDirectionLeft;
		} else {
			if (aw->com.curWindowEvent == WindowEventMinimize ||
			    aw->com.curWindowEvent == WindowEventUnminimize)
				// min/unmin. should always result in +/- y direction
				dir =
				    ...
				    w->screen->height -
				    aw->com.icon.
				    y ? AnimDirectionUp : AnimDirectionDown;
			else if (fabs(relDiffY) > fabs(relDiffX))
				dir =
				    relDiffY >
				    0 ? AnimDirectionUp : AnimDirectionDown;
			else
				dir =
				    relDiffX >
				    0 ? AnimDirectionLeft : AnimDirectionRight;
		}
	}
	return dir;
}
...
float defaultAnimProgress(CompWindow * w)
{
	...
	float forwardProgress =
	    1 - aw->com.animRemainingTime / (aw->com.animTotalTime -
					     aw->com.timestep);
...
float sigmoidAnimProgress(CompWindow * w)
{
	...
	forwardProgress =
	    (sigmoid(forwardProgress) - sigmoid(0)) / (sigmoid(1) - sigmoid(0));
...
float getProgressAndCenter(CompWindow * w, Point * center)
{
...
void defaultAnimStep(CompWindow * w, float time)
{
	...
	float timestep = (w->screen->slowAnimations ? 2 :	// For smooth slow-mo (refer to display.c)
			  as->opt[ANIM_SCREEN_OPTION_TIME_STEP].value.i);
...
void defaultUpdateWindowTransform(CompWindow * w, CompTransform * wTransform)
{
	ANIM_WINDOW(w);
	...
	if (aw->com.usingTransform) {
		if (aw->com.curAnimEffect->properties.modelAnimIs3D) {
			...
			applyPerspectiveSkew(as->output, &skewTransform,
					     &center);
...
inline void
applyTransform(CompTransform * wTransform, CompTransform * transform)
{
	matrixMultiply(wTransform, wTransform, transform);
}

static void copyResetBB(AnimWindow * aw)
{
...
expandBoxWithPoint2DTransform(CompScreen * s,
			      Box * target,
			      CompVector * coords, CompTransform * transformMat)
{
			       ...
			       Object * objects, int nPoints)
{
	...
	GLint viewport[4] = { output->region.extents.x1,
	...
	if (points)		// use points
	{
		for (; nPoints; nPoints--, points += 3) {
			...
			expandBoxWithPoint(targetBox, x + 0.5,
					   (s->height - y) + 0.5);
		}
	} else			// use objects
	{
...
static void modelUpdateBB(CompOutput * output, CompWindow * w, Box * BB)
{
	int i;
	...
	if (aw->com.usingTransform) {
		if (aw->com.curAnimEffect->properties.modelAnimIs3D) {
			...
			memcpy(fullTransform.m, aw->com.transform.m,
			       sizeof(float) * 16);
			applyPerspectiveSkew(output, &fullTransform, &center);

			prepareTransform(w->screen, output, &wTransform,
					 &fullTransform);
...
void updateBBWindow(CompOutput * output, CompWindow * w, Box * BB)
{
...
void updateBBScreen(CompOutput * output, CompWindow * w, Box * BB)
{
...
prepareTransform(CompScreen * s,
		 CompOutput * output,
		 CompTransform * resultTransform, CompTransform * transform)
{
...
void compTransformUpdateBB(CompOutput * output, CompWindow * w, Box * BB)
{
	ANIM_WINDOW(w);
...
static const CompMetadataOptionInfo animScreenOptionInfo[] = {
	...
	// Effect settings
	{"curved_fold_amp_mult", "float", "<min>-1.5</min><max>2.0</max>", 0,
	 0},
	...
	{"horizontal_folds_amp_mult", "float", "<min>-1.0</min><max>3.0</max>",
	 0, 0},
	...
	{"sidekick_zoom_from_center", "int",
	 RESTOSTRING(0, LAST_ZOOM_FROM_CENTER), 0, 0},
	...
	{"zoom_from_center", "int", RESTOSTRING(0, LAST_ZOOM_FROM_CENTER), 0,
	 0},
	{"zoom_springiness", "float", "<min>0</min><max>1</max>", 0, 0}
};

static CompOption *animGetScreenOptions(CompPlugin * plugin,
...
void modelInitObjects(Model * model, int x, int y, int width, int height)
{
	int gridX, gridY;
	...
	if (model->forWindowEvent == WindowEventShade ||
			...
			for (gridX = 0; gridX < model->gridWidth; gridX++) {
				objectInit(&model->
					   ...
					   x + ((gridX * width / nGridCellsX) -
						x0) * model->scale.x + x0,
					   objectY, (float)gridX / nGridCellsX,
					   gridPosY);
			}
		}
		...
		for (gridX = 0; gridX < model->gridWidth; gridX++) {
			objectInit(&model->
				   objects[gridY * model->gridWidth + gridX],
				   x + ((gridX * width / nGridCellsX) -
...
static void animFreeModel(AnimWindow * aw)
{
	if (!aw->com.model)
		return;
...
static Bool animEnsureModel(CompWindow * w)
{
	ANIM_WINDOW(w);
	...
	if (forAnimEffect->properties.initGridFunc)
		forAnimEffect->properties.initGridFunc(w, &gridWidth,
						       &gridHeight);
...
static void cleanUpParentChildChainItem(AnimScreen * as, AnimWindow * aw)
{
	if (aw->winThisIsPaintedBefore
	    && !aw->winThisIsPaintedBefore->destroyed) {
		AnimWindow *aw2 =
...
static void clearDodgersSubject(AnimScreen * as, CompWindow * w)
{
	CompWindow *dw;
	AnimWindow *adw;
...
static void removeFromDodgeChain(AnimScreen * as, CompWindow * dw)
{
	...
	if (adw->dodgeSubjectWin) {
		AnimWindow *awSubject =
		    GET_ANIM_WINDOW(adw->dodgeSubjectWin, as);
...
static void postAnimationCleanupCustom(CompWindow * w,
				       Bool closing,
				       Bool finishing, Bool clearMatchingRow)
{
	ANIM_WINDOW(w);
	ANIM_SCREEN(w->screen);
	...
	aw->dodgeMaxAmount = 0;
	if (!(aw->moreToBePaintedPrev || aw->moreToBePaintedNext)) {
		aw->isDodgeSubject = FALSE;
		aw->skipPostPrepareScreen = FALSE;
	}
...
static void
postAnimationCleanupPrev(CompWindow * w, Bool closing, Bool clearMatchingRow)
{
	ANIM_WINDOW(w);
...
static void animActivateEvent(CompScreen * s, Bool activating)
{
	ANIM_SCREEN(s);
	...
	(*s->display->handleCompizEvent) (s->display, "animation", "activate",
					  o, 2);
}
...
static Bool otherPluginsActive(AnimScreen * as)
{
	int i;
...
static inline Bool isWinVisible(CompWindow * w)
{
	return (!w->destroyed &&
		!(!w->shaded && (w->attrib.map_state != IsViewable)));
}

static inline void
getHostedOnWin(AnimScreen * as, CompWindow * w, CompWindow * wHost)
{
	ANIM_WINDOW(w);
...
static void initiateFocusAnimation(CompWindow * w)
{
	CompScreen *s = w->screen;
	ANIM_SCREEN(s);
	ANIM_WINDOW(w);
	...
	if (chosenEffect != AnimEffectNone &&
		...
		if (chosenEffect == AnimEffectFocusFade ||
			...
			XUnionRectWithRegion(&rect, &emptyRegion,
					     subjectWinRegion);
			...
			for (dw = wStart; dw && dw != wEnd->next; dw = dw->next) {
				...
				if (w == dw
				    && chosenEffect == AnimEffectFocusFade)
					continue;

				Bool nonMatching = FALSE;
				if (chosenEffect == AnimEffectDodge &&
				    getMatchingAnimSelection(dw, AnimEventFocus,
							     NULL) !=
				    chosenEffect)
					nonMatching = TRUE;
				...
				XUnionRectWithRegion(&rect, &emptyRegion,
						     thisWinRegion);
				XIntersectRegion(subjectWinRegion,
						 thisWinRegion,
						 thisAndSubjectIntersection);
				XUnionRegion(fadeRegion,
					     thisAndSubjectIntersection,
					     fadeRegion);

				if (chosenEffect == AnimEffectFocusFade) {
					adw->winPassingThrough = w;
				} else if (chosenEffect == AnimEffectDodge && !XEmptyRegion(thisAndSubjectIntersection) && (adw->com.curAnimEffect == AnimEffectNone || (adw->com.curAnimEffect == AnimEffectDodge)) && dw->id != w->id)	// don't let the subject dodge itself
				{
			...
			if (chosenEffect == AnimEffectDodge) {
				...
				for (dw = wStart; dw && dw != wEnd->next;
				     dw = dw->next) {
					AnimWindow *adw =
					    GET_ANIM_WINDOW(dw, as);
					...
					if (adw->com.curAnimEffect !=
					    AnimEffectDodge) {
						adw->com.curAnimEffect =
						    AnimEffectDodge;
						as->walkerAnimCount++;
					}
					...
					if (maxTransformTotalProgress <
					    transformTotalProgress)
						maxTransformTotalProgress =
						    transformTotalProgress;
					...
					if (wDodgeChainLastVisited) {
						AnimWindow
						    *awDodgeChainLastVisited =
						    GET_ANIM_WINDOW
						    (wDodgeChainLastVisited,
						     as);
						if (raised)
							awDodgeChainLastVisited->
							    dodgeChainNext = dw;
...
static Bool relevantForFadeFocus(CompWindow * nw)
{
	if (!((nw->wmType &
...
static Bool restackInfoStillGood(CompScreen * s, RestackInfo * restackInfo)
{
...
static void resetStackingInfo(CompScreen * s)
{
	CompWindow *w;
	for (w = s->windows; w; w = w->next) {
		ANIM_WINDOW(w);
...
static Bool wontCreateCircularChain(CompWindow * wCur, CompWindow * wNext)
{
...
static void animPreparePaintScreen(CompScreen * s, int msSinceLastPaint)
{
	...
	if (as->aWinWasRestackedJustNow) {
		...
		for (w = s->reverseWindows; w; w = w->prev) {
			...
			CompWindow *dw;
			AnimWindow *adw;
			for (dw = aw->dodgeChainStart; dw;
			     dw = adw->dodgeChainNext) {
				adw = GET_ANIM_WINDOW(dw, as);
				if (!adw)
					break;
	...
	if (as->animInProgress) {
		...
		for (w = s->windows; w; w = w->next) {
			...
			if (aw->com.animRemainingTime > 0 &&
				...
				if (aw->com.curAnimEffect->properties.
				    prePrepPaintScreenFunc
				    && aw->com.curAnimEffect->properties.
				    prePrepPaintScreenFunc(w, msSinceLastPaint))
				...
				if (aw->com.curAnimEffect->properties.
				    animStepFunc)
					aw->com.curAnimEffect->properties.
					    animStepFunc(w, msSinceLastPaint);
...
void perspectiveDistortAndResetZ(CompScreen * s, CompTransform * transform)
{
	float v = -1.0 / s->width;
	/*
	   This does
...
applyPerspectiveSkew(CompOutput * output,
		     CompTransform * transform, Point * center)
{
	...
	if (aw->com.animRemainingTime > 0 && aw->com.model &&
		...
		if (aw->com.curAnimEffect->properties.addCustomGeometryFunc) {
			if (nMatrix == 0)
				return;
			aw->com.curAnimEffect->
			    properties.addCustomGeometryFunc(w, nClip, pClip,
							     nMatrix, matrix);
		...
		for (; nClip--; pClip++) {
			...
			for (jy = 0; jy < nVertY - 1; jy++) {
				for (jx = 0; jx < nVertX - 1; jx++) {
					*i++ =
					    nVertices + nVertX * (2 * jy + 1) +
					    jx;
					*i++ =
					    nVertices + nVertX * (2 * jy + 1) +
					    jx + 1;
					*i++ =
					    nVertices + nVertX * 2 * jy + jx +
					    1;
			...
			for (jy = 0, y = y1; jy < nVertY; jy++) {
				...
				for (jx = 0, x = x1; jx < nVertX; jx++) {
					...
					deformedX =
					    (1 - iny) * hor1x + iny * hor2x;
					deformedY =
					    (1 - iny) * hor1y + iny * hor2y;
					deformedZ =
					    (1 - iny) * hor1z + iny * hor2z;
		...
		(*w->screen->addWindowGeometry) (w, matrix, nMatrix, region,
						 clip);
		...
		Region region, unsigned int mask)
{
	...
	if (aw->com.animRemainingTime > 0) {
		if (!as->animInProgress) {
			...
			status =
			    (*w->screen->paintWindow) (w, attrib, transform,
						       region, mask);
		...
		if (aw->com.curAnimEffect->properties.updateWindowAttribFunc)
			aw->com.curAnimEffect->
			    properties.updateWindowAttribFunc(w, &wAttrib);

		if (aw->com.curAnimEffect->properties.updateWinTransformFunc)
			aw->com.curAnimEffect->
			    properties.updateWinTransformFunc(w, &wTransform);
		...
		UNWRAP(as, w->screen, paintWindow);
		status =
		    (*w->screen->paintWindow) (w, &wAttrib, &wTransform, region,
					       mask);
		WRAP(as, w->screen, paintWindow, animPaintWindow);
		...
		UNWRAP(as, w->screen, paintWindow);
		status =
		    (*w->screen->paintWindow) (w, attrib, transform, region,
					       mask);
		WRAP(as, w->screen, paintWindow, animPaintWindow);
	}
...
// dialog is raised.
static CompWindow *getBottommostInFocusChain(CompWindow * w)
{
	if (!w)
		return w;
...
static void resetWalkerMarks(CompScreen * s)
{
	CompWindow *w;
	for (w = s->windows; w; w = w->next) {
		ANIM_WINDOW(w);
...
static CompWindow *animWalkFirst(CompScreen * s)
{
	ANIM_SCREEN(s);
...
static CompWindow *animWalkLast(CompScreen * s)
{
	ANIM_SCREEN(s);
...
static Bool markNewCopy(CompWindow * w)
{
	ANIM_WINDOW(w);

	// if window is in a focus chain
	if (aw->winThisIsPaintedBefore || aw->moreToBePaintedPrev) {
...
static CompWindow *animWalkNext(CompWindow * w)
{
	ANIM_WINDOW(w);
...
static CompWindow *animWalkPrev(CompWindow * w)
{
	ANIM_WINDOW(w);
	CompWindow *wRet = NULL;
...
static void animInitWindowWalker(CompScreen * s, CompWalker * walker)
{
	ANIM_SCREEN(s);
				  ...
				  int nOption)
{
	...
	for (i = 0; i < NUM_WATCHED_PLUGINS; i++)
		if (strcmp(pluginName, watchedPlugins[i].pluginName) == 0) {
			if (strcmp
			    (eventName,
			     watchedPlugins[i].activateEventName) == 0) {
				Window xid =
				...
				if (s) {
					ANIM_SCREEN(s);
					as->pluginActive[i] =
					    getBoolOptionNamed(option, nOption,
							       "active", FALSE);
...
static void updateLastClientListStacking(CompScreen * s)
{
	ANIM_SCREEN(s);
	int n = s->nClientList;
...
static inline Bool shouldIgnoreForAnim(CompWindow * w, Bool checkPixmap)
{
...
static void animHandleEvent(CompDisplay * d, XEvent * event)
{
	...
	case PropertyNotify:
		if (event->xproperty.atom == d->clientListStackingAtom) {
			CompScreen *s =
			    findScreenAtDisplay(d, event->xproperty.window);
			if (s)
	...
	case DestroyNotify:
		...
		if (w) {
			...
			if (AnimEffectNone ==
			    getMatchingAnimSelection(w, AnimEventClose,
						     &duration))
				break;
	...
	case UnmapNotify:
		...
		if (w) {
			...
			if (w->pendingUnmaps && onCurrentDesktop(w))	// Normal -> Iconic
			{
				...
				int duration = 200;
				AnimEffect chosenEffect =
				    getMatchingAnimSelection(w, AnimEventShade,
							     &duration);
				...
				if (w->shaded) {
					...
					if (chosenEffect != AnimEffectNone) {
						...
						if (aw->com.curWindowEvent !=
						    WindowEventNone) {
							if (aw->com.
							    curWindowEvent !=
							    WindowEventUnshade)
							{
								postAnimationCleanupPrev
								    (w, FALSE,
								     FALSE);
							} else {
			...
			if (!winOpenedClosed) {
				...
				if (changeStart >= 0 && changeEnd >= 0) {
					...
					if (preferRaised ||
						...
						wEnd = wRestacked;
						wOldAbove = wStart;
					} else if (clientListStacking[changeStart] == as->lastClientListStacking[changeEnd] &&	// lowered
					...
					for (;
					     wOldAbove
					     && !isWinVisible(wOldAbove);
					     wOldAbove = wOldAbove->next) ;
				}
				if (wRestacked && wStart && wEnd && wOldAbove) {
					AnimWindow *awRestacked =
					    GET_ANIM_WINDOW(wRestacked, as);
	...
	case PropertyNotify:
		if (event->xproperty.atom == d->winActiveAtom &&
			...
			if (w) {
				int duration = 200;
				AnimEffect chosenEffect =
				    getMatchingAnimSelection(w, AnimEventFocus,
							     &duration);
...
static Bool animDamageWindowRect(CompWindow * w, Bool initial, BoxPtr rect)
{
	...
	if (initial)		// Unminimize or Open
	{
		...
		if (aw->state == IconicState) {
			chosenEffect =
			    getMatchingAnimSelection(w, AnimEventMinimize,
						     &duration);

			// UNMINIMIZE event!

			if (!w->invisible && !w->hidden && chosenEffect != AnimEffectNone && !as->pluginActive[3])	// fadedesktop
			{
				Bool startingNew = TRUE;
			...
			if (!otherPluginsActive(as) &&
			    ...
			    AnimEffectNone !=
			    (chosenEffect =
			     getMatchingAnimSelection(w, AnimEventOpen,
						      &duration))
			    && getMousePointerXY(w->screen, &aw->com.icon.x,
						 &aw->com.icon.y)) {
				Bool startingNew = TRUE;
				Bool playEffect = TRUE;
				   ...
				   int dheight)
{
	...
	if (!(aw->com.curWindowEvent == WindowEventOpen &&
		...
		CompWindowTypeComboMask | CompWindowTypeDndMask)))) {
		if (aw->com.curAnimEffect->properties.refreshFunc)
			aw->com.curAnimEffect->properties.refreshFunc(w,
								      aw->
								      animInitialized);
...
static void animWindowMoveNotify(CompWindow * w, int dx, int dy, Bool immediate)
{
	ANIM_SCREEN(w->screen);
	ANIM_WINDOW(w);
	...
	if (!immediate) {
		if (!(aw->com.animRemainingTime > 0 &&
			...
			if (aw->com.animRemainingTime > 0 && aw->grabbed) {
				...
				if (as->animInProgress) {
					Bool animStillInProgress = FALSE;
					for (w2 = w->screen->windows; w2;
					     w2 = w2->next) {
						AnimWindow *aw2;

						aw2 = GET_ANIM_WINDOW(w2, as);
						if (aw2->com.animRemainingTime >
						    0) {
							animStillInProgress =
							    TRUE;
							break;
			...
			if (aw->com.model) {
				modelInitObjects(aw->com.model, WIN_X(w),
		...
		Region region, CompOutput * output, unsigned int mask)
{
	...
	if (as->animInProgress) {
		int p;
		for (p = 0; p < as->nExtensionPlugins; p++) {
			const ExtensionPluginInfo *extPlugin =
			    as->extensionPlugins[p];
...
static CompOption *animGetDisplayOptions(CompPlugin * plugin,
...
static AnimWindowCommon *getAnimWindowCommon(CompWindow * w)
{
	ANIM_WINDOW(w);
...
AnimEffect AnimEffectNone = &(AnimEffectInfo) { "animation:None",
	{TRUE, TRUE, TRUE, TRUE, TRUE}
};

AnimEffect AnimEffectRandom = &(AnimEffectInfo) { "animation:Random",
	{TRUE, TRUE, TRUE, TRUE, FALSE}
};

AnimEffect AnimEffectCurvedFold = &(AnimEffectInfo) { "animation:Curved Fold",
	{TRUE, TRUE, TRUE, TRUE, FALSE},
...
AnimEffect AnimEffectDodge = &(AnimEffectInfo) { "animation:Dodge",
...
AnimEffect AnimEffectDream = &(AnimEffectInfo) { "animation:Dream",
...
AnimEffect AnimEffectFade = &(AnimEffectInfo) { "animation:Fade",
	{TRUE, TRUE, TRUE, FALSE, FALSE},
...
AnimEffect AnimEffectFocusFade = &(AnimEffectInfo) { "animation:Focus Fade",
	{FALSE, FALSE, FALSE, FALSE, TRUE},
...
AnimEffect AnimEffectGlide1 = &(AnimEffectInfo) { "animation:Glide 1",
	{TRUE, TRUE, TRUE, FALSE, FALSE},
...
AnimEffect AnimEffectGlide2 = &(AnimEffectInfo) { "animation:Glide 2",
	{TRUE, TRUE, TRUE, FALSE, FALSE},
	{.updateWindowAttribFunc = fxGlideUpdateWindowAttrib,
	 .prePaintWindowFunc = fxGlidePrePaintWindow,
	 .postPaintWindowFunc = fxGlidePostPaintWindow,
...
AnimEffect AnimEffectMagicLamp = &(AnimEffectInfo) { "animation:Magic Lamp",
	{TRUE, TRUE, TRUE, FALSE, FALSE},
...
AnimEffect AnimEffectRollUp = &(AnimEffectInfo) { "animation:Roll Up",
	{TRUE, TRUE, TRUE, TRUE, FALSE},
...
AnimEffect AnimEffectSidekick = &(AnimEffectInfo) { "animation:Sidekick",
	{TRUE, TRUE, TRUE, FALSE, FALSE},
...
AnimEffect AnimEffectVacuum = &(AnimEffectInfo) { "animation:Vacuum",
...
AnimEffect AnimEffectWave = &(AnimEffectInfo) { "animation:Wave",
...
AnimEffect AnimEffectZoom = &(AnimEffectInfo) { "animation:Zoom",
	{TRUE, TRUE, TRUE, FALSE, FALSE},
	{.updateWindowAttribFunc = fxZoomUpdateWindowAttrib,
	 .animStepFunc = defaultAnimStep,
	 .initFunc = fxZoomInit,
...
static Bool animInitScreen(CompPlugin * p, CompScreen * s)
{
	...
	memcpy(animEffects, animEffectsTmp, NUM_EFFECTS * sizeof(AnimEffect));
...
static CompBool animInitObject(CompPlugin * p, CompObject * o)
{
...
static void animFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *animGetObjectOptions(CompPlugin * plugin,
...
static CompMetadata *animGetMetadata(CompPlugin * plugin)
{
	return &animMetadata;
}
