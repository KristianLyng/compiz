...
static char *programFindOffset(FragmentOffset * offset, char *name);
 ...
 */
static char *programCleanName(char *name)
{
	char *dest, *current;
...
static char *programReadSource(char *fname)
{
	...
	if (!fp && home && strlen(home)) {
		...
		if (ret <= 0) {
			fprintf(stderr,
				"asprintf() failed. This is not good.\n");
			return NULL;
		}
		fp = fopen(path, "r");
		free(path);
 ...
 */
static FragmentOffset *programAddOffsetFromAddOp(FragmentOffset * offsets,
						 char *source)
{
 ...
 */
static void programFreeOffset(FragmentOffset * offset)
{
	if (!offset)
		return;
...
static void
programParseSource(CompFunctionData * data, int target, char *source)
{
	...
	while (line) {
		...
		if (strncmp(current, "END", 3) == 0)
			type = NoOp;
		else if (!strncmp(current, "ABS", 3)
			 || !strncmp(current, "CMP", 3)
			 || !strncmp(current, "COS", 3)
			 || !strncmp(current, "DP3", 3)
			 || !strncmp(current, "DP4", 3)
			 || !strncmp(current, "EX2", 3)
			 || !strncmp(current, "FLR", 3)
			 || !strncmp(current, "FRC", 3)
			 || !strncmp(current, "KIL", 3)
			 || !strncmp(current, "LG2", 3)
			 || !strncmp(current, "LIT", 3)
			 || !strncmp(current, "LRP", 3)
			 || !strncmp(current, "MAD", 3)
			 || !strncmp(current, "MAX", 3)
			 || !strncmp(current, "MIN", 3)
			 || !strncmp(current, "POW", 3)
			 || !strncmp(current, "RCP", 3)
			 || !strncmp(current, "RSQ", 3)
			 || !strncmp(current, "SCS", 3)
			 || !strncmp(current, "SIN", 3)
			 || !strncmp(current, "SGE", 3)
			 || !strncmp(current, "SLT", 3)
			 || !strncmp(current, "SUB", 3)
			 || !strncmp(current, "SWZ", 3)
			 || !strncmp(current, "TXB", 3)
			 || !strncmp(current, "TXP", 3)
			...
			if (strstr(current, "fragment.texcoord"))
				offsets =
				    programAddOffsetFromAddOp(offsets, current);
			else
				type = DataOp;
		...
		case ParamOp:
			...
			while (current && *current &&
				...
				if (type == TempOp)
					...
					addParamHeaderOpToFunctionData(data,
								       arg1);
				else if (type == AttribOp)
					addAttribHeaderOpToFunctionData(data,
									arg1);
				free(arg1);
			}
			break;
		case FetchOp:
			...
			if ((arg1 = getFirstArgument(&current))) {
				...
				if (strcmp(temp, "fragment.texcoord[0]") == 0)
					addFetchOpToFunctionData(data, arg1,
								 NULL, target);
				else {
					arg2 = programFindOffset(offsets, temp);
					if (arg2) {
						addFetchOpToFunctionData(data,
									 arg1,
									 arg2,
									 target);
						free(arg2);
