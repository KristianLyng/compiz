...
static void setOpacity(CompWindow * w, int opacity)
{
	OPACIFY_WINDOW(w);
...
*/
static void resetOpacity(CompScreen * s, Window id)
{
	CompWindow *w;
...
*/
static void clearPassive(CompScreen * s)
{
	int i;
 ...
 */
static void dimWindow(CompWindow * w)
{
	OPACIFY_SCREEN(w->screen);

	if (os->passiveNum >= MAX_WINDOWS - 1) {
		compWarn("Trying to store information "
 ...
 */
static int passiveWindows(CompScreen * s, Region region)
{
	CompWindow *w;
	Bool flag = FALSE;
	int i = 0;
 ...
 */
static void opacifyHandleEnter(CompScreen * s, CompWindow * w)
{
	OPACIFY_SCREEN(s);
	...
	if (w->id != os->active && !w->shaded &&
		...
		if (num || opacifyGetOnlyIfBlock(s))
			setOpacity(w,
				   MAX(OPAQUE * opacifyGetActiveOpacity(s) /
				       100, w->paint.opacity));
	}
}
 ...
 */
static Bool checkScreenSwitch(CompScreen * s)
{
	CompScreen *tmp;
 ...
 */
static Bool handleTimeout(void *data)
{
 ...
 */
static inline Bool checkDelay(CompScreen * s)
{
	...
	case EnterNotify:
		...
		if (s) {
			...
			if (checkDelay(s))
				...
				od->timeoutHandle =
				    compAddTimeout(opacifyGetTimeout(d), (float)
...
opacifyDisplayOptionChanged(CompDisplay * d,
			    CompOption * opt, OpacifyDisplayOptions num)
{
	OPACIFY_DISPLAY(d);
...
static CompBool opacifyInitObject(CompPlugin * p, CompObject * o)
{
...
static void opacifyFiniObject(CompPlugin * p, CompObject * o)
{
