...
typedef struct _AnimEffectProperties {
	void (*updateWindowAttribFunc) (CompWindow *, WindowPaintAttrib *);
	...
	void (*updateWinTransformFunc) (CompWindow *, CompTransform *);
...
typedef void
 (*UpdateBBProc) (CompOutput * output, CompWindow * w, Box * BB);
...
typedef struct _AnimBaseFunctions {
	...
	CompOptionValue *(*getPluginOptVal)
	 (CompWindow * w, ExtensionPluginInfo * extensionPluginInfo,
	  int optionId);
