...
static void putFinishWindowMovement(CompWindow * w)
{
	PUT_WINDOW(w);
...
static unsigned int putGetOutputForWindow(CompWindow * w)
{
	PUT_WINDOW(w);
...
putGetDistance(CompWindow * w,
	       PutType type,
	       CompOption * option, int nOption, int *distX, int *distY)
{
	CompScreen *s = w->screen;
	...
	case PutViewport:
		{
			...
			viewport =
			    getIntOptionNamed(option, nOption, "viewport", -1);
	...
	case PutNextOutput:
		{
			...
			dx = (newOutput->region.extents.x1 +
			      newOutput->width / 2) -
			    (currentOutput->region.extents.x1 +
			     currentOutput->width / 2);
			dy = (newOutput->region.extents.y1 +
			      newOutput->height / 2) -
			    (currentOutput->region.extents.y1 +
			     currentOutput->height / 2);
	...
	case PutAbsolute:
		...
		if (posX < 0)
			...
			dx = posX + s->width - w->serverWidth - x -
			    RIGHT_BORDER(w);
		else
		...
		if (posY < 0)
			/* account for a specified negative position,
			   like geometry without (-0) */
			dy = posY + s->height - w->serverHeight - y -
			    BOTTOM_BORDER(w);
		else
	...
	case PutPointer:
		{
			...
			if (XQueryPointer
			    (s->display->display, w->id, &root, &child, &rx,
			     &ry, &winX, &winY, &pMask)) {
				if (putGetWindowCenter(s)) {
					...
					/* bottom left quad */
					dx = rx - x + LEFT_BORDER(w);
					dy = ry - w->serverHeight - y -
					    BOTTOM_BORDER(w);
				} else if (rx >= s->workArea.width / 2
					   && ry < s->workArea.height / 2) {
					/* top right quad */
					dx = rx - w->serverWidth - x -
					    RIGHT_BORDER(w);
					dy = ry - y + TOP_BORDER(w);
				} else {
					/* bottom right quad */
...
putInitiateCommon(CompDisplay * d,
		  ...
		  CompOption * option, int nOption, PutType type)
{
	CompWindow *w;
	Window xid;
	...
	if (w) {
		...
		if (!ps->grabIndex) {
			...
			ps->grabIndex =
			    pushScreenGrab(s, s->invisibleCursor, "put");
		}
...
static PutType putTypeFromString(const char *type)
{
	    ...
	    CompActionState state, CompOption * option, int nOption)
{
	...
	if (type == PutViewport)
		...
		return putInitiateCommon(d, action, state, option, nOption,
					 type);
}
...
restore(CompDisplay * d,
	...
	CompActionState state, CompOption * option, int nOption)
{
	return putInitiateCommon(d, action, state, option, nOption, PutRestore);
}
...
putPointer(CompDisplay * d,
	   ...
	   CompActionState state, CompOption * option, int nOption)
{
	return putInitiateCommon(d, action, state, option, nOption, PutPointer);
}
...
putCenter(CompDisplay * d,
	  ...
	  CompActionState state, CompOption * option, int nOption)
{
	return putInitiateCommon(d, action, state, option, nOption, PutCenter);
}
...
putLeft(CompDisplay * d,
	...
	CompActionState state, CompOption * option, int nOption)
{
	return putInitiateCommon(d, action, state, option, nOption, PutLeft);
}
...
putTopLeft(CompDisplay * d,
	   ...
	   CompActionState state, CompOption * option, int nOption)
{
	return putInitiateCommon(d, action, state, option, nOption, PutTopLeft);
}
...
putTop(CompDisplay * d,
       ...
       CompActionState state, CompOption * option, int nOption)
{
	return putInitiateCommon(d, action, state, option, nOption, PutTop);
}
...
putRight(CompDisplay * d,
	 ...
	 CompActionState state, CompOption * option, int nOption)
{
	return putInitiateCommon(d, action, state, option, nOption, PutRight);
}
...
putBottom(CompDisplay * d,
	  ...
	  CompActionState state, CompOption * option, int nOption)
{
	return putInitiateCommon(d, action, state, option, nOption, PutBottom);
}
...
static CompBool putInitObject(CompPlugin * p, CompObject * o)
{
...
static void putFiniObject(CompPlugin * p, CompObject * o)
{
