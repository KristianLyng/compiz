...
static void regexMatchExpFini(CompDisplay * d, CompPrivate private)
{
...
static Bool
regexMatchExpEvalTitle(CompDisplay * d, CompWindow * w, CompPrivate private)
{
	regex_t *preg = (regex_t *) private.ptr;
	int status;
...
static Bool
regexMatchExpEvalRole(CompDisplay * d, CompWindow * w, CompPrivate private)
{
	regex_t *preg = (regex_t *) private.ptr;
	int status;
...
static Bool
regexMatchExpEvalClass(CompDisplay * d, CompWindow * w, CompPrivate private)
{
	regex_t *preg = (regex_t *) private.ptr;
	int status;
...
static Bool
regexMatchExpEvalName(CompDisplay * d, CompWindow * w, CompPrivate private)
{
	regex_t *preg = (regex_t *) private.ptr;
	int status;
...
static void
regexMatchInitExp(CompDisplay * d, CompMatchExp * exp, const char *value)
{
	...
	if (i < sizeof(prefix) / sizeof(prefix[0])) {
		...
		if (preg) {
			...
			status =
			    regcomp(preg, value, REG_NOSUB | prefix[i].flags);
			if (status) {
				char errMsg[1024];
...
static char *regexGetStringProperty(CompWindow * w,
				    Atom propAtom, Atom formatAtom)
{
	Atom type;
...
static char *regexGetWindowTitle(CompWindow * w)
{
	...
	title =
	    regexGetStringProperty(w, rd->visibleNameAtom, d->utf8StringAtom);
	if (title)
		return title;
...
static void regexHandleEvent(CompDisplay * d, XEvent * event)
{
	REGEX_DISPLAY(d);
	...
	if (event->type == PropertyNotify) {
		...
		if (event->xproperty.atom == rd->roleAtom) {
			...
			if (w) {
				...
				rw->role =
				    regexGetStringProperty(w, rd->roleAtom,
							   XA_STRING);
...
static Bool regexRegisterExpHandler(void *closure)
{
...
static Bool regexInitDisplay(CompPlugin * p, CompDisplay * d)
{
	RegexDisplay *rd;
...
static void regexFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	REGEX_DISPLAY(d);
...
static Bool regexInitScreen(CompPlugin * p, CompScreen * s)
{
	RegexScreen *rs;
...
static void regexFiniScreen(CompPlugin * p, CompScreen * s)
{
	REGEX_SCREEN(s);
...
static Bool regexInitWindow(CompPlugin * p, CompWindow * w)
{
	RegexWindow *rw;
...
static void regexFiniWindow(CompPlugin * p, CompWindow * w)
{
	REGEX_WINDOW(w);
...
static CompBool regexInitObject(CompPlugin * p, CompObject * o)
{
...
static void regexFiniObject(CompPlugin * p, CompObject * o)
{
...
static Bool regexInit(CompPlugin * p)
{
...
static void regexFini(CompPlugin * p)
{
	freeDisplayPrivateIndex(displayPrivateIndex);
	compFiniMetadata(&regexMetadata);
}

static CompMetadata *regexGetMetadata(CompPlugin * plugin)
{
