...
fxDodgeProcessSubject(CompWindow * wCur,
		      Region wRegion, Region dodgeRegion, Bool alwaysInclude)
{
	XRectangle rect;
	rect.x = WIN_X(wCur);
	rect.y = WIN_Y(wCur);
	...
	if (!alwaysInclude) {
		...
		if (intersectionRegion) {
			...
			if (!XEmptyRegion(intersectionRegion))
				XUnionRegion(dodgeRegion, wCurRegion,
					     dodgeRegion);
...
static Bool fxDodgeFindDodgeBox(CompWindow * w, XRectangle * dodgeBox)
{
...
static void applyDodgeTransform(CompWindow * w, CompTransform * transform)
{
	ANIM_WINDOW(w);
	...
	float amount =
	    sin(M_PI * aw->com.transformProgress) * aw->dodgeMaxAmount;
...
void fxDodgePostPreparePaintScreen(CompWindow * w)
{
	...
	if (aw->restackInfo->raised && dw != aw->winThisIsPaintedBefore)	// w's host is changing
	{
