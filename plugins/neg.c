		     ...
		     const FragmentAttrib * attrib, unsigned int mask)
{
	...
	if (nw->isNeg && (texture->name == w->texture->name)) {
		if (w->screen->fragmentProgram) {
			...
			function =
			    getNegFragmentFunction(w->screen, texture,
						   w->alpha);
			if (function)
...
static void NEGWindowAdd(CompScreen * s, CompWindow * w)
{
	NEG_SCREEN(s);
...
static void
NEGScreenOptionChanged(CompScreen * s, CompOption * opt, NegScreenOptions num)
{
	switch (num) {
	...
	case NegScreenOptionExcludeMatch:
		{
			...
			for (w = s->windows; w; w = w->next) {
				...
				isNeg = isNeg
				    && !matchEval(negGetExcludeMatch(s), w);
...
static void NEGObjectAdd(CompObject * parent, CompObject * object)
{
...
static Bool NEGInitCore(CompPlugin * p, CompCore * c)
{
	NEGCore *nc;
...
static void NEGFiniCore(CompPlugin * p, CompCore * c)
{
	NEG_CORE(c);
...
static CompBool NEGInitObject(CompPlugin * p, CompObject * o)
{
...
static void NEGFiniObject(CompPlugin * p, CompObject * o)
{
