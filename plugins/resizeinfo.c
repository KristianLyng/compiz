...
static void freeInfoLayer(CompScreen * s, InfoLayer * il)
{
	if (il->cr)
		cairo_destroy(il->cr);
	il->cr = NULL;
...
static void setupCairoLayer(CompScreen * s, InfoLayer * il)
{
	...
	if (!bindPixmapToTexture(s, &il->texture, il->pixmap, w, h, 32)) {
		compWarn("Bind Pixmap to Texture failure");
		freeInfoLayer(s, il);
		return;
	}
	...
	if (cairo_surface_status(il->surface) != CAIRO_STATUS_SUCCESS) {
		compWarn("Could not create cairo layer surface,");
		freeInfoLayer(s, il);
		return;
	}
	...
	if (cairo_status(il->cr) != CAIRO_STATUS_SUCCESS) {
		compWarn("Could not create cairo context");
		freeInfoLayer(s, il);
		return;
	}
}
...
static void updateTextLayer(CompScreen * s)
{
...
static void
gradientChanged(CompDisplay * d, CompOption * o, ResizeinfoDisplayOptions num)
{
	CompScreen *s;
...
static void damagePaintRegion(CompScreen * s)
{
	REGION reg;
	int x, y;
		     ...
		     int x, int y, unsigned int state, unsigned int mask)
{
	...
	if ((!is->pWindow || !is->drawing) && !(w->state & MAXIMIZE_STATE)) {
		...
		if (showInfo && (mask & CompWindowGrabResizeMask)) {
			...
			is->drawing = TRUE;
			is->fadeTime =
			    resizeinfoGetFadeTime(s->display) - is->fadeTime;
...
static void
drawLayer(CompScreen * s, int x, int y, CompMatrix matrix, CompTexture * t)
{
	BOX box;
	float opacity;
...
static void infoHandleEvent(CompDisplay * d, XEvent * event)
{
	INFO_DISPLAY(d);
		...
		if (event->xclient.message_type == id->resizeNotifyAtom) {
			...
			if (w) {
				...
				if (w == is->pWindow) {
					...
					is->resizeGeometry.width =
					    event->xclient.data.l[2];
					is->resizeGeometry.height =
...
static Bool infoInitDisplay(CompPlugin * p, CompDisplay * d)
{
	InfoDisplay *id;
	...
	id->resizeNotifyAtom =
	    XInternAtom(d->display, "_COMPIZ_RESIZE_NOTIFY", 0);
...
static void infoFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	INFO_DISPLAY(d);
...
static Bool infoInitScreen(CompPlugin * p, CompScreen * s)
{
	InfoScreen *is;
...
static void infoFiniScreen(CompPlugin * p, CompScreen * s)
{
	INFO_SCREEN(s);
...
static CompBool infoInitObject(CompPlugin * p, CompObject * o)
{
...
static void infoFiniObject(CompPlugin * p, CompObject * o)
{
...
static Bool infoInit(CompPlugin * p)
{
...
static void infoFini(CompPlugin * p)
{
