	     ...
	     CompActionState state, CompOption * option, int nOption)
{
	...
	if (w && (w->actions & CompWindowActionMoveMask)) {
		...
		sourceExternalApp =
		    getBoolOptionNamed(option, nOption, "external", FALSE);
		...
		getWorkareaForOutput(w->screen,
				     outputDeviceForWindow(w), &workArea);
	...
	if (md->w) {
		...
		if (state & CompActionStateCancel)
			moveWindow(md->w,
				   ...
				   md->savedY - md->w->attrib.y, TRUE, FALSE);
	...
	for (i = 0; i < s->nOutputDev; i++) {
		...
		for (w = s->windows; w; w = w->next) {
			...
			if (w->struts) {
				...
				r.extents.x2 =
				    r.extents.x1 + w->struts->top.width;
				r.extents.y2 =
				    r.extents.y1 + w->struts->top.height;
				...
				r.extents.x2 =
				    r.extents.x1 + w->struts->bottom.width;
				r.extents.y2 =
				    r.extents.y1 + w->struts->bottom.height;
...
static void moveHandleMotionEvent(CompScreen * s, int xRoot, int yRoot)
{
	...
	if (ms->grabIndex) {
			...
			if (md->constrainY) {
				...
				if (md->region) {
					...
					width =
					    ...
					    w->input.right;
					height =
					    w->input.top ? w->input.top : 1;

					status =
					    XRectInRegion(md->region, x, y,
							  width, height);
		...
		if (dx || dy) {
			moveWindow(w,
				   wX + dx - w->attrib.x,
				   wY + dy - w->attrib.y, TRUE, FALSE);

			if (md->opt[MOVE_DISPLAY_OPTION_LAZY_POSITIONING].value.
			    b) {
		...
		if (s) {
			...
			if (ms->grabIndex) {
				    ...
				    md->releaseButton == event->xbutton.button)
				{
					CompAction *action;
					int opt =
					    MOVE_DISPLAY_OPTION_INITIATE_BUTTON;

					action = &md->opt[opt].value.action;
					moveTerminate(d, action,
			...
			if (event->xclient.data.l[2] == WmMoveResizeMove ||
				...
				if (w) {
					...
					if (event->xclient.data.l[2] ==
					    WmMoveResizeMoveKeyboard) {
						option =
						    MOVE_DISPLAY_OPTION_INITIATE_KEY;
...
static CompOption *moveGetDisplayOptions(CompPlugin * plugin,
...
static CompBool moveInitObject(CompPlugin * p, CompObject * o)
{
...
static void moveFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *moveGetObjectOptions(CompPlugin * plugin,
...
static CompMetadata *moveGetMetadata(CompPlugin * plugin)
{
	return &moveMetadata;
}
