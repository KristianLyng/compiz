...
static Bool iniSaveOptions(CompObject * object, const char *plugin);
...
static IniFileData *iniGetFileDataFromFilename(const char *filename)
{
	int len, i;
	...
	strncpy(screenStr, &filename[pluginSep + 2],
		(screenSep - pluginSep) - 1);
...
static char *iniOptionValueToString(CompDisplay * d, CompOptionValue * value,
				    CompOptionType type)
{
...
static Bool iniGetHomeDir(char **homeDir)
{
	char *home = NULL, *tmp;
...
static Bool
iniGetFilename(CompObject * object, const char *plugin, char **filename)
{
	int len;
...
static Bool iniParseLine(char *line, char **optionName, char **optionValue)
{
	char *splitPos;
...
static Bool iniMakeDirectories(void)
{
	char *homeDir;
...
iniLoadOptionsFromFile(FILE * optionFile,
		       CompObject * object, const char *plugin, Bool * reSave)
{
	...
	while (fgets(tmp, MAX_OPTION_LENGTH, optionFile) != NULL) {
		...
		if (option) {
			...
			if (o) {
				...
				case CompOptionTypeColor:
					hasValue =
					    stringToColor(optionValue, value.c);
					break;
				case CompOptionTypeKey:
					hasValue = TRUE;
					stringToKeyAction(GET_CORE_DISPLAY
							  (object), optionValue,
							  &value.action);
					break;
				case CompOptionTypeButton:
					hasValue = TRUE;
					stringToButtonAction(GET_CORE_DISPLAY
							     (object),
							     optionValue,
							     &value.action);
					break;
				case CompOptionTypeEdge:
					hasValue = TRUE;
					value.action.edgeMask =
					    stringToEdgeMask(optionValue);
					break;
				case CompOptionTypeBell:
					hasValue = TRUE;
					value.action.bell =
					    (Bool) atoi(optionValue);
					break;
				case CompOptionTypeList:
					hasValue =
					    csvToList(GET_CORE_DISPLAY(object),
						      optionValue, &value.list,
						      value.list.type);
					break;
				case CompOptionTypeMatch:
					hasValue = TRUE;
					matchInit(&value.match);
					matchAddFromString(&value.match,
							   optionValue);
					break;
				default:
					break;
...
static Bool iniSaveOptions(CompObject * object, const char *plugin)
{
	...
	fullPath =
	    malloc(sizeof(char) * (strlen(filename) + strlen(directory) + 2));
	if (!fullPath) {
		free(filename);
		free(directory);
		return FALSE;
			...
			case CompOptionTypeMatch:
				{
					...
					for (i = 0;
					     i < option->value.list.nValue;
					     i++) {
						itemVal =
						    iniOptionValueToString
						    (GET_CORE_DISPLAY(object),
						     &option->value.list.
						     value[i],
...
static Bool iniLoadOptions(CompObject * object, const char *plugin)
{
	...
	if (!optionFile) {
		if (!plugin && object->type == COMP_OBJECT_TYPE_DISPLAY) {
			CompOptionValue value;
			value.list.value =
			    malloc(NUM_DEFAULT_PLUGINS * sizeof(CompListValue));
			if (!value.list.value) {
				free(filename);
				free(directory);
				free(fullPath);
			...
			if (!csvToList
			    (GET_CORE_DISPLAY(object), DEFAULT_PLUGINS,
...
static void iniFileModified(const char *name, void *closure)
{
	IniFileData *fd;
...
static void iniFreeFileData(void)
{
	IniFileData *fd, *tmp;
...
static Bool iniInitPluginForDisplay(CompPlugin * p, CompDisplay * d)
{
...
static Bool iniInitPluginForScreen(CompPlugin * p, CompScreen * s)
{
...
static CompBool iniInitPluginForObject(CompPlugin * p, CompObject * o)
{
	CompBool status;
...
static Bool iniInitCore(CompPlugin * p, CompCore * c)
{
	IniCore *ic;
	char *homeDir;
...
static void iniFiniCore(CompPlugin * p, CompCore * c)
{
	INI_CORE(c);
...
static Bool iniInitDisplay(CompPlugin * p, CompDisplay * d)
{
...
static Bool iniInitScreen(CompPlugin * p, CompScreen * s)
{
...
static CompBool iniInitObject(CompPlugin * p, CompObject * o)
{
...
static void iniFiniObject(CompPlugin * p, CompObject * o)
{
...
static Bool iniInit(CompPlugin * p)
{
...
static void iniFini(CompPlugin * p)
{
	freeCorePrivateIndex(corePrivateIndex);
}

static CompMetadata *iniGetMetadata(CompPlugin * plugin)
{
	return &iniMetadata;
}
