...
#define N_QUADS_MAX    24
	...
	int
	 decor_property_to_quads(long *data,
				 ...
				 decor_extents_t * max_input,
				 int *min_width,
				 int *min_height, decor_quad_t * quad);
	...
	void
	 decor_region_to_blur_property(long *data,
				       ...
				       Region leftRegion,
				       int leftOffset,
				       Region rightRegion, int rightOffset);
	...
	void
	 decor_apply_gravity(int gravity,
			     ...
			     int y,
			     int width,
			     int height, int *return_x, int *return_y);
	...
	int
	 decor_set_vert_quad_row(decor_quad_t * q,
				 ...
				 int splitY,
				 int splitGravity,
				 double x0, double y0, int rotation);
	...
	int
	 decor_set_horz_quad_line(decor_quad_t * q,
				  ...
				  int width,
				  int splitX,
				  int splitGravity, double x0, double y0);
	...
	void
	 decor_shadow_destroy(Display * xdisplay, decor_shadow_t * shadow);

	void
	 decor_shadow_reference(decor_shadow_t * shadow);

	void
	 decor_shadow(Display * xdisplay, decor_shadow_t * shadow);
	...
	void
	 decor_draw_simple(Display * xdisplay,
			   ...
			   Picture picture,
			   int width,
			   int height, decor_context_t * c, void *closure);

	void
	 decor_get_default_layout(decor_context_t * c,
				  int width,
				  int height, decor_layout_t * layout);
	...
	void
	 decor_blend_transform_picture(Display * xdisplay,
				       ...
				       decor_layout_t * layout,
				       Region region,
				       unsigned short alpha, int shade_alpha);
...
#define DECOR_ACQUIRE_STATUS_OTHER_DM_RUNNING 2
	...
	void
	 decor_set_dm_check_hint(Display * xdisplay, int screen);
...
#define DECOR_SELECTION_GIVE_UP 1
	...
	int
	 decor_handle_selection_clear(Display * xdisplay,
				      XEvent * xevent, int screen);

	void
	 decor_handle_selection_request(Display * xdisplay,
					XEvent * event, Time dm_sn_timestamp);
