...
static void expoFinishWindowMovement(CompWindow * w)
{
			...
			CompOutput * output, int vertex[2])
{
	...
	if (expoGetDeform(s->display) == DeformCurve && s->desktopWindowCount) {
		...
		const float p =
		    ((2.0 * sws * (p1[2] - es->curveDistance) * v[2]) +
		     (2.0 * p1[0] * v[0]) -
		     (v[0] * (float)s->width)) / ((v[2] * v[2] * sws) +
						  (v[0] * v[0]));
		const float q =
		    (-(sws * rs) +
		     (sws * (p1[2] - es->curveDistance) *
		      (p1[2] - es->curveDistance)) + (0.25 * sws) +
		     (p1[0] * p1[0]) - (p1[0] * (float)s->width))
		    / ((v[2] * v[2] * sws) + (v[0] * v[0]));
		...
		for (i = 0; i < s->hsize; i++)
			for (j = 0; j < s->vsize; j++) {
				...
				if (i == es->selectedVX && j == es->selectedVY)
					es->vpActivity[vp] =
					    MIN(1.0, es->vpActivity[vp] + val);
				else
					es->vpActivity[vp] =
					    MAX(0.0, es->vpActivity[vp] - val);
			}

		for (i = 0; i < 360; i++) {
			es->vpNormals[i * 3] =
			    (-sin((float)i * DEG2RAD) / s->width)
			    * es->expoCam;
...
expoPaintWall(CompScreen * s,
	      ...
	      const CompTransform * transform,
	      Region region,
	      CompOutput * output, unsigned int mask, Bool reflection)
{
	EXPO_SCREEN(s);
	...
	for (j = 0; j < s->vsize; j++) {
		...
		for (i = 0; i < s->hsize; i++) {
			if (expoGetExpoAnimation(s->display) ==
			    ExpoAnimationVortex)
				matrixRotate(&sTransform2, 360 * es->expoCam,
					     0.0f, 1.0f, 2.0f * es->expoCam);
			...
			if (vp < es->vpActivitySize) {
				...
				es->vpSaturation =
				    vpp +
				    ((1.0 -
				      vpp) * expoGetVpSaturation(s->display) /
				     100.0);
			} else {
				es->vpBrightness = 1.0;
				es->vpSaturation = 1.0;
			...
			if (expoGetDeform(s->display) == DeformCurve) {
				...
				matrixRotate(&sTransform3,
					     es->curveAngle * (-i +
							       interpolate((((float)s->hsize / 2.0) - 0.5), s->x, progress)), 0.0f, 1.0f, 0.0);
			...
			paintTransformedOutput(s, sAttrib, &sTransform3,
					       &s->region, output, mask);

			if (!reflection) {
				int cursor[2] = { pointerX, pointerY };

				invertTransformedVertex(s, sAttrib,
							&sTransform3, output,
							cursor);
			...
			if (expoGetDeform(s->display) != DeformCurve)
				matrixTranslate(&sTransform2,
						((1.0 * sx) + gapX), 0.0f, 0.0);
		}
			   ...
			   CompOutput * output, unsigned int mask)
{
	...
	if (es->expoCam <= 0 || (es->expoCam < 1.0 && es->expoCam > 0.0 &&
				 expoGetExpoAnimation(s->display) !=
				 ExpoAnimationZoom)) {
	...
	if (es->expoCam > 0.0) {
		if (expoGetReflection(s->display))
			expoPaintWall(s, sAttrib, transform, region, output,
				      mask, TRUE);

		expoPaintWall(s, sAttrib, transform, region, output, mask,
			      FALSE);
		es->anyClick = FALSE;
	}
	       ...
	       Region region, unsigned int mask)
{
	...
	if (es->expoCam > 0.0) {
		...
		UNWRAP(es, s, drawWindow);
		status =
		    (*s->drawWindow) (w, transform, fragment, region, mask);
		WRAP(es, s, drawWindow, expoDrawWindow);
	}
		      ...
		      int nMatrix, Region region, Region clip)
{
	...
	if (es->expoCam > 0.0 && expoGetDeform(s->display) == DeformCurve &&
		...
		const float radSquare =
		    (es->curveDistance * es->curveDistance) + 0.25;
		float ang;
		...
		if (region->numRects > 1) {
			...
			while (x1 < region->extents.x2) {
				...
				(*w->screen->addWindowGeometry) (w, matrix,
								 nMatrix, &reg,
								 clip);
		...
		for (i = oldVCount; i < w->vCount; i++) {
			if (v[0] == lastX) {
				...
				if (ang < radSquare) {
					v[2] =
					    es->curveDistance - sqrt(radSquare -
								     ang);
		      ...
		      const FragmentAttrib * attrib, unsigned int mask)
{
	...
	if (es->expoCam > 0.0 && expoGetDeform(s->display) == DeformCurve &&
		...
		for (i = 0; i < w->vCount; i++) {
			x = ((float)(v[0] + offX - (s->width / 2)) *
			     es->curveAngle) / (float)s->width;
			while (x < 0)
				x += 360.0;
			...
			es->winNormals[(i * 3) + 1] =
			    es->vpNormals[(idx * 3) + 1];
			es->winNormals[(i * 3) + 2] =
			    es->vpNormals[(idx * 3) + 2];
...
static void expoDonePaintScreen(CompScreen * s)
{
	...
	case VPUpdatePrevious:
		moveScreenViewport(s, s->x - es->origVX, s->y - es->origVY,
				   TRUE);
...
static CompBool expoInitObject(CompPlugin * p, CompObject * o)
{
...
static void expoFiniObject(CompPlugin * p, CompObject * o)
{
