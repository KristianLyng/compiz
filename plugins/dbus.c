...
static void dbusUpdatePluginList(CompDisplay * d)
{
	CompListValue *pl;
	unsigned int i;
...
static CompOption *dbusGetOptionsFromPath(char **path,
...
static void dbusIntrospectStartInterface(xmlTextWriterPtr writer)
{
...
static void dbusIntrospectEndInterface(xmlTextWriterPtr writer)
{
...
static void dbusIntrospectAddNode(xmlTextWriterPtr writer, char *name)
{
...
static void dbusIntrospectStartRoot(xmlTextWriterPtr writer)
{
	xmlTextWriterStartElement(writer, BAD_CAST "node");
...
static void dbusIntrospectEndRoot(xmlTextWriterPtr writer)
{
...
dbusHandleActionMessage(DBusConnection * connection,
			DBusMessage * message, char **path, Bool activate)
{
	CompObject *object;
	CompOption *option;
	int nOption;
	...
	while (nOption--) {
		if (strcmp(option->name, path[2]) == 0) {
			...
			if (activate) {
				(*option->value.action.
				 initiate) (GET_CORE_DISPLAY(object),
					    &option->value.action, 0, argument,
					    nArgument);
			} else {
				(*option->value.action.
				 terminate) (GET_CORE_DISPLAY(object),
					     &option->value.action, 0, argument,
					     nArgument);
			}
		   ...
		   CompOptionType type, CompOptionValue * value)
{
	...
	case CompOptionTypeKey:
		if (dbusTryGetValueWithType(iter, DBUS_TYPE_STRING, &s)) {
			...
			stringToKeyAction(GET_CORE_DISPLAY(object), s,
					  &value->action);
			return TRUE;
		}
		break;
	...
	while (nOption--) {
		if (strcmp(option->name, path[2]) == 0) {
			...
			if (option->type == CompOptionTypeList) {
				...
				status =
				    dbusGetOptionValue(object, &iter,
						       option->type, &value);
			}
			...
			if (status) {
				...
				if (!dbus_message_get_no_reply(message)) {
					...
					dbus_connection_send(connection, reply,
							     NULL);
					dbus_connection_flush(connection);
			  ...
			  CompOptionType type, CompOptionValue * value)
{
	...
	for (i = 0; i < value->list.nValue; i++) {
		...
		case CompOptionTypeButton:
			...
			break;
		case CompOptionTypeEdge:
			s = edgeMaskToString(value->list.value[i].action.
					     edgeMask);
			if (s) {
				dbus_message_iter_append_basic(&listIter,
							       sig[0], &s);
		...
		if (strcmp(option->name, path[2]) == 0) {
			...
			if (m) {
				if (object->type == COMP_OBJECT_TYPE_SCREEN) {
					shortDesc =
					    compGetShortScreenOptionDescription
					    (m, option);
					longDesc =
					...
					shortDesc =
					    ...
					    (m, option);
					longDesc =
					    compGetLongDisplayOptionDescription
					    (m, option);
				}
			}
 ...
 */
static Bool
dbusHandleGetPluginsMessage(DBusConnection * connection, DBusMessage * message)
{
	DBusMessage *reply;
	char **plugins, **p;
	int n;
				   ...
				   DBusMessage * message)
{
	...
	if (!dbusTryGetValueWithType(&iter, DBUS_TYPE_STRING, &name))
		return FALSE;
	...
	if (p) {
		...
		reply = dbus_message_new_error(message, DBUS_ERROR_FAILED, str);
		  ...
		  DBusMessage * message, void *userData)
{
	...
	if (!path[3]) {
		if (dbus_message_is_method_call(message,
						...
						"Introspect")) {
			if (dbusHandleRootIntrospectMessage
			    (connection, message)) {
				dbus_free_string_array(path);
			...
			(message, COMPIZ_DBUS_INTERFACE,
			 COMPIZ_DBUS_GET_PLUGIN_METADATA_MEMBER_NAME)) {
			if (dbusHandleGetPluginMetadataMessage
			    (connection, message)) {
				dbus_free_string_array(path);
				return DBUS_HANDLER_RESULT_HANDLED;
			}
	...
	/* plugin message */
		...
		if (dbus_message_is_method_call(message,
						DBUS_INTERFACE_INTROSPECTABLE,
						"Introspect")) {
			if (dbusHandlePluginIntrospectMessage
			    (connection, message, &path[3])) {
				dbus_free_string_array(path);
				return DBUS_HANDLER_RESULT_HANDLED;
			}
		}
	...
	/* screen message */
		...
		if (dbus_message_is_method_call(message,
						DBUS_INTERFACE_INTROSPECTABLE,
						"Introspect")) {
			if (dbusHandleScreenIntrospectMessage
			    (connection, message, &path[3])) {
				dbus_free_string_array(path);
				return DBUS_HANDLER_RESULT_HANDLED;
			}
	...
	{
		status =
		    dbusHandleActionMessage(connection, message, &path[3],
					    TRUE);
	} else
	    if (dbus_message_is_method_call
		(message, COMPIZ_DBUS_INTERFACE,
	    ...
	    if (dbus_message_is_method_call
		(message, COMPIZ_DBUS_INTERFACE, COMPIZ_DBUS_SET_MEMBER_NAME)) {
		status =
		    dbusHandleSetOptionMessage(connection, message, &path[3]);
	} else
	    if (dbus_message_is_method_call
		(message, COMPIZ_DBUS_INTERFACE, COMPIZ_DBUS_GET_MEMBER_NAME)) {
		status =
		    dbusHandleGetOptionMessage(connection, message, &path[3]);
	} else
	    if (dbus_message_is_method_call
		(message, COMPIZ_DBUS_INTERFACE,
		 COMPIZ_DBUS_GET_METADATA_MEMBER_NAME)) {
		status =
		    dbusHandleGetMetadataMessage(connection, message, &path[3]);
	}
...
dbusSendChangeSignalForOption(CompObject * object,
			      CompOption * o, const char *plugin)
{
	DBusMessage *signal;
	char *name, path[256];
	...
	if (name) {
		sprintf(path, "%s/%s/%s%s/%s", COMPIZ_DBUS_ROOT_PATH,
			plugin, compObjectTypeName(object->type), name,
			o->name);
...
static Bool dbusGetPathDecomposed(char *data, char ***path, int *count)
{
	char **retval;
	char *temp;
	char *token;
...
static void dbusFreePathDecomposed(char **path, int count)
{
	int i;
...
static Bool dbusRegisterOptions(DBusConnection * connection, char *screenPath)
{
	CompOption *option = NULL;
	int nOptions;
	char objectPath[256];
...
static Bool dbusUnregisterOptions(DBusConnection * connection, char *screenPath)
{
	CompOption *option = NULL;
	int nOptions;
	char objectPath[256];
...
static void
dbusRegisterPluginsForDisplay(DBusConnection * connection, CompDisplay * d)
{
	unsigned int i;
	char path[256];
...
static void
dbusRegisterPluginsForScreen(DBusConnection * connection, CompScreen * s)
{
	unsigned int i;
	char path[256];
...
static void
dbusUnregisterPluginsForDisplay(DBusConnection * connection, CompDisplay * d)
{
	unsigned int i;
	...
	for (i = 0; i < dd->nPlugins; i++)
		dbusUnregisterPluginForDisplay(connection, d,
					       dd->pluginList[i]);
}
...
static void
dbusUnregisterPluginsForScreen(DBusConnection * connection, CompScreen * s)
{
	unsigned int i;
...
static CompBool dbusInitPluginForDisplay(CompPlugin * p, CompDisplay * d)
{
	char objectPath[256];
...
static Bool dbusInitPluginForScreen(CompPlugin * p, CompScreen * s)
{
	char objectPath[256];
...
static CompBool dbusInitPluginForObject(CompPlugin * p, CompObject * o)
{
	CompBool status;
		       ...
		       const char *name, CompOptionValue * value)
{
	...
	if (status) {
		...
		if (p && p->vTable->getObjectOptions) {
			...
			option =
			    (*p->vTable->getObjectOptions) (p, object,
							    &nOption);
			...
			if (object->type == COMP_OBJECT_TYPE_DISPLAY &&
				...
				dbusUnregisterPluginsForDisplay(dc->connection,
								d);
				for (s = d->screens; s; s = s->next)
					dbusUnregisterPluginsForScreen(dc->
								       connection,
								       s);

				dbusUpdatePluginList(d);

				dbusRegisterPluginsForDisplay(dc->connection,
							      d);
				for (s = d->screens; s; s = s->next)
					dbusRegisterPluginsForScreen(dc->
								     connection,
								     s);
			}
		}
...
static void dbusSendPluginsChangedSignal(const char *name, void *closure)
{
	DBusMessage *signal;
...
static Bool dbusInitCore(CompPlugin * p, CompCore * c)
{
	DbusCore *dc;
	DBusError error;
	dbus_bool_t status;
		...
		if (plugindir) {
			...
			dc->fileWatch[DBUS_FILE_WATCH_HOME] =
			    addFileWatch(plugindir,
					 mask, dbusSendPluginsChangedSignal, 0);
...
static void dbusFiniCore(CompPlugin * p, CompCore * c)
{
	int i;
...
static CompBool dbusInitObject(CompPlugin * p, CompObject * o)
{
...
static void dbusFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompMetadata *dbusGetMetadata(CompPlugin * plugin)
{
	return &dbusMetadata;
}
