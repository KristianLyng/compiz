...
static float fxZoomGetSpringiness(CompWindow * w)
{
	ANIM_WINDOW(w);
			...
			float *scaleProgress, Bool neverSpringy)
{
	...
	float dampBase =
	    (pow(1 - pow(x, 1.2) * 0.5, 10) - pow(0.5, 10)) / (1 -
							       pow(0.5, 10));
	...
	if (moveProgress && scaleProgress) {
		...
		float damping2 =
		    ((pow(1 - (pow(x, 0.7) * 0.5), 10) - pow(0.5, 10)) / (1 -
									  pow
									  (0.5,
									   10)))
		    * 0.7 + 0.3;
		...
		if (springiness > 1e-4f) {
			if (x > 0.2) {
				...
				springyMoveProgress =
				    ...
				    progressUpto02 * springyMoveProgress *
				    springiness;
			}
		       ...
		       float *pRotateProgress)
{
	...
	Point winCenter = { (WIN_X(w) + WIN_W(w) / 2.0),
		(WIN_Y(w) + WIN_H(w) / 2.0)
	};
	Point iconCenter = { aw->com.icon.x + aw->com.icon.width / 2.0,
		aw->com.icon.y + aw->com.icon.height / 2.0
	};
	Point winSize = { WIN_W(w), WIN_H(w) };
	...
	Point curScale =
	    { ((1 - scaleProgress) * winSize.x +
	       scaleProgress * aw->com.icon.width) / winSize.x,
		((1 - scaleProgress) * winSize.y +
		 scaleProgress * aw->com.icon.height) / winSize.y
	};
...
inline void
getZoomCenterScale(CompWindow * w, Point * pCurCenter, Point * pCurScale)
{
...
void applyZoomTransform(CompWindow * w)
{
	...
	if (fxZoomGetSpringiness(w) == 0.0f &&
	    ...
	    (aw->com.curWindowEvent == WindowEventOpen ||
	     aw->com.curWindowEvent == WindowEventClose)) {
		matrixTranslate(transform, iconCenter.x, iconCenter.y, 0);
		...
		if (aw->com.curAnimEffect == AnimEffectSidekick) {
			...
			matrixRotate(transform,
				     rotateProgress * 360 *
				     aw->numZoomRotations, 0.0f, 0.0f, 1.0f);
			matrixTranslate(transform, -winCenter.x, -winCenter.y,
					0);
		}
	} else {
		matrixTranslate(transform, winCenter.x, winCenter.y, 0);
		...
		if (aw->com.curAnimEffect != AnimEffectZoom) {
			...
			matrixScale(transform, curScale.x, curScale.y,
				    curScale.y);
