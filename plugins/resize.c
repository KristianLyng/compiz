...
static void resizeGetPaintRectangle(CompDisplay * d, BoxPtr pBox)
{
	RESIZE_DISPLAY(d);
...
static void
resizeGetStretchScale(CompWindow * w, BoxPtr pBox, float *xScale, float *yScale)
{
	int width, height;
...
static void resizeGetStretchRectangle(CompDisplay * d, BoxPtr pBox)
{
	BoxRec box;
	float xScale, yScale;
...
static void resizeDamageRectangle(CompScreen * s, BoxPtr pBox)
{
	REGION reg;
...
static Cursor resizeCursorFromResizeMask(CompScreen * s, unsigned int mask)
{
	Cursor cursor;
...
static void resizeSendResizeNotify(CompDisplay * d)
{
	XEvent xev;
...
static void resizeUpdateWindowProperty(CompDisplay * d)
{
...
static void resizeFinishResizing(CompDisplay * d)
{
	RESIZE_DISPLAY(d);
	BoxRec box;
	...
	XDeleteProperty(d->display, rd->w->id, rd->resizeInformationAtom);
...
static Region resizeGetConstraintRegion(CompScreen * s)
{
	Region region;
	int i;
	...
	if (w && (w->actions & CompWindowActionResizeMask)) {
		...
		if ((w->state & MAXIMIZE_STATE) == MAXIMIZE_STATE) {
			...
			if (i > RESIZE_MODE_LAST) {
				...
				for (i = 0; i <= RESIZE_MODE_LAST; i++) {
					index =
					    RESIZE_DISPLAY_OPTION_NORMAL_MATCH +
					    i;
					if (matchEval
					    (&rd->opt[index].value.match, w)) {
						rd->mode = i;
						break;
					}
				}
		...
		if (!rs->grabIndex) {
			...
			if (state & CompActionStateInitKey) {
				...
				cursor =
				    resizeCursorFromResizeMask(w->screen, mask);
			}
		...
		if (rs->grabIndex) {
			...
			if (state & CompActionStateInitKey) {
				...
				warpPointer(w->screen, xRoot - pointerX,
					    yRoot - pointerY);
			}
			...
			if (sourceExternalApp) {
				...
				rd->constraintRegion =
				    resizeGetConstraintRegion(w->screen);
			} else {
...
static void resizeUpdateWindowSize(CompDisplay * d)
{
	RESIZE_DISPLAY(d);
	...
	if (rs->grabIndex && rd->w) {
		...
		for (i = 0; i < NUM_KEYS; i++) {
			...
			if (rd->mask & rKeys[i].warpMask) {
				XWarpPointer(s->display->display, None, None, 0,
					     0, 0, 0, rKeys[i].dx * widthInc,
...
static void resizeHandleMotionEvent(CompScreen * s, int xRoot, int yRoot)
{
	...
	if (rs->grabIndex) {
		...
		if (!rd->mask) {
			...
			yDist =
			    yRoot - (win->serverY + (win->serverHeight / 2));
		...
		if (rd->constraintRegion) {
			...
			if (rd->inRegionStatus == RectangleIn) {
				...
				if (rd->
				    mask & (ResizeLeftMask | ResizeRightMask)) {
					xStatus = status;
...
static void resizeHandleEvent(CompDisplay * d, XEvent * event)
{
	...
	case ButtonRelease:
		...
		if (s) {
			...
			if (rs->grabIndex) {
				...
				{
					int opt =
					    RESIZE_DISPLAY_OPTION_INITIATE_BUTTON;
					CompAction *action =
					    &rd->opt[opt].value.action;
			...
			if (event->xclient.data.l[2] <= WmMoveResizeSizeLeft ||
				...
				if (w) {
					...
					if (event->xclient.data.l[2] ==
					    WmMoveResizeSizeKeyboard) {
						option =
						    RESIZE_DISPLAY_OPTION_INITIATE_KEY;
	...
	case DestroyNotify:
		if (rd->w && rd->w->id == event->xdestroywindow.window) {
			...
			resizeTerminate(d, &rd->opt[option].value.action, 0,
					NULL, 0);
...
resizePaintRectangle(CompScreen * s,
		     ...
		     CompOutput * output,
		     unsigned short *borderColor, unsigned short *fillColor)
{
	BoxRec box;
		  ...
		  Region region, CompOutput * output, unsigned int mask)
{
	...
	if (status && rd->w && (s == rd->w->screen)) {
		...
		case RESIZE_MODE_OUTLINE:
			resizePaintRectangle(s, sAttrib, transform, output,
					     border, NULL);
			break;
		  ...
		  Region region, unsigned int mask)
{
	...
	if (w == rd->w && rd->mode == RESIZE_MODE_STRETCH) {
		...
		matrixTranslate(&wTransform,
				(rd->geometry.x - w->attrib.x) / xScale -
				xOrigin,
				(rd->geometry.y - w->attrib.y) / yScale -
				yOrigin, 0.0f);
...
static Bool resizeDamageWindowRect(CompWindow * w, Bool initial, BoxPtr rect)
{
	Bool status = FALSE;
...
static CompOption *resizeGetDisplayOptions(CompPlugin * plugin,
...
static CompBool resizeInitObject(CompPlugin * p, CompObject * o)
{
...
static void resizeFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *resizeGetObjectOptions(CompPlugin * plugin,
...
static CompMetadata *resizeGetMetadata(CompPlugin * plugin)
{
