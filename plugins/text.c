...
static char *textGetTextProperty(CompDisplay * d, Window id, Atom atom)
{
	...
	if (XGetTextProperty(d->display, id, &text, atom)) {
		if (text.value) {
			...
			if (retval) {
				strncpy(retval, (char *)text.value,
					text.nitems);
...
static Bool
textInitCairo(CompScreen * s, TextSurfaceData * data, int width, int height)
{
...
static Bool textInitSurface(CompScreen * s, TextSurfaceData * data)
{
	Display *dpy = s->display->display;
...
static Bool
textUpdateSurface(CompScreen * s, TextSurfaceData * data, int width, int height)
{
	Display *dpy = s->display->display;
...
textRenderTextToSurface(CompScreen * s,
			const char *text,
			TextSurfaceData * data, const CompTextAttrib * attrib)
{
	...
	if (attrib->flags & CompTextFlagStyleBold)
		pango_font_description_set_weight(data->font,
						  PANGO_WEIGHT_BOLD);

	if (attrib->flags & CompTextFlagStyleItalic)
		pango_font_description_set_style(data->font,
						 PANGO_STYLE_ITALIC);
	...
	if (attrib->flags & CompTextFlagWithBackground) {
		textDrawTextBackground(data->cr, 0, 0, width, height,
				       MIN(attrib->bgHMargin,
					   attrib->bgVMargin));
		cairo_set_source_rgba(data->cr, attrib->bgColor[0] / 65535.0,
...
static void textCleanupSurface(TextSurfaceData * data)
{
	if (data->layout)
...
static CompTextData *textRenderText(CompScreen * s,
				    const char *text,
...
static CompTextData *textRenderWindowTitle(CompScreen * s,
					   Window window,
	...
	if (withViewportNumber) {
		...
		if (title) {
			...
			if (w) {
				...
				ret =
				    asprintf(&text, "%s -[%d]-", title,
					     viewport);
				if (ret <= 0) {
					fprintf(stderr,
						"asprintf failed. This is ominous.\n");
					return NULL;
				}
...
textDrawText(CompScreen * s,
	     const CompTextData * data, float x, float y, float alpha)
{
	GLboolean wasBlend;
...
static void textFiniTextData(CompScreen * s, CompTextData * data)
{
	if (data->texture) {
...
static CompOption *textGetDisplayOptions(CompPlugin * plugin,
...
static CompBool textInitObject(CompPlugin * p, CompObject * o)
{
...
static void textFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *textGetObjectOptions(CompPlugin * plugin,
...
static Bool textInit(CompPlugin * p)
{
	if (!compInitPluginMetadataFromInfo(&textMetadata,
					    p->vTable->name,
					    textDisplayOptionInfo,
					    TEXT_DISPLAY_OPTION_NUM, NULL, 0))
		return FALSE;
...
static CompMetadata *textGetMetadata(CompPlugin * p)
{
	return &textMetadata;
}
