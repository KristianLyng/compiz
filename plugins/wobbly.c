...
static CompOption *wobblyGetScreenOptions(CompPlugin * plugin,
	...
	if (x <= workAreaEdge) {
		...
		for (p = w->screen->windows; p; p = p->next) {
			...
			if (p->mapNum && p->struts) {
				...
				e = p->struts->right.y +
				    p->struts->right.height + w->output.bottom;
			} else if (!p->invisible
				   && (p->type & SNAP_WINDOW_TYPE)) {
				s = p->attrib.y - p->input.top - w->output.top;
				e = p->attrib.y + p->height + p->input.bottom +
		...
		for (p = w->screen->windows; p; p = p->next) {
			...
			if (p->mapNum && p->struts) {
				...
				e = p->struts->top.x + p->struts->top.width +
				   ...
				   && (p->type & SNAP_WINDOW_TYPE)) {
				s = p->attrib.x - p->input.left -
				    w->output.left;
				e = p->attrib.x + p->width + p->input.right +
				    w->output.right;
			} else {
				continue;
			}
	...
	if (y <= workAreaEdge) {
		...
		for (p = w->screen->windows; p; p = p->next) {
			...
			if (p->mapNum && p->struts) {
				...
				e = p->struts->bottom.x +
				    p->struts->bottom.width + w->output.right;
			} else if (!p->invisible
				   && (p->type & SNAP_WINDOW_TYPE)) {
				s = p->attrib.x - p->input.left -
				    w->output.left;
...
static void modelSetTopAnchor(Model * model, int x, int y, int width)
{
	float gx;
			...
			if (mask != model->objects[i].edgeMask) {
				...
				if (mask & WestEdgeMask) {
					if (!model->objects[i].vertEdge.snapped)
						findNextWestEdge(window,
								 &model->
								 objects[i]);
				} else if (mask & EastEdgeMask) {
					if (!model->objects[i].vertEdge.snapped)
						findNextEastEdge(window,
								 &model->
								 objects[i]);
				} else
					model->objects[i].vertEdge.snapped =
					    FALSE;

				if (mask & NorthEdgeMask) {
					if (!model->objects[i].horzEdge.snapped)
						findNextNorthEdge(window,
								  &model->
								  objects[i]);
				} else if (mask & SouthEdgeMask) {
					if (!model->objects[i].horzEdge.snapped)
						findNextSouthEdge(window,
								  &model->
								  objects[i]);
				} else
...
static void modelReduceEdgeEscapeVelocity(Model * model)
{
	...
	for (gridY = 0; gridY < GRID_HEIGHT; gridY++) {
		for (gridX = 0; gridX < GRID_WIDTH; gridX++) {
			if (model->objects[i].vertEdge.snapped)
				model->objects[i].vertEdge.velocity *=
				    drand48() * 0.25f;

			if (model->objects[i].horzEdge.snapped)
				model->objects[i].horzEdge.velocity *=
				    drand48() * 0.25f;
...
static void
modelAdjustObjectsForShiver(Model * model, int x, int y, int width, int height)
{
	int gridX, gridY, i = 0;
	float vX, vY;
	float w, h;
...
static void modelInitSprings(Model * model, int x, int y, int width, int height)
{
	int gridX, gridY, i = 0;
	float hpad, vpad;
...
static Model *createModel(int x,
			  int y, int width, int height, unsigned int edgeMask)
{
	Model *model;
		...
		if (object->edgeMask) {
			if (object->edgeMask & WestEdgeMask) {
				...
				if (!object->vertEdge.snapped ||
				    objectReleaseWestEdge(window, model,
							  object)) {
					object->position.x +=
					    object->velocity.x;
				...
				if (!object->vertEdge.snapped ||
				    objectReleaseEastEdge(window, model,
							  object)) {
					object->position.x +=
					    object->velocity.x;
			...
			if (object->edgeMask & NorthEdgeMask) {
				...
				if (!object->horzEdge.snapped ||
				    objectReleaseNorthEdge(window, model,
							   object)) {
					object->position.y +=
					    object->velocity.y;
				...
				if (!object->horzEdge.snapped ||
				    objectReleaseSouthEdge(window, model,
							   object)) {
					object->position.y +=
					    object->velocity.y;
...
static void wobblyPreparePaintScreen(CompScreen * s, int msSinceLastPaint)
{
	...
	if (ws->wobblyWindows & (WobblyInitial | WobblyVelocity)) {
		...
		for (w = s->windows; w; w = w->next) {
			...
			if (ww->wobbly) {
				if (ww->
				    wobbly & (WobblyInitial | WobblyVelocity)) {
					model = ww->model;
			...
			int nMatrix, Region region, Region clip)
{
	...
	if (ww->wobbly) {
		...
		if (gridW < ws->opt[WOBBLY_SCREEN_OPTION_MIN_GRID_SIZE].value.i)
			gridW =
			    ws->opt[WOBBLY_SCREEN_OPTION_MIN_GRID_SIZE].value.i;
		...
		while (nClip--) {
			...
			if (((nVertices + iw * ih) * vSize) > w->vertexSize) {
				if (!moreWindowVertices
				    (w, (nVertices + iw * ih) * vSize))
					return;
...
static void wobblyHandleEvent(CompDisplay * d, XEvent * event)
{
	...
	default:
		if (event->type == d->xkbEvent) {
			...
			if (xkbEvent->xkb_type == XkbStateNotify) {
				...
				action =
				    &wd->opt[WOBBLY_DISPLAY_OPTION_SNAP_KEY].
				    value.action;
				inverted =
				    wd->
				    opt[WOBBLY_DISPLAY_OPTION_SNAP_INVERTED].
				    value.b;
				...
				if ((stateEvent->mods & mods) == mods) {
					if (inverted)
						wobblyDisableSnapping(d, NULL,
								      0, NULL,
								      0);
					else
						wobblyEnableSnapping(d, NULL, 0,
								     NULL, 0);
				} else {
					if (inverted)
						wobblyEnableSnapping(d, NULL, 0,
	...
	case MotionNotify:
		...
		if (s) {
			...
			if (ws->grabWindow &&
			    ws->moveWindow &&
			    ws->opt[WOBBLY_SCREEN_OPTION_MAXIMIZE_EFFECT].value.
			    b) {
	...
	if (d->activeWindow != activeWindow) {
		...
		if (w && isWobblyWin(w)) {
			...
			focusEffect =
			    ws->opt[WOBBLY_SCREEN_OPTION_FOCUS_EFFECT].value.i;
...
static Bool wobblyDamageWindowRect(CompWindow * w, Bool initial, BoxPtr rect)
{
	...
	if (initial) {
		if (isWobblyWin(w)) {
			...
			mapEffect =
			    ws->opt[WOBBLY_SCREEN_OPTION_MAP_EFFECT].value.i;

			if (ws->opt[WOBBLY_SCREEN_OPTION_MAXIMIZE_EFFECT].value.
			    b)
				wobblyEnsureModel(w);
			 ...
			 int dx, int dy, int dwidth, int dheight)
{
	...
	if (ws->opt[WOBBLY_SCREEN_OPTION_MAXIMIZE_EFFECT].value.b &&
	    ...
	    (dheight != -1) && ((w->state | ww->state) & MAXIMIZE_STATE)) {
		...
		if (wobblyEnsureModel(w)) {
			if (w->state & MAXIMIZE_STATE) {
				if (!ww->grabbed && ww->model->anchorObject) {
					ww->model->anchorObject->immobile =
					    FALSE;
		...
		if (ww->wobbly) {
			if (!(ww->state & MAXIMIZE_STATE))
				modelSetTopAnchor(ww->model, WIN_X(w), WIN_Y(w),
						  WIN_W(w));
		} else {
			modelInitObjects(ww->model,
					 WIN_X(w), WIN_Y(w), WIN_W(w),
		       ...
		       int x, int y, unsigned int state, unsigned int mask)
{
	...
	if ((mask & CompWindowGrabButtonMask) &&
	    matchEval(&ws->opt[mIndex].value.match, w) && isWobblyWin(w)) {
		WOBBLY_WINDOW(w);
			...
			if (mask & CompWindowGrabExternalAppMask) {
				...
				if (pMove && pMove->vTable->getObjectOptions) {
					...
					moveOptions =
					    ...
					    (pMove, &core.displays->base,
					     &nOption);

					wd->yConstrained =
					    getBoolOptionNamed(moveOptions,
							       nOption,
							       "constrain_y",
							       TRUE);
				}
...
static CompOption *wobblyGetDisplayOptions(CompPlugin * plugin,
	...
	case WOBBLY_DISPLAY_OPTION_SNAP_INVERTED:
		if (compSetBoolOption(o, value)) {
			if (value->b)
				...
				wobblyDisableSnapping(display, NULL, 0, NULL,
						      0);
			return TRUE;
		}
		break;
...
static CompBool wobblyInitObject(CompPlugin * p, CompObject * o)
{
...
static void wobblyFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *wobblyGetObjectOptions(CompPlugin * plugin,
...
static CompMetadata *wobblyGetMetadata(CompPlugin * plugin)
{
