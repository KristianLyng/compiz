...
static void windowDebug(CompWindow * w)
{
...
static int reallocWindowPrivates(int size, void *closure)
{
	...
	for (w = s->windows; w; w = w->next) {
		privates =
		    realloc(w->base.privates, size * sizeof(CompPrivate));
		if (!privates)
			return FALSE;
...
CompBool
forEachWindowObject(CompObject * parent, ObjectCallBackProc proc, void *closure)
{
...
static void recalcNormalHints(CompWindow * window)
{
	...
	if (!(window->sizeHints.flags & PBaseSize)) {
		if (window->sizeHints.flags & PMinSize) {
			window->sizeHints.base_width =
			    window->sizeHints.min_width;
			window->sizeHints.base_height =
...
void recalcWindowActions(CompWindow * w)
{
	...
	(*w->screen->getAllowedActionsForWindow) (w, &setActions,
						  &clearActions);
...
getAllowedActionsForWindow(CompWindow * w,
			   unsigned int *setActions, unsigned int *clearActions)
{
	*setActions = 0;
	*clearActions = 0;
}
...
static void updateFrameWindow(CompWindow * w)
{
	CompDisplay *d = w->screen->display;
	...
	if (w->input.left || w->input.right || w->input.top || w->input.bottom) {
		...
		if (!w->frame) {
			...
			XGrabButton(d->display, AnyButton, AnyModifier,
				    w->frame, TRUE,
				    ButtonPressMask | ButtonReleaseMask |
				    ButtonMotionMask, GrabModeSync,
...
void updateWindowOutputExtents(CompWindow * w)
{
	...
	if (output.left != w->output.left ||
	    ...
	    output.top != w->output.top || output.bottom != w->output.bottom) {
		w->output = output;
...
void
setWindowFullscreenMonitors(CompWindow * w, CompFullscreenMonitorSet * monitors)
{
	...
	if (!hasNew) {
		...
		if (result == Success && data) {
			...
			if (n == 4) {
				...
				new.right.width = struts[1];
				new.right.x =
				    w->screen->width - new.right.width;
				...
				new.bottom.height = struts[3];
				new.bottom.y =
				    w->screen->height - new.bottom.height;
			}
...
void addWindow(CompScreen * screen, Window id, Window aboveId)
{
	...
	if (screen->windowPrivateLen) {
		privates =
		    malloc(screen->windowPrivateLen * sizeof(CompPrivate));
		if (!privates) {
	...
	if (!w->attrib.override_redirect) {
		...
		if (!
			...
			w->desktop =
			    getWindowProp(d, w->id, d->winDesktopAtom,
					  w->desktop);
...
void removeWindow(CompWindow * w)
{
	...
	if (!w->destroyed) {
		...
		if (!w->attrib.override_redirect) {
			if (w->saveMask)
				XConfigureWindow(d->display, w->id, w->saveMask,
						 &w->saveWc);
...
void sendConfigureNotify(CompWindow * w)
{
	...
	if (w->attrib.override_redirect) {
		...
		if (XGetWindowAttributes
		    (w->screen->display->display, w->id, &attrib)) {
			xev.x = attrib.x;
			xev.y = attrib.y;
	     ...
	     int x, int y, int width, int height, int borderWidth)
{
	if (w->attrib.width != width ||
		...
		if (w->mapNum && w->redirected && !w->invisible) {
			pixmap =
			    XCompositeNameWindowPixmap(w->screen->display->
						       display, w->id);
			result =
			    XGetGeometry(w->screen->display->display, pixmap,
					 &root, &i, &i, &actualWidth,
					 &actualHeight, &ui, &ui);
			if (!result || actualWidth != pw || actualHeight != ph) {
				XFreePixmap(w->screen->display->display,
					    pixmap);
				return FALSE;
			}
		} else if (w->shaded) {
...
static Bool initializeSyncCounter(CompWindow * w)
{
	...
	result = XGetWindowProperty(w->screen->display->display, w->id,
				    w->screen->display->
				    wmSyncRequestCounterAtom, 0L, 1L, FALSE,
...
static Bool syncWaitTimeout(void *closure)
{
...
void sendSyncRequest(CompWindow * w)
{
	...
	XSendEvent(w->screen->display->display, w->id, FALSE, 0,
		   (XEvent *) & xev);
	...
	if (!w->syncWaitHandle)
		w->syncWaitHandle =
		    compAddTimeout(1000, 1200, syncWaitTimeout, w);
}
...
void syncWindowPosition(CompWindow * w)
{
	...
	XMoveWindow(w->screen->display->display, w->id, w->attrib.x,
		    w->attrib.y);
...
void windowResizeNotify(CompWindow * w, int dx, int dy, int dwidth, int dheight)
{
}
...
void moveInputFocusToWindow(CompWindow * w)
{
	...
	if (w->state & CompWindowStateHiddenMask) {
		...
		if (!setFocus && !modalTransient) {
			...
			for (ancestor = s->windows; ancestor;
...
static Bool validSiblingBelow(CompWindow * w, CompWindow * sibling)
{
	Window clientLeader = w->clientLeader;
	unsigned int type = w->type;
...
static void saveWindowGeometry(CompWindow * w, int mask)
{
...
static int restoreWindowGeometry(CompWindow * w, XWindowChanges * xwc, int mask)
{
	int m = mask & w->saveMask;
...
static void
reconfigureXWindow(CompWindow * w, unsigned int valueMask, XWindowChanges * xwc)
{
	if (valueMask & CWX)
		w->serverX = xwc->x;
...
stackTransients(CompWindow * w, CompWindow * avoid, XWindowChanges * xwc)
{
	...
	for (t = w->screen->reverseWindows; t; t = t->prev) {
		...
		if (t->transientFor == w->id
		    || isGroupTransient(t, clientLeader)) {
			if (w->type & CompWindowTypeDockMask)
			...
			if (t->mapNum || t->pendingMaps)
				reconfigureXWindow(t, CWSibling | CWStackMode,
						   xwc);
		}
	}
...
addWindowSizeChanges(CompWindow * w,
		     XWindowChanges * xwc,
		     int oldX,
		     int oldY, int oldWidth, int oldHeight, int oldBorderWidth)
{
	XRectangle workArea;
	int mask = 0;
	int x, y;
	...
	viewportForGeometry(w->screen,
			    oldX,
			    oldY,
			    oldWidth, oldHeight, oldBorderWidth, &vx, &vy);
	...
	output = outputDeviceForGeometry(w->screen,
					 oldX,
					 oldY,
					 oldWidth, oldHeight, oldBorderWidth);
	...
	if (w->type & CompWindowTypeFullscreenMask) {
		...
		if (w->fullscreenMonitorsSet) {
			...
			xwc->x =
			    x + w->screen->outputDev[output].region.extents.x1;
			xwc->y =
			    y + w->screen->outputDev[output].region.extents.y1;
		...
		if (mask & (CWWidth | CWHeight)) {
			...
			constrainNewWindowSize(w, width, height, &width,
					       &height);
			...
			if (w->state & CompWindowStateMaximizedVertMask) {
				if (oldY < y + workArea.y + w->input.top) {
					...
					if (oldY + oldHeight + w->input.bottom >
					    max) {
						xwc->y =
						    max - height -
						    w->input.bottom;
			...
			if (w->state & CompWindowStateMaximizedHorzMask) {
				if (oldX < x + workArea.x + w->input.left) {
					...
					if (oldX + oldWidth + w->input.right >
					    max) {
						xwc->x =
						    max - width -
						    w->input.right;
		      ...
		      XWindowChanges * xwc, CompWindow * sibling)
{
	...
	if (!sibling || sibling->id != w->id) {
		...
		if (prev) {
			if (!sibling) {
				XLowerWindow(w->screen->display->display,
					     w->id);
				if (w->frame)
					XLowerWindow(w->screen->display->
						     display, w->frame);
...
static CompWindow *focusTopmostWindow(CompScreen * s)
{
	CompDisplay *d = s->display;
	CompWindow *w;
...
void updateWindowAttributes(CompWindow * w, CompStackingUpdateMode stackingMode)
{
	XWindowChanges xwc;
	int mask = 0;
...
static void ensureWindowVisibility(CompWindow * w)
{
	...
	x2 = x1 + w->screen->workArea.width +
	    w->screen->hsize * w->screen->width;
...
void closeWindow(CompWindow * w, Time serverTime)
{
	...
	if (w->alive) {
		if (w->protocols & CompWindowProtocolDeleteMask) {
			...
			XSendEvent(display->display, w->id, FALSE, NoEventMask,
				   &ev);
		} else {
		...
		toolkitAction(w->screen,
			      w->screen->display->
			      toolkitActionForceQuitDialogAtom, serverTime,
			      w->id, TRUE, 0, 0);
	}
...
#define CLAMP(v, min, max) ((v) <= (min) ? (min) : (v) >= (max) ? (max) : (v))
	 ...
	 */
	if ((flags & PAspect) && hints->min_aspect.y > 0
		...
		if (width * max_aspect_y > max_aspect_x * height) {
			...
			if (width - delta >= min_width)
				...
				delta =
				    FLOOR64(width * min_aspect_y /
					    min_aspect_x - height, yinc);
...
static void minimizeTransients(CompWindow * w, void *closure)
{
...
static void unminimizeTransients(CompWindow * w, void *closure)
{
	CompWindow *ancestor = closure;
...
void unminimizeWindow(CompWindow * w)
{
	if (w->minimized) {
		...
		forEachWindowOnScreen(w->screen, unminimizeTransients,
				      (void *)w);
	}
}
...
static Bool getUsageTimestampForWindow(CompWindow * w, Time * timestamp)
{
...
static Bool getFocusWindowUsageTimestamp(CompWindow * w, Time * timestamp)
{
...
allowWindowFocus(CompWindow * w,
		 ...
		 unsigned int viewportX, unsigned int viewportY, Time timestamp)
{
	Bool status;
...
static CARD32 *allocateWindowIcon(CompWindow * w,
...
static void readWindowIconHint(CompWindow * w)
{
	...
	for (j = 0; j < height; j++) {
		for (i = 0; i < width; i++) {
			if (maskImage && !XGetPixel(maskImage, i, j))
				...
				*p++ =
				    colors[k].pixel ? 0xffffffff : 0xff000000;
			else
				*p++ = 0xff000000 |	/* alpha */
...
void setDesktopForWindow(CompWindow * w, unsigned int desktop)
{
	if (desktop != 0xffffffff) {
		if (w->
		    type & (CompWindowTypeDesktopMask | CompWindowTypeDockMask))
			return;
