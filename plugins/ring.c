...
static Bool isRingWin(CompWindow * w)
{
...
static void ringFreeWindowTitle(CompScreen * s)
{
	RING_SCREEN(s);
...
static void ringRenderWindowTitle(CompScreen * s)
{
...
static void ringDrawWindowTitle(CompScreen * s)
{
	float x, y;
	int ox1, ox2, oy1, oy2;
	...
	if (rs->state != RingStateNone) {
		...
		if (scaled && w->texture->pixmap) {
			...
			matrixTranslate(&wTransform,
					...
					rw->ty / rw->scale - w->attrib.y, 0.0f);
		...
		if (scaled && (rs->state != RingStateIn) &&
			...
			if (icon
				...
				matrix.x0 -=
				    (w->attrib.x * icon->texture.matrix.xx);
				matrix.y0 -=
				    (w->attrib.y * icon->texture.matrix.yy);
...
static int compareRingWindowDepth(const void *elem1, const void *elem2)
{
...
static void ringAddWindowToList(CompScreen * s, CompWindow * w)
{
	RING_SCREEN(s);

	if (rs->windowsSize <= rs->nWindows) {
		rs->windows = realloc(rs->windows,
				      sizeof(CompWindow *) * (rs->nWindows +
							      32));
		if (!rs->windows)
			return;

		rs->drawSlots = realloc(rs->drawSlots,
					sizeof(RingDrawSlot) * (rs->nWindows +
								32));
...
static Bool ringUpdateWindowList(CompScreen * s)
{
	int i;
	RING_SCREEN(s);
...
static Bool ringCreateWindowList(CompScreen * s)
{
	CompWindow *w;
...
static int ringCountWindows(CompScreen * s)
{
	CompWindow *w;
	int count = 0;
...
static int adjustRingVelocity(CompWindow * w)
{
...
static void ringPreparePaintScreen(CompScreen * s, int msSinceLastPaint)
{
	RING_SCREEN(s);
			...
			for (w = s->windows; w; w = w->next) {
				...
				if (rw->adjust) {
					...
					rw->scale =
					    ...
					    rw->slot->depthScale;
					rw->tx =
					    rw->slot->x - w->attrib.x -
					    (w->attrib.width * rw->scale) / 2;
					rw->ty =
					    rw->slot->y - w->attrib.y -
...
static void ringDonePaintScreen(CompScreen * s)
{
	RING_SCREEN(s);
	     ...
	     CompActionState state, CompOption * option, int nOption)
{
	...
	if (!rs->grabIndex) {
		if (ringGetSelectWithMouse(s))
			rs->grabIndex =
			    pushScreenGrab(s, s->normalCursor, "ring");
		else
			rs->grabIndex =
			    pushScreenGrab(s, s->invisibleCursor, "ring");
	}
...
ringDoSwitch(CompDisplay * d,
	     ...
	     CompOption * option, int nOption, Bool nextWindow, RingType type)
{
	CompScreen *s;
	Window xid;
	Bool ret = TRUE;
			...
			if (type == RingTypeGroup) {
				...
				if (w) {
					...
					rs->clientLeader =
					    ...
					    clientLeader : w->id;
					ret =
					    ringInitiate(s, action, state,
							 option, nOption);
				}
			} else {
				rs->type = type;
				ret =
				    ringInitiate(s, action, state, option,
						 nOption);
			}
...
static void ringWindowSelectAt(CompScreen * s, int x, int y, Bool terminate)
{
	int i;
	...
	for (i = rs->nWindows - 1; i >= 0; i--) {
		if (rs->drawSlots[i].slot && *(rs->drawSlots[i].slot)) {
			...
			if ((x >= (rw->tx + w->attrib.x)) &&
			    (x <=
			     (rw->tx + w->attrib.x +
			      (w->attrib.width * rw->scale)))
			    && (y >= (rw->ty + w->attrib.y))
			    && (y <=
				(rw->ty + w->attrib.y +
				 (w->attrib.height * rw->scale)))) {
...
static void ringWindowRemove(CompDisplay * d, CompWindow * w)
{
	if (w) {
		Bool inList = FALSE;
		int j, i = 0;
...
static void ringHandleEvent(CompDisplay * d, XEvent * event)
{
	CompScreen *s;
	CompWindow *w = NULL;
...
static Bool ringDamageWindowRect(CompWindow * w, Bool initial, BoxPtr rect)
{
	Bool status = FALSE;
	CompScreen *s = w->screen;
...
static Bool ringInitDisplay(CompPlugin * p, CompDisplay * d)
{
	RingDisplay *rd;
	int index;
...
static void ringFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	RING_DISPLAY(d);
...
static Bool ringInitScreen(CompPlugin * p, CompScreen * s)
{
	RingScreen *rs;
...
static void ringFiniScreen(CompPlugin * p, CompScreen * s)
{
	RING_SCREEN(s);
...
static Bool ringInitWindow(CompPlugin * p, CompWindow * w)
{
	RingWindow *rw;
...
static void ringFiniWindow(CompPlugin * p, CompWindow * w)
{
	RING_WINDOW(w);
...
static CompBool ringInitObject(CompPlugin * p, CompObject * o)
{
...
static void ringFiniObject(CompPlugin * p, CompObject * o)
{
...
static Bool ringInit(CompPlugin * p)
{
...
static void ringFini(CompPlugin * p)
{
