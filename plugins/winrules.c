...
static void
winrulesSetProtocols(CompDisplay * display, unsigned int protocols, Window id)
{
	Atom protocol[4];
	int count = 0;
...
static Bool isWinrulesWindow(CompWindow * w)
{
...
static void winrulesSetNoFocus(CompWindow * w, int optNum)
{
...
static void winrulesSetNoAlpha(CompWindow * w, int optNum)
{
	WINRULES_SCREEN(w->screen);
	WINRULES_WINDOW(w);
...
static void winrulesUpdateState(CompWindow * w, int optNum, int mask)
{
...
static void winrulesSetAllowedActions(CompWindow * w, int optNum, int action)
{
	WINRULES_SCREEN(w->screen);
	WINRULES_WINDOW(w);
...
winrulesMatchSizeValue(CompWindow * w,
		       ...
		       CompOption * heightValues, int *width, int *height)
{
	int i, min;
...
static Bool winrulesMatchSize(CompWindow * w, int *width, int *height)
{
	WINRULES_SCREEN(w->screen);

	return winrulesMatchSizeValue(w,
				      &ws->
				      opt[WINRULES_SCREEN_OPTION_SIZE_MATCHES],
				      &ws->
				      opt
				      [WINRULES_SCREEN_OPTION_SIZE_WIDTH_VALUES],
				      &ws->
				      opt
				      [WINRULES_SCREEN_OPTION_SIZE_HEIGHT_VALUES],
				      width, height);
}

static void winrulesUpdateWindowSize(CompWindow * w, int width, int height)
{
	XWindowChanges xwc;
	unsigned int xwcm = 0;
...
static CompOption *winrulesGetScreenOptions(CompPlugin * plugin,
	...
	case WINRULES_SCREEN_OPTION_NOARGB_MATCH:
		if (compSetMatchOption(o, value)) {
			...
			for (w = screen->windows; w; w = w->next)
				winrulesSetNoAlpha(w,
						   WINRULES_SCREEN_OPTION_NOARGB_MATCH);
...
static Bool winrulesApplyRules(CompWindow * w)
{
	int width, height;
...
static Bool winrulesApplyRulesTimeout(void *closure)
{
...
static void winrulesHandleEvent(CompDisplay * d, XEvent * event)
{
	CompWindow *w;
...
static void winrulesMatchExpHandlerChanged(CompDisplay * d)
{
	CompScreen *s;
	CompWindow *w;
...
static void winrulesMatchPropertyChanged(CompDisplay * d, CompWindow * w)
{
	WINRULES_DISPLAY(d);
...
static Bool winrulesInitDisplay(CompPlugin * p, CompDisplay * d)
{
	WinrulesDisplay *wd;
...
static void winrulesFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	WINRULES_DISPLAY(d);
...
static Bool winrulesInitScreen(CompPlugin * p, CompScreen * s)
{
	WinrulesScreen *ws;
...
static void winrulesFiniScreen(CompPlugin * p, CompScreen * s)
{
	WINRULES_SCREEN(s);
...
static Bool winrulesInitWindow(CompPlugin * p, CompWindow * w)
{
	WinrulesWindow *ww;
...
static void winrulesFiniWindow(CompPlugin * p, CompWindow * w)
{
	WINRULES_WINDOW(w);
...
static CompBool winrulesInitObject(CompPlugin * p, CompObject * o)
{
...
static void winrulesFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *winrulesGetObjectOptions(CompPlugin * plugin,
...
static Bool winrulesInit(CompPlugin * p)
{
...
static void winrulesFini(CompPlugin * p)
{
...
static CompMetadata *winrulesGetMetadata(CompPlugin * plugin)
{
