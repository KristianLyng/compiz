...
static void usage(void)
{
	printf("Usage: %s\n       "
	       ...
	       "[--replace]\n       "
	       "[--sm-disable] "
	       "[--sm-client-id ID] " "[--only-current-screen]\n      "
#ifdef USE_COW
	       " [--use-root-window] "
#endif
	       "[--debug] "
...
static int readCoreXmlCallback(void *context, char *buffer, int length)
{
	...
	i = compReadXmlChunk("<compiz><core><display>", &offset, buffer,
			     length);
	...
	for (i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "--help")) {
			...
			if (i + 1 < argc) {
				...
				defaultRefreshRate =
				    RESTRICT_VALUE(defaultRefreshRate, 1, 1000);
			}
