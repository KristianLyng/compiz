...
static void changePaintModifier(CompWindow * w, int modifier, int direction)
{
	int value;
...
static void updatePaintModifier(CompWindow * w, int modifier)
{
	int lastFactor;
	...
	if ((w->type & CompWindowTypeDesktopMask)
		...
		min =
		    MIN(matches->value.list.nValue, values->value.list.nValue);
		...
		for (i = 0; i < min; i++) {
			if (matchEval(&matches->value.list.value[i].match, w)) {
				ow->matchFactor[modifier] =
				    values->value.list.value[i].i;
				break;
			}
		}
	      ...
	      const FragmentAttrib * attrib, Region region, unsigned int mask)
{
	...
	if (hasCustomFactor) {
		...
		if (factor != 100)
			fragment.brightness =
			    (int)fragment.brightness * factor / 100;

		factor = ow->customFactor[MODIFIER_SATURATION];
		if (factor != 100)
			fragment.saturation =
			    (int)fragment.saturation * factor / 100;
...
static void obsMatchExpHandlerChanged(CompDisplay * d)
{
	CompScreen *s;
	CompWindow *w;
	int i;
...
static void obsMatchPropertyChanged(CompDisplay * d, CompWindow * w)
{
	int i;
...
static Bool obsUpdateWindow(void *closure)
{
...
static CompOption *obsGetDisplayOptions(CompPlugin * p,
...
static CompOption *obsGetScreenOptions(CompPlugin * p,
...
obsSetScreenOption(CompPlugin * p,
		   CompScreen * s, char *name, CompOptionValue * value)
{
	CompOption *o;
	int i;
...
static CompOption *obsGetObjectOptions(CompPlugin * plugin,
...
static CompBool obsInitDisplay(CompPlugin * p, CompDisplay * d)
{
	ObsDisplay *od;
	int opt;
...
static void obsFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	OBS_DISPLAY(d);
...
static CompBool obsInitScreen(CompPlugin * p, CompScreen * s)
{
	ObsScreen *os;
	int mod;
...
static void obsFiniScreen(CompPlugin * p, CompScreen * s)
{
	OBS_SCREEN(s);
...
static CompBool obsInitWindow(CompPlugin * p, CompWindow * w)
{
	ObsWindow *ow;
	int i;
...
static void obsFiniWindow(CompPlugin * p, CompWindow * w)
{
	OBS_WINDOW(w);
...
static CompBool obsInitObject(CompPlugin * p, CompObject * o)
{
...
static void obsFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompBool obsInit(CompPlugin * p)
{
...
static void obsFini(CompPlugin * p)
{
...
static CompMetadata *obsGetMetadata(CompPlugin * plugin)
{
	return &obsMetadata;
}
