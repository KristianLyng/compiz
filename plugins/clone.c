...
static void cloneRemove(CompScreen * s, int i)
{
	CloneClone *clone;
...
static void cloneFinish(CompScreen * s)
{
	CloneClone *clone;
	int i;
		...
		if (!clone) {
			...
			if (region) {
				...
				if (clone) {
					...
					x = s->outputDev[cs->dst].region.
					    extents.x1;
					y = s->outputDev[cs->dst].region.
					    extents.y1;
...
static void cloneDonePaintScreen(CompScreen * s)
{
	CLONE_SCREEN(s);
...
static void cloneSetStrutsForCloneWindow(CompScreen * s, CloneClone * clone)
{
...
static void cloneHandleMotionEvent(CompScreen * s, int xRoot, int yRoot)
{
	CLONE_SCREEN(s);
...
static void cloneOutputChangeNotify(CompScreen * s)
{
	int i;
...
static Bool cloneInitScreen(CompPlugin * p, CompScreen * s)
{
	CloneScreen *cs;
...
static void cloneFiniScreen(CompPlugin * p, CompScreen * s)
{
	int i;
...
static CompBool cloneInitObject(CompPlugin * p, CompObject * o)
{
...
static void cloneFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *cloneGetObjectOptions(CompPlugin * plugin,
...
static void cloneFini(CompPlugin * p)
{
	freeDisplayPrivateIndex(displayPrivateIndex);
	compFiniMetadata(&cloneMetadata);
}

static CompMetadata *cloneGetMetadata(CompPlugin * plugin)
{
