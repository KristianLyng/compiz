...
static void renderThumbText(CompScreen * s, Thumbnail * t, Bool freeThumb)
{
	...
	t->textData =
	    (td->textFunc->renderWindowTitle) (s, t->win->id, FALSE, &tA);
}
		...
		if (!ts->showingThumb &&
			...
			if (ts->displayTimeout)
			{
				if (ts->pointedWin != found) {
					...
					ts->displayTimeout =
					    compAddTimeout(thumbnailGetShowDelay
							   (s),
							   thumbnailGetShowDelay
							   (s) + 500,
							   thumbShowThumbnail,
							   s);
				}
				...
				ts->displayTimeout =
				    compAddTimeout(thumbnailGetShowDelay(s),
						   thumbnailGetShowDelay(s) +
						   500, thumbShowThumbnail, s);
			}
		}
...
static void thumbHandleEvent(CompDisplay * d, XEvent * event)
{
	...
	case PropertyNotify:
		if (event->xproperty.atom == d->wmNameAtom) {
			...
			if (w) {
				...
				if (ts->thumb.win == w
				    && thumbnailGetTitleEnabled(w->screen))
					renderThumbText(w->screen, &ts->thumb,
							TRUE);
			}
		}
		break;
	...
	case ButtonPress:
		{
			CompScreen *s =
			    findScreenAtDisplay(d, event->xbutton.root);
	...
	case EnterNotify:
		...
		if (w) {
			...
			if (w->wmType & CompWindowTypeDockMask) {
				...
				if (ts->pollHandle) {
					(*td->mpFunc->removePositionPolling) (s,
									      ts->
									      pollHandle);
					ts->pollHandle = 0;
				}
	...
	case LeaveNotify:
		...
		if (w) {
			...
			if (w->wmType & CompWindowTypeDockMask) {
				...
				if (ts->pollHandle) {
					(*td->mpFunc->
					 removePositionPolling) (w->screen,
								 ts->
								 pollHandle);
					ts->pollHandle = 0;
				}
...
static void
thumbPaintThumb(CompScreen * s, Thumbnail * t, const CompTransform * transform)
{
	...
	if (w->texture->pixmap) {
		...
		if (thumbnailGetWindowLike(s)) {
			...
			enableTexture(s, &ts->windowTexture,
			...
			enableTexture(s, &ts->glowTexture,
				      COMP_TEXTURE_FILTER_GOOD);
		}
...
static CompBool thumbInitObject(CompPlugin * p, CompObject * o)
{
...
static void thumbFiniObject(CompPlugin * p, CompObject * o)
{
