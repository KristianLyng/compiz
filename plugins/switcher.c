...
static CompOption *switchGetScreenOptions(CompPlugin * plugin,
...
switchInitiate(CompScreen * s, SwitchWindowSelection selection, Bool showPopup)
{
	...
	if (!ss->popupWindow && showPopup) {
		...
		XSetWMProperties(dpy, ss->popupWindow, NULL, NULL,
				 programArgv, programArgc, &xsh, &xwmh, &xch);
	...
	if (!ss->grabIndex)
		ss->grabIndex =
		    pushScreenGrab(s, s->invisibleCursor, "switcher");
	...
	if (w) {
		...
		if (ss->nWindows == 2) {
			if (ss->windows[0] == ss->windows[1]) {
				...
				switchAddWindowToList(w->screen,
						      ss->windows[0]);
				switchAddWindowToList(w->screen,
						      ss->windows[1]);
			}
	...
	if (ss->zooming) {
		...
		if (ss->selectedWindow == ss->zoomedWindow) {
			if (fabs(dx) < 0.1f && fabs(ss->mVelocity) < 0.2f &&
				...
				ss->mVelocity = ss->tVelocity = ss->sVelocity =
				    0.0f;
				return 0;
			}
		}
		  ...
		  Region region, CompOutput * output, unsigned int mask)
{
	...
	if (ss->grabIndex || (ss->zooming && ss->translate > 0.001f)) {
		...
		if (ss->zooming) {
			...
			matrixTranslate(&sTransform, 0.0f, 0.0f,
					-ss->translate);
		...
		if (ss->zooming) {
			...
			UNWRAP(ss, s, paintOutput);
			status =
			    (*s->paintOutput) (s, sAttrib, &sTransform, region,
					       output, mask);
			WRAP(ss, s, paintOutput, switchPaintOutput);
		}
...
switchPaintThumb(CompWindow * w,
		 ...
		 const CompTransform * transform,
		 unsigned int mask, int x, int y, int x1, int x2)
{
	...
	if (w->texture->pixmap) {
		...
		if (ss->opt[SWITCH_SCREEN_OPTION_MIPMAP].value.b)
			w->screen->display->textureFilter =
		...
		if (ss->opt[SWITCH_SCREEN_OPTION_ICON].value.b) {
			...
			if (icon) {
				...
				wx = x + WIDTH - icon->width * sAttrib.xScale -
				    SPACE;
				wy = y + HEIGHT -
				    icon->height * sAttrib.yScale - SPACE;
			}
		}
	} else {
		width = WIDTH - (WIDTH >> 2);
		  ...
		  Region region, unsigned int mask)
{
	...
	if (w->id == ss->popupWindow) {
		...
		for (i = 0; i < 4; i++)
			color[i] =
			    (unsigned int)ss->fgColor[i] *
			    w->lastPaint.opacity / 0xffff;
		glColor4usv(color);
		glPushMatrix();
...
static CompOption *switchGetDisplayOptions(CompPlugin * plugin,
...
static CompOption *switchGetObjectOptions(CompPlugin * plugin,
...
static CompMetadata *switchGetMetadata(CompPlugin * plugin)
{
