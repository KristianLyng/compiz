...
static void convertToZoomedTarget(CompScreen * s,
				  int out,
				  int x, int y, int *resultX, int *resultY);
static void
convertToZoomed(CompScreen * s,
		int out, int x, int y, int *resultX, int *resultY);
...
static Bool dontuseScreengrabExist(CompScreen * s, char *grab)
{
	int i;
...
static Bool outputIsZoomArea(CompScreen * s, int out)
{
	ZOOM_SCREEN(s);
...
static Bool isActive(CompScreen * s, int out)
{
	ZOOM_SCREEN(s);
 ...
 */
static Bool isZoomed(CompScreen * s, int out)
{
	ZOOM_SCREEN(s);
...
static int distanceToEdge(CompScreen * s, int out, ZoomEdge edge)
{
	int x1, y1, x2, y2;
...
static void updateActualTranslates(ZoomArea * za)
{
 ...
 */
static Bool isInMovement(CompScreen * s, int out)
{
	ZOOM_SCREEN(s);

	if (zs->zooms[out].currentZoom == 1.0f &&
	    zs->zooms[out].newZoom == 1.0f && zs->zooms[out].zVelocity == 0.0f)
		return FALSE;
...
static void initialiseZoomArea(ZoomArea * za, int out)
{
	za->output = out;
	za->currentZoom = 1.0f;
	za->newZoom = 1.0f;
...
static void adjustZoomVelocity(CompScreen * s, int out, float chunk)
{
	...
	if (fabs(d) < 0.1f && fabs(zs->zooms[out].zVelocity) < 0.005f) {
		...
		zs->zooms[out].currentZoom +=
		    (zs->zooms[out].zVelocity * chunk) / s->redrawTime;
	}
}

/* Adjust the X/Y velocity based on target translation and real
 * translation. */
static void adjustXYVelocity(CompScreen * s, int out, float chunk)
{
	float xdiff, ydiff;
...
drawBox(CompScreen * s,
	const CompTransform * transform, CompOutput * output, Box box)
{
		...
		Region region, CompOutput * output, unsigned int mask)
{
	...
	if (isActive(s, out)) {
		...
		matrixScale(&zTransform,
			    ...
			    1.0f / zs->zooms[out].currentZoom, 1.0f);
		matrixTranslate(&zTransform,
				zs->zooms[out].xtrans,
				zs->zooms[out].ytrans, 0);
		...
		status =
		    (*s->paintOutput) (s, &sa, &zTransform, region, output,
				       mask);
 ...
 * of the screen/head.  */
static inline void constrainZoomTranslate(CompScreen * s)
{
	int out;
	ZOOM_SCREEN(s);
 ...
 */
static void setCenter(CompScreen * s, int x, int y, Bool instant)
{
...
setZoomArea(CompScreen * s, int x, int y, int width, int height, Bool instant)
{
	...
	zs->zooms[out].yTranslate =
	    (float)-((o->height / 2) -
		     (y + (height / 2) - o->region.extents.y1))
	    / (o->height);
...
static void zoomAreaToWindow(CompWindow * w)
{
 ...
 * TODO: Fix output. */
static void panZoom(CompScreen * s, int xvalue, int yvalue)
{
	int out;
	ZOOM_SCREEN(s);
 ...
 * stored values.
 */
static void enableMousePolling(CompScreen * s)
{
	ZOOM_SCREEN(s);
 ...
 */
static void setScale(CompScreen * s, int out, float value)
{
	ZOOM_SCREEN(s);
 ...
 */
static inline void setScaleBigger(CompScreen * s, int out, float x, float y)
{
...
static Bool ensureVisibility(CompScreen * s, int x, int y, int margin)
{
	int zoomX, zoomY;
	int out;
	CompOutput *o;
...
ensureVisibilityArea(CompScreen * s,
		     int x1,
		     int y1, int x2, int y2, int margin, ZoomGravity gravity)
{
...
static void restrainCursor(CompScreen * s, int out)
{
	...
	convertToZoomedTarget
	    (s, out,
	     ...
	     zs->mouseY - zs->cursor.hotY + zs->cursor.height, &x2, &y2);
	...
	if (abs(diffX) * z > 0 || abs(diffY) * z > 0)
		warpPointer(s,
			    (int)(zs->mouseX - pointerX) -
			    (int)((float)diffX * z),
			    (int)(zs->mouseY - pointerY) -
			    (int)((float)diffY * z));
}
 ...
 */
static void cursorMoved(CompScreen * s)
{
	int out;
	ZOOM_SCREEN(s);
...
static void updateMouseInterval(CompScreen * s, int x, int y)
{
	...
	if (!zs->grabbed) {
		ZOOM_DISPLAY(s->display);
		if (zs->pollHandle)
			(*zd->mpFunc->removePositionPolling) (s,
							      zs->pollHandle);
		zs->pollHandle = 0;
		cursorMoved(s);
	}
...
drawCursor(CompScreen * s, CompOutput * output, const CompTransform * transform)
{
	...
	if (zs->cursor.isSet) {
		...
		if (zs->opt[SOPT_SCALE_MOUSE_DYNAMIC].value.b)
			...
			scaleFactor =
			    1.0f / zs->opt[SOPT_SCALE_MOUSE_STATIC].value.f;
		  ...
		  CompActionState state, CompOption * option, int nOption)
{
	...
	if (s) {
		...
		restrain =
		    getBoolOptionNamed(option, nOption, "restrain", FALSE);
		  ...
		  CompActionState state, CompOption * option, int nOption)
{
	...
	for (s = d->screens; s; s = s->next) {
		...
		if (zs->grabIndex) {
			...
			out =
			    outputDeviceForGeometry(s, x, y, width, height, 0);
			o = &s->outputDev[out];
...
zoomSpecific(CompDisplay * d,
	     ...
	     CompOption * option, int nOption, float target)
{
	CompScreen *s;
	Window xid;
		    ...
		    CompActionState state, CompOption * option, int nOption)
{
	...
	constrainNewWindowSize(w,
			       xwc.width, xwc.height, &xwc.width, &xwc.height);
 ...
 */
static void focusTrack(CompDisplay * d, XEvent * event)
{
	int out;
	...
	if (zs->opt[SOPT_FOCUS_FIT_WINDOW].value.b) {
		...
		int height = w->height + w->input.top + w->input.bottom;
		float scale =
		    MAX((float)width / w->screen->outputDev[out].width,
...
static const CompMetadataOptionInfo zoomDisplayOptionInfo[] = {
	...
	{"zoom_spec1", "float",
	 "<min>0.1</min><max>1.0</max><default>1.0</default>", 0, 0},
	{"zoom_spec2", "float",
	 "<min>0.1</min><max>1.0</max><default>0.5</default>", 0, 0},
	{"zoom_spec3", "float",
	 "<min>0.1</min><max>1.0</max><default>0.2</default>", 0, 0},
...
static CompOption *zoomGetScreenOptions(CompPlugin * plugin,
...
static CompOption *zoomGetDisplayOptions(CompPlugin * plugin,
...
static Bool zoomInitDisplay(CompPlugin * p, CompDisplay * d)
{
	...
	zd->fixesSupported =
	    XFixesQueryExtension(d->display,
				 &zd->fixesEventBase, &zd->fixesErrorBase);
...
static CompBool zoomInitObject(CompPlugin * p, CompObject * o)
{
...
static void zoomFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *zoomGetObjectOptions(CompPlugin * plugin,
...
static CompMetadata *zoomGetMetadata(CompPlugin * plugin)
{
	return &zoomMetadata;
}
