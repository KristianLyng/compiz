...
static void minSetShade(CompWindow * w, int shade)
{
	REGION rect;
...
static int minGetWindowState(CompWindow * w)
{
	Atom actual;
	int result, format;
...
static int adjustMinVelocity(CompWindow * w)
{
...
static void minPreparePaintScreen(CompScreen * s, int msSinceLastPaint)
{
	MIN_SCREEN(s);
	...
	if (ms->moreAdjust) {
		...
		steps =
		    amount / (0.5f *
			      ms->opt[MIN_SCREEN_OPTION_TIMESTEP].value.f);
		if (!steps)
			steps = 1;
		...
		while (steps--) {
			...
			for (w = s->windows; w; w = w->next) {
				...
				if (mw->adjust) {
					...
					mw->xScale +=
					    mw->xScaleVelocity * chunk;
					mw->yScale +=
					    mw->yScaleVelocity * chunk;
...
static void minDonePaintScreen(CompScreen * s)
{
	MIN_SCREEN(s);
	       ...
	       Region region, unsigned int mask)
{
	...
	if (mw->adjust) {
		...
		matrixTranslate(&wTransform,
				...
				mw->ty / mw->yScale - w->attrib.y, 0.0f);
...
static void minHandleEvent(CompDisplay * d, XEvent * event)
{
	CompWindow *w;
	...
	case UnmapNotify:
		...
		if (w) {
			...
			if (w->pendingUnmaps && onCurrentDesktop(w)) {	/* Normal -> Iconic */
				CompMatch *match =
				    &ms->opt[MIN_SCREEN_OPTION_WINDOW_MATCH].
				    value.match;
				...
				if (w->shaded) {
					...
					if (mw->region && ms->shadeStep) {
						XSubtractRegion(w->region,
								&emptyRegion,
								mw->region);
						XOffsetRegion(mw->region,
							      -w->attrib.x,
							      w->attrib.height +
							      w->attrib.
							      border_width * 2 -
...
static Bool minDamageWindowRect(CompWindow * w, Bool initial, BoxPtr rect)
{
	Bool status = FALSE;
	...
	if (initial) {
		if (mw->state == IconicState) {
			CompMatch *match =
			    &ms->opt[MIN_SCREEN_OPTION_WINDOW_MATCH].value.
			    match;

			mw->state = NormalState;

			if (!w->invisible &&
			    w->iconGeometrySet && matchEval(match, w)) {
				if (!mw->adjust) {
					mw->adjust = TRUE;
					ms->moreAdjust = TRUE;
					...
					mw->xScale =
					    (float)w->iconGeometry.width /
					    w->width;
					mw->yScale =
					    (float)w->iconGeometry.height /
					    w->height;
			...
			if (ms->shadeStep && !w->invisible) {
				XSubtractRegion(w->region, &emptyRegion,
						mw->region);
...
static Bool minFocusWindow(CompWindow * w)
{
	Bool status;
...
static Bool minInitDisplay(CompPlugin * p, CompDisplay * d)
{
	MinDisplay *md;
...
static Bool minInitWindow(CompPlugin * p, CompWindow * w)
{
	MinWindow *mw;
...
static CompBool minInitObject(CompPlugin * p, CompObject * o)
{
...
static void minFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *minGetObjectOptions(CompPlugin * plugin,
...
static void minFini(CompPlugin * p)
{
	freeDisplayPrivateIndex(displayPrivateIndex);
	compFiniMetadata(&minMetadata);
}

static CompMetadata *minGetMetadata(CompPlugin * plugin)
{
	return &minMetadata;
}
