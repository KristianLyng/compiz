	       ...
	       unsigned int height, GLenum format, GLenum type)
{
	...
	internalFormat =
	    (screen->opt[COMP_SCREEN_OPTION_TEXTURE_COMPRESSION].value.b &&
	     screen->textureCompression ? GL_COMPRESSED_RGBA_ARB : GL_RGBA);
...
imageBufferToTexture(CompScreen * screen,
		     CompTexture * texture,
		     const char *image, unsigned int width, unsigned int height)
{
...
imageDataToTexture(CompScreen * screen,
		   ...
		   unsigned int height, GLenum format, GLenum type)
{
	return imageToTexture(screen, texture, image, width, height, format,
			      type);
}
	...
	if (!target) {
		if (!(config->textureTargets & GLX_TEXTURE_2D_BIT_EXT))
			target = GLX_TEXTURE_RECTANGLE_EXT;
			 ...
			 (config->
			  textureTargets & GLX_TEXTURE_RECTANGLE_BIT_EXT))
			target = GLX_TEXTURE_2D_EXT;
	}
