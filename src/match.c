...
static void matchResetOps(CompDisplay * display, CompMatchOp * op, int nOp)
{
	while (nOp--) {
		switch (op->type) {
...
static void matchReset(CompMatch * match)
{
	if (match->display)
...
static void matchFiniOps(CompMatchOp * op, int nOp)
{
	while (nOp--) {
		switch (op->type) {
		case CompMatchOpTypeGroup:
...
static Bool matchOpsEqual(CompMatchOp * op1, CompMatchOp * op2, int nOp)
{
	while (nOp--) {
		...
		case CompMatchOpTypeGroup:
			...
			if (!matchOpsEqual
			    (op1->group.op, op2->group.op, op1->group.nOp))
				return FALSE;
...
static CompMatchOp *matchAddOp(CompMatch * match,
			       CompMatchOpType type, int flags)
{
	CompMatchOp *op;
...
static Bool matchCopyOps(CompMatchOp * opDst, CompMatchOp * opSrc, int nOpSrc)
{
	...
	while (nOpSrc--) {
		...
		case CompMatchOpTypeGroup:
			...
			if (!matchCopyOps
			    (op, opSrc->group.op, opSrc->group.nOp)) {
				free(op);
				matchFiniOps(first, count);
				return FALSE;
			}
...
Bool matchAddExp(CompMatch * match, int flags, const char *str)
{
	CompMatchOp *op;
	char *value;
...
static int nextIndex(const char *str, int i)
{
	while (str[i] == '\\')
		if (str[++i] != '\0')
			i++;
...
static char *strndupValue(const char *str, int n)
{
	char *value;
...
static char *matchOpsToString(CompMatchOp * op, int nOp)
{
	char *value, *group;
	char *str = NULL;
	int length = 0;
	...
	while (nOp--) {
		...
		case CompMatchOpTypeGroup:
			...
			if (group) {
				value =
				    malloc(sizeof(char) * (strlen(group) + 7));
				if (value)
		...
		case CompMatchOpTypeExp:
			value =
			    malloc(sizeof(char) * (strlen(op->exp.value) + 5));
			if (value)
				sprintf(value, "%s%s%s%s", !str ? "" :
					((op->any.
					  flags & MATCH_OP_AND_MASK) ? "& " :
...
static void matchUpdateOps(CompDisplay * display, CompMatchOp * op, int nOp)
{
	while (nOp--) {
		switch (op->type) {
		case CompMatchOpTypeGroup:
			...
			break;
		case CompMatchOpTypeExp:
			(*display->matchInitExp) (display, &op->exp.e,
						  op->exp.value);
			break;
		}
...
matchEvalOps(CompDisplay * display,
	     CompMatchOp * op, int nOp, CompWindow * window)
{
	...
	while (nOp--) {
		...
		case CompMatchOpTypeGroup:
			value =
			    matchEvalOps(display, op->group.op, op->group.nOp,
					 window);
			break;
		case CompMatchOpTypeExp:
		default:
			value =
			    (*op->exp.e.eval) (display, window, op->exp.e.priv);
			break;
		}
...
{
	if (match->display)
		return matchEvalOps(match->display, match->op, match->nOp,
				    window);
...
matchEvalTypeExp(CompDisplay * display,
		 CompWindow * window, CompPrivate private)
{
...
static Bool
matchEvalIdExp(CompDisplay * display, CompWindow * window, CompPrivate private)
{
...
static void matchUpdateMatchOptions(CompOption * option, int nOption)
{
	while (nOption--) {
		switch (option->type) {
		case CompOptionTypeMatch:
			if (option->value.match.display)
				matchUpdate(option->value.match.display,
					    &option->value.match);
			break;
...
void matchExpHandlerChanged(CompDisplay * display)
{
	...
	for (p = getPlugins(); p; p = p->next) {
		...
		option =
		    (*p->vTable->getObjectOptions) (p, &display->base,
						    &nOption);
	...
	for (s = display->screens; s; s = s->next) {
		for (p = getPlugins(); p; p = p->next) {
			...
			option =
			    (*p->vTable->getObjectOptions) (p, &s->base,
							    &nOption);
			matchUpdateMatchOptions(option, nOption);
		}
	}
...
void matchPropertyChanged(CompDisplay * display, CompWindow * w)
{
}
