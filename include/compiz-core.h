...
typedef CompBool(*ObjectCallBackProc) (CompObject * object, void *closure);

typedef CompBool(*ObjectTypeCallBackProc) (CompObjectType type,
					   CompObject * parent, void *closure);

void
compObjectInit(CompObject * object,
	       CompPrivate * privates, CompObjectType type);

void compObjectFini(CompObject * object);

int compObjectAllocatePrivateIndex(CompObject * parent, CompObjectType type);

void
compObjectFreePrivateIndex(CompObject * parent, CompObjectType type, int index);

CompBool
compObjectForEach(CompObject * parent,
		  CompObjectType type, ObjectCallBackProc proc, void *closure);

CompBool
compObjectForEachType(CompObject * parent,
		      ObjectTypeCallBackProc proc, void *closure);

const char *compObjectTypeName(CompObjectType type);

char *compObjectName(CompObject * object);

CompObject *compObjectFind(CompObject * parent,
			   CompObjectType type, const char *name);
...
sessionEvent(CompCore * c,
	     CompSessionEvent event,
	     CompOption * arguments, unsigned int nArguments);

char *getSessionClientId(CompSessionClientIdType type);
...
getBoolOptionNamed(CompOption * option,
		   int nOption, const char *name, Bool defaultValue);

int
getIntOptionNamed(CompOption * option,
		  int nOption, const char *name, int defaultValue);

float
getFloatOptionNamed(CompOption * option,
		    int nOption, const char *name, float defaultValue);

char *getStringOptionNamed(CompOption * option,
			   int nOption, const char *name, char *defaultValue);
...
CompMatch *getMatchOptionNamed(CompOption * option,
			       int nOption,
			       const char *name, CompMatch * defaultValue);
...
char *keyActionToString(CompDisplay * d, CompAction * action);
...
void
stringToKeyAction(CompDisplay * d, const char *binding, CompAction * action);

void
stringToButtonAction(CompDisplay * d, const char *binding, CompAction * action);

const char *edgeToString(unsigned int edge);
...
const char *optionTypeToString(CompOptionType type);

Bool isActionOption(CompOption * option);
...
typedef void (*ObjectAddProc) (CompObject * parent, CompObject * object);
typedef void (*ObjectRemoveProc) (CompObject * parent, CompObject * object);
...
typedef void (*FileWatchCallBackProc) (const char *name, void *closure);
...
typedef void (*FileWatchAddedProc) (CompCore * core, CompFileWatch * fileWatch);
...
typedef void (*LogMessageProc) (const char *componentName,
				CompLogLevel level, const char *message);
...
int allocCoreObjectPrivateIndex(CompObject * parent);

void freeCoreObjectPrivateIndex(CompObject * parent, int index);

CompBool
forEachCoreObject(CompObject * parent, ObjectCallBackProc proc, void *closure);

char *nameCoreObject(CompObject * object);

CompObject *findCoreObject(CompObject * parent, const char *name);
...
void freeCorePrivateIndex(int index);

void addDisplayToCore(CompDisplay * d);

CompFileWatchHandle
addFileWatch(const char *path,
	     int mask, FileWatchCallBackProc callBack, void *closure);
...
typedef void (*HandleEventProc) (CompDisplay * display, XEvent * event);
...
typedef void (*ForEachWindowProc) (CompWindow * window, void *closure);

typedef Bool(*FileToImageProc) (CompDisplay * display,
				const char *name,
				int *width,
				int *height, int *stride, void **data);
...
typedef void (*CompMatchExpFiniProc) (CompDisplay * display, CompPrivate priv);

typedef Bool(*CompMatchExpEvalProc) (CompDisplay * display,
				     CompWindow * window, CompPrivate priv);
...
typedef void (*MatchInitExpProc) (CompDisplay * display,
				  CompMatchExp * exp, const char *value);
...
CompBool allocDisplayObjectPrivates(CompObject * object, CompObject * parent);

int allocDisplayObjectPrivateIndex(CompObject * parent);

void freeDisplayObjectPrivateIndex(CompObject * parent, int index);

CompBool
forEachDisplayObject(CompObject * parent,
		     ObjectCallBackProc proc, void *closure);

char *nameDisplayObject(CompObject * object);

CompObject *findDisplayObject(CompObject * parent, const char *name);

int allocateDisplayPrivateIndex(void);

void freeDisplayPrivateIndex(int index);

CompOption *getDisplayOptions(CompPlugin * plugin,
...
compLogMessage(const char *componentName,
	       CompLogLevel level, const char *format, ...);
...
Bool addDisplay(const char *name);

void removeDisplay(CompDisplay * d);
...
CompWindow *findWindowAtDisplay(CompDisplay * display, Window id);
...
void warpPointer(CompScreen * screen, int dx, int dy);
...
CompCursor *findCursorAtDisplay(CompDisplay * display);
...
void handleEvent(CompDisplay * display, XEvent * event);
...
Bool eventMatches(CompDisplay * display, XEvent * event, CompOption * option);

Bool
eventTerminates(CompDisplay * display, XEvent * event, CompOption * option);
...
typedef void (*WalkerFiniProc) (CompScreen * screen, CompWalker * walker);
...
transformToScreenSpace(CompScreen * screen,
		       CompOutput * output, float z, CompTransform * transform);

/* XXX: prepareXCoords will be removed */
void prepareXCoords(CompScreen * screen, CompOutput * output, float z);
...
applyScreenTransform(CompScreen * screen,
		     const ScreenPaintAttrib * sAttrib,
		     CompOutput * output, CompTransform * transform);
...
void disableOutputClipping(CompScreen * screen);

void
paintScreen(CompScreen * screen,
	    CompOutput * outputs, int numOutput, unsigned int mask);
...
Bool moreWindowVertices(CompWindow * w, int newSize);

Bool moreWindowIndices(CompWindow * w, int newSize);

void
addWindowGeometry(CompWindow * w,
		  CompMatrix * matrix, int nMatrix, Region region, Region clip);

void
drawWindowTexture(CompWindow * w,
		  CompTexture * texture,
		  const FragmentAttrib * fragment, unsigned int mask);

Bool
drawWindow(CompWindow * w,
	   const CompTransform * transform,
	   const FragmentAttrib * fragment, Region region, unsigned int mask);
...
typedef int (*GLXWaitVideoSyncProc) (int divisor,
				     int remainder, unsigned int *count);
...
typedef void (*GLXDestroyPixmapProc) (Display * display, GLXPixmap pixmap);
...
typedef void (*GLGetProgramivProc) (GLenum target, GLenum pname, int *params);
...
typedef Bool(*DamageWindowRectProc) (CompWindow * w, Bool initial, BoxPtr rect);

typedef Bool(*DamageWindowRegionProc) (CompWindow * w, Region region);

typedef Bool(*DamageCursorRectProc) (CompCursor * c, Bool initial, BoxPtr rect);
...
typedef unsigned int (*AddSupportedAtomsProc) (CompScreen * s,
					       Atom * atoms, unsigned int size);

typedef void (*InitWindowWalkerProc) (CompScreen * screen, CompWalker * walker);
...
CompBool allocScreenObjectPrivates(CompObject * object, CompObject * parent);

int allocScreenObjectPrivateIndex(CompObject * parent);

void freeScreenObjectPrivateIndex(CompObject * parent, int index);

CompBool
forEachScreenObject(CompObject * parent,
		    ObjectCallBackProc proc, void *closure);

char *nameScreenObject(CompObject * object);
...
void updateOutputWindow(CompScreen * s);
...
void removeScreen(CompScreen * s);

void damageScreenRegion(CompScreen * screen, Region region);

void damageScreen(CompScreen * screen);
...
void focusDefaultWindow(CompScreen * s);
...
void moveWindowToViewportPosition(CompWindow * w, int x, int y, Bool sync);
...
void getWorkareaForOutput(CompScreen * s, int output, XRectangle * area);
...
void clearScreenOutput(CompScreen * s, CompOutput * output, unsigned int mask);
...
viewportForGeometry(CompScreen * s,
		    ...
		    int width,
		    int height,
		    int borderWidth, int *viewportX, int *viewportY);
...
CompCursor *findCursorAtScreen(CompScreen * screen);

CompCursorImage *findCursorImageAtScreen(CompScreen * screen,
					 unsigned long serial);

void setCurrentActiveWindowHistory(CompScreen * s, int x, int y);

void addToCurrentActiveWindowHistory(CompScreen * s, Window id);

void setWindowPaintOffset(CompScreen * s, int x, int y);
...
CompBool allocWindowObjectPrivates(CompObject * object, CompObject * parent);

int allocWindowObjectPrivateIndex(CompObject * parent);

void freeWindowObjectPrivateIndex(CompObject * parent, int index);

CompBool
forEachWindowObject(CompObject * parent,
		    ObjectCallBackProc proc, void *closure);

char *nameWindowObject(CompObject * object);

CompObject *findWindowObject(CompObject * parent, const char *name);
...
void changeWindowState(CompWindow * w, unsigned int newState);
...
readWindowProp32(CompDisplay * display,
		 Window id, Atom property, unsigned short *returnValue);
...
void updateNormalHints(CompWindow * window);

void updateWmHints(CompWindow * w);

void updateWindowClassHints(CompWindow * window);

void updateTransientHint(CompWindow * w);

void updateIconGeometry(CompWindow * w);
...
void setWindowFrameExtents(CompWindow * w, CompWindowExtents * input);

void updateWindowOutputExtents(CompWindow * w);
...
adjustConfigureRequestForGravity(CompWindow * w,
				 XWindowChanges * xwc,
				 unsigned int xwcm, int gravity, int direction);

void
moveResizeWindow(CompWindow * w,
		 XWindowChanges * xwc,
		 unsigned int xwcm, int gravity, unsigned int source);

void syncWindowPosition(CompWindow * w);

void sendSyncRequest(CompWindow * w);
...
void getOutputExtentsForWindow(CompWindow * w, CompWindowExtents * output);
...
Bool placeWindow(CompWindow * w, int x, int y, int *newX, int *newY);
...
void windowStateChangeNotify(CompWindow * w, unsigned int lastState);
...
void lowerWindow(CompWindow * w);

void restackWindowAbove(CompWindow * w, CompWindow * sibling);

void restackWindowBelow(CompWindow * w, CompWindow * sibling);
...
void showWindow(CompWindow * w);

void minimizeWindow(CompWindow * w);

void unminimizeWindow(CompWindow * w);

void maximizeWindow(CompWindow * w, int state);

Bool getWindowUserTime(CompWindow * w, Time * time);

void setWindowUserTime(CompWindow * w, Time time);

CompFocusResult
allowWindowFocus(CompWindow * w,
		 unsigned int noFocusMask,
		 unsigned int viewportX,
		 unsigned int viewportY, Time timestamp);

void unredirectWindow(CompWindow * w);
...
void freeWindowIcons(CompWindow * w);
...
int compareWindowActiveness(CompWindow * w1, CompWindow * w2);

Bool windowOnAllViewports(CompWindow * w);

void
getWindowMovementForOffset(CompWindow * w,
			   int offX, int offY, int *retX, int *retY);
...
typedef char **(*ListPluginsProc) (const char *path, int *n);
...
CompPlugin *loadPlugin(const char *plugin);
...
void matrixVectorDiv(CompVector * v);
...
void matchInit(CompMatch * match);

void matchFini(CompMatch * match);

Bool matchEqual(CompMatch * m1, CompMatch * m2);

Bool matchCopy(CompMatch * dst, CompMatch * src);

Bool matchAddGroup(CompMatch * match, int flags, CompMatch * group);

Bool matchAddExp(CompMatch * match, int flags, const char *value);

void matchAddFromString(CompMatch * match, const char *str);

char *matchToString(CompMatch * match);

void matchUpdate(CompDisplay * display, CompMatch * match);

Bool matchEval(CompMatch * match, CompWindow * window);

void matchInitExp(CompDisplay * display, CompMatchExp * exp, const char *value);

void matchExpHandlerChanged(CompDisplay * display);

void matchPropertyChanged(CompDisplay * display, CompWindow * window);
...
compInitScreenOptionFromMetadata(CompScreen * screen,
				 ...
				 CompOption * option, const char *name);

void compFiniScreenOption(CompScreen * screen, CompOption * option);

Bool
compInitScreenOptionsFromMetadata(CompScreen * screen,
				  CompMetadata * metadata,
				  const CompMetadataOptionInfo * info,
				  CompOption * option, int n);

void compFiniScreenOptions(CompScreen * screen, CompOption * option, int n);

Bool
compSetScreenOption(CompScreen * screen,
		    CompOption * option, CompOptionValue * value);
...
void compFiniDisplayOption(CompDisplay * display, CompOption * option);
...
void compFiniDisplayOptions(CompDisplay * display, CompOption * option, int n);

Bool
compSetDisplayOption(CompDisplay * display,
		     CompOption * option, CompOptionValue * value);

char *compGetShortPluginDescription(CompMetadata * metadata);

char *compGetLongPluginDescription(CompMetadata * metadata);

char *compGetShortScreenOptionDescription(CompMetadata * metadata,
					  CompOption * option);

char *compGetLongScreenOptionDescription(CompMetadata * metadata,
					 CompOption * option);

char *compGetShortDisplayOptionDescription(CompMetadata * metadata,
					   CompOption * option);

char *compGetLongDisplayOptionDescription(CompMetadata * metadata,
					  CompOption * option);

int
compReadXmlChunkFromMetadataOptionInfo(const CompMetadataOptionInfo * info,
				       int *offset, char *buffer, int length);
