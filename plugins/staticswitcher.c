...
static void switchActivateEvent(CompScreen * s, Bool activating)
{
	...
	(*d->handleCompizEvent) (s->display, "staticswitcher", "activate", o,
				 2);
}
...
static void switchUpdatePopupWindow(CompScreen * s, int count)
{
...
static Bool switchGetPaintRectangle(CompWindow * w, BoxPtr rect, int *opacity)
{
...
static void switchDoWindowDamage(CompWindow * w)
{
	if (w->attrib.map_state == IsViewable || w->shaded)
		addWindowDamage(w);
	else {
...
static void switchToWindow(CompScreen * s, Bool toNext)
{
	...
	if (w) {
		...
		if (ss->selection == AllViewports
		    && staticswitcherGetAutoChangeVp(s)) {
			XEvent xev;
			int x, y;
...
static Bool switchShowPopup(void *closure)
{
...
static inline Cursor switchGetCursor(CompScreen * s, Bool mouseSelect)
{
	if (mouseSelect)
...
static void
switchInitiate(CompScreen * s, SwitchWindowSelection selection, Bool showPopup)
{
	CompDisplay *d = s->display;
	int count;
	Bool mouseSelect;
	...
	if (!ss->popupWindow && showPopup) {
		...
		XSetWMProperties(dpy, ss->popupWindow, NULL, NULL,
				 programArgv, programArgc, NULL, &xwmh, &xch);
		...
		XChangeProperty(dpy, ss->popupWindow,
				...
				XA_ATOM, 32, PropModeReplace,
				(unsigned char *)&s->display->winTypeUtilAtom,
				1);
	...
	if (!ss->grabIndex)
		ss->grabIndex =
		    pushScreenGrab(s, switchGetCursor(s, mouseSelect),
				   "switcher");
	else if (mouseSelect != ss->mouseSelect)
		updateScreenGrab(s, ss->grabIndex,
				 switchGetCursor(s, mouseSelect));
		...
		if (ss->grabIndex) {
			...
			if (ss->popupWindow) {
				...
				if (w && w->managed && w->mapNum) {
					...
					XUnmapWindow(s->display->display,
						     ss->popupWindow);
				}
			}
...
switchInitiateCommon(CompDisplay * d,
		     ...
		     int nOption,
		     SwitchWindowSelection selection,
		     Bool showPopup, Bool nextWindow)
{
	CompScreen *s;
	Window xid;
	...
	if (s) {
		...
		if (!ss->switching) {
			if (selection == Group) {
				...
				xid2 =
				    getIntOptionNamed(option, nOption, "window",
						      0);
...
static void switchWindowRemove(CompDisplay * d, CompWindow * w)
{
	if (w) {
		Bool inList = FALSE;
		int count, j, i = 0;
...
static void updateForegroundColor(CompScreen * s)
{
	Atom actual;
	int result, format;
...
static inline int switchGetRowXOffset(CompScreen * s, SwitchScreen * ss, int y)
{
	int retval = 0;
...
static void
switchGetWindowPosition(CompScreen * s, unsigned int index, int *x, int *y)
{
	int row, column;
...
static CompWindow *switchFindWindowAt(CompScreen * s, int x, int y)
{
	CompWindow *popup;
...
switchPaintRect(BoxRec * box,
		unsigned int offset, unsigned short *color, int opacity)
{
	...
	if (ss->grabIndex) {
		...
		if (mode == HighlightModeBringSelectedToFront) {
			...
			if (zoomed && !zoomed->destroyed) {
				...
				for (w = zoomed->prev; w && w->id <= 1;
				     w = w->prev) ;
				zoomedAbove = (w) ? w->id : None;

				unhookWindowFromScreen(s, zoomed);
				insertWindowIntoScreen(s, zoomed,
						       s->reverseWindows->id);
			}
		} else {
			zoomed = NULL;
			...
			if (mode == HighlightModeShowRectangle) {
				...
				if (w) {
					...
					if (switchGetPaintRectangle
						...
						color =
						    staticswitcherGetHighlightBorderColor
						    (s);
						switchPaintRect(&box, 0, color,
								opacity);
						switchPaintRect(&box, 2, color,
								opacity);
						color =
						    staticswitcherGetHighlightBorderInlayColor
						    (s);
						switchPaintRect(&box, 1, color,
								opacity);
		...
		UNWRAP(ss, s, paintOutput);
		status =
		    (*s->paintOutput) (s, sAttrib, transform, region, output,
				       mask);
		WRAP(ss, s, paintOutput, switchPaintOutput);
	}
		 ...
		 unsigned int mask, int x, int y)
{
	...
	if (w->texture->pixmap) {
		...
		sAttrib.yTranslate =
		    wy - w->attrib.y + w->input.top * sAttrib.yScale;
		...
		matrixTranslate(&wTransform,
				sAttrib.xTranslate / sAttrib.xScale -
				w->attrib.x,
				sAttrib.yTranslate / sAttrib.yScale -
				w->attrib.y, 0.0f);
		...
		if (staticswitcherGetIcon(s)) {
			...
			if (icon) {
				...
				sAttrib.xScale =
				    (float)ss->previewWidth * xScale /
				    PREVIEWSIZE;
				sAttrib.yScale =
				    (float)ss->previewWidth * yScale /
				    PREVIEWSIZE;

				wx = x + ss->previewWidth -
				    (sAttrib.xScale * icon->width);
				wy = y + ss->previewHeight -
				    (sAttrib.yScale * icon->height);
			}
		}
	} else {
	...
	if (icon && (icon->texture.name || iconToTexture(w->screen, icon))) {
		...
		if (w->vCount) {
			...
			matrixTranslate(&wTransform, w->attrib.x, w->attrib.y,
					0.0f);
			matrixScale(&wTransform, sAttrib.xScale, sAttrib.yScale,
				    1.0f);
			matrixTranslate(&wTransform,
					sAttrib.xTranslate / sAttrib.xScale -
					w->attrib.x,
...
switchPaintSelectionRect(SwitchScreen * ss,
			 int x, int y, float dx, float dy, unsigned int opacity)
{
	int i;
	float color[4], op;
	unsigned int w, h;
		  ...
		  Region region, unsigned int mask)
{
	...
	if (w->id == ss->popupWindow) {
		...
		if (pos > ss->nWindows - 1) {
			...
			switchPaintSelectionRect(ss, w->attrib.x, w->attrib.y,
						 px, 0.0, w->lastPaint.opacity);

			px = fmod(pos, ss->xCount);
			switchPaintSelectionRect(ss, w->attrib.x + offX,
						 w->attrib.y, px, py,
						 w->lastPaint.opacity);
		}
		if (px > ss->xCount - 1) {
			...
			py = fmod(py + 1,
				  ceil((double)ss->nWindows / ss->xCount));
			offX = switchGetRowXOffset(s, ss, py);

			switchPaintSelectionRect(ss, w->attrib.x + offX,
						 w->attrib.y, px - ss->xCount,
						 py, w->lastPaint.opacity);
		} else {
			switchPaintSelectionRect(ss, w->attrib.x + offX,
						 w->attrib.y, px, py,
						 w->lastPaint.opacity);
...
static CompBool switchInitObject(CompPlugin * p, CompObject * o)
{
...
static void switchFiniObject(CompPlugin * p, CompObject * o)
{
