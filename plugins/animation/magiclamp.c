...
Bool fxMagicLampInit(CompWindow * w)
{
	...
	aw->magicLampWaveCount = 1 + (float)maxWaves *distance / screenHeight;
	...
	for (i = 0; i < aw->magicLampWaveCount; i++) {
		...
		if (i > 0)
			posInAvailSegment =
			    (availPos / aw->magicLampWaveCount) * rand() /
			    RAND_MAX;
...
void fxMagicLampModelStep(CompWindow * w, float time)
{
	...
	float iconShadowLeft =
	    ((float)(w->output.left - w->input.left)) * icon->width / w->width;
	...
	for (i = 0; i < model->numObjects; i++, object++) {
		...
		float fx = ((iconCloseEndY - object->position.y) /
			    (iconCloseEndY - winFarEndY));
		float fy = ((sigmoid(fx) - sigmoid0) / (sigmoid1 - sigmoid0));
