...
static void videoSetSupportedHint(CompScreen * s)
{
	Atom data[16];
	int i, n = 0;
...
static CompOption *videoGetDisplayOptions(CompPlugin * plugin,
...
static int
getYV12FragmentFunction(CompScreen * s, CompTexture * texture, int param)
{
...
static void
videoDestroyFragmentFunctions(CompScreen * s, VideoFunction ** videoFunctions)
{
	...
	if (vw->context) {
		...
		if (src->format == IMAGE_FORMAT_YV12 &&
			...
			if (function) {
				...
				maxX =
				    COMP_TEX_COORD_X(&texture->matrix,
						     src->width - 1.0f);

				y1 = COMP_TEX_COORD_Y(&texture->matrix, 1.0f);
				y2 = COMP_TEX_COORD_Y(&texture->matrix,
						      src->height - 1.0f);
				...
				(GL_FRAGMENT_PROGRAM_ARB, param, minX, minY,
				 maxX, maxY);
		...
		const FragmentAttrib * attrib, Region region, unsigned int mask)
{
	...
	if (status) {
		...
		if (vw->context && region->numRects) {
			CompTexture *texture =
			    &vw->context->source->texture->texture;
			int saveFilter;

			w->vCount = w->indexCount = 0;

			if (vw->context->box.extents.x1 <
			...
			if (w->vCount)
				(*w->screen->drawWindowTexture) (w, texture,
								 attrib, mask);
...
static VideoTexture *videoGetTexture(CompScreen * screen, Pixmap pixmap)
{
	VideoTexture *texture;
...
static void videoReleaseTexture(CompScreen * screen, VideoTexture * texture)
{
	VIDEO_DISPLAY(screen->display);
...
static void updateWindowVideoMatrix(CompWindow * w)
{
	VIDEO_WINDOW(w);
...
static void updateWindowVideoContext(CompWindow * w, VideoSource * source)
{
	int x1, y1, x2, y2;
	...
	if (source->aspect) {
		...
		if (aspect < source->aspectRatio) {
			v = (width +
			     width * source->panScan) / source->aspectRatio;
			height = MIN(vw->context->height, v);
			width = height * source->aspectRatio;
		} else {
			v = (height +
			     height * source->panScan) * source->aspectRatio;
	...
	if (x1 == 0 && y1 == 0 && x2 == w->width && y2 == w->height) {
...
static void videoWindowUpdate(CompWindow * w)
{
	Atom actual;
	int result, format, i;
...
static void videoHandleEvent(CompDisplay * d, XEvent * event)
{
	CompWindow *w;
...
static Bool videoDamageWindowRect(CompWindow * w, Bool initial, BoxPtr rect)
{
	Bool status;
...
static void
videoWindowMoveNotify(CompWindow * w, int dx, int dy, Bool immediate)
{
	VIDEO_SCREEN(w->screen);
	VIDEO_WINDOW(w);
...
static void
videoWindowResizeNotify(CompWindow * w, int dx, int dy, int dwidth, int dheight)
{
	VIDEO_SCREEN(w->screen);
	VIDEO_WINDOW(w);
...
static Bool videoInitDisplay(CompPlugin * p, CompDisplay * d)
{
	VideoDisplay *vd;
	...
	vd->videoAtom = XInternAtom(d->display, "_COMPIZ_VIDEO", 0);
...
static void videoFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	VIDEO_DISPLAY(d);
...
static Bool videoInitScreen(CompPlugin * p, CompScreen * s)
{
	VideoScreen *vs;
...
static void videoFiniScreen(CompPlugin * p, CompScreen * s)
{
	VIDEO_DISPLAY(s->display);
	VIDEO_SCREEN(s);
...
static Bool videoInitWindow(CompPlugin * p, CompWindow * w)
{
	VideoWindow *vw;
...
static void videoFiniWindow(CompPlugin * p, CompWindow * w)
{
	VIDEO_WINDOW(w);
...
static CompBool videoInitObject(CompPlugin * p, CompObject * o)
{
...
static void videoFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *videoGetObjectOptions(CompPlugin * plugin,
...
static Bool videoInit(CompPlugin * p)
{
	if (!compInitPluginMetadataFromInfo(&videoMetadata,
					    p->vTable->name,
					    videoDisplayOptionInfo,
					    VIDEO_DISPLAY_OPTION_NUM, 0, 0))
		return FALSE;
...
static void videoFini(CompPlugin * p)
{
	freeDisplayPrivateIndex(displayPrivateIndex);
	compFiniMetadata(&videoMetadata);
}

static CompMetadata *videoGetMetadata(CompPlugin * plugin)
{
