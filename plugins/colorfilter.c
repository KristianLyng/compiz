 ...
 */
static void colorFilterToggleWindow(CompWindow * w)
{
	FILTER_WINDOW(w);
 ...
 */
static void colorFilterToggleScreen(CompScreen * s)
{
	CompWindow *w;
 ...
 */
static void colorFilterSwitchFilter(CompScreen * s)
{
	int id;
	CompFunction *function;
	CompWindow *w;
 ...
 */
static void unloadFilters(CompScreen * s)
{
	int i;
 ...
 */
static int loadFilters(CompScreen * s, CompTexture * texture)
{
			     ...
			     const FragmentAttrib * attrib, unsigned int mask)
{
	...
	if (cfs->filtersCount && cfw->isFiltered &&
		...
		/* Single filter mode */
			...
			function =
			    cfs->filtersFunctions[cfs->currentFilter - 1];
			if (function)
				addFragmentFunction(&fa, function);
		}
		UNWRAP(cfs, w->screen, drawWindowTexture);
		(*w->screen->drawWindowTexture) (w, texture, &fa, mask);
		WRAP(cfs, w->screen, drawWindowTexture,
		     colorFilterDrawWindowTexture);
	} else {		/* Not filtering */
 ...
 */
static void colorFilterWindowAdd(CompScreen * s, CompWindow * w)
{
	FILTER_SCREEN(s);
...
static void colorFilterObjectAdd(CompObject * parent, CompObject * object)
{
...
static Bool colorFilterInitCore(CompPlugin * p, CompCore * c)
{
	ColorFilterCore *fc;
...
static void colorFilterFiniCore(CompPlugin * p, CompCore * c)
{
	FILTER_CORE(c);
...
static Bool colorFilterInitDisplay(CompPlugin * p, CompDisplay * d)
{
...
static void colorFilterFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	FILTER_DISPLAY(d);
...
static Bool colorFilterInitScreen(CompPlugin * p, CompScreen * s)
{
...
static void colorFilterFiniScreen(CompPlugin * p, CompScreen * s)
{
	FILTER_SCREEN(s);
...
static Bool colorFilterInitWindow(CompPlugin * p, CompWindow * w)
{
...
static void colorFilterFiniWindow(CompPlugin * p, CompWindow * w)
{
	if (!w->screen->fragmentProgram)
		return;
...
static CompBool colorFilterInitObject(CompPlugin * p, CompObject * o)
{
...
static void colorFilterFiniObject(CompPlugin * p, CompObject * o)
{
...
static Bool colorFilterInit(CompPlugin * p)
{
...
static void colorFilterFini(CompPlugin * p)
{
