...
initSvgTexture(CompWindow * w,
	       SvgSource * source, SvgTexture * texture, int width, int height)
{
...
static void finiSvgTexture(CompScreen * s, SvgTexture * texture)
{
	if (texture->cr)
...
static void updateWindowSvgMatrix(CompWindow * w)
{
	CompMatrix *m;
	int width, height;
	      ...
	      const FragmentAttrib * attrib, Region region, unsigned int mask)
{
	...
	if (status) {
		...
		if (sw->context && region->numRects) {
			...
			(*w->screen->drawWindowTexture) (w, texture, attrib,
							 mask);
			...
			if (r.extents.x1 < r.extents.x2
				...
				if (r.extents.x1 != sw->context->rect.x1 ||
					...
					finiSvgTexture(w->screen,
						       &sw->context->
						       texture[1]);

					if (initSvgTexture
					    (w, sw->context->source,
					     &sw->context->texture[1], width,
					     height)) {
						renderSvg(w->screen,
							  sw->context->source,
							  &sw->context->
				...
				initSvgTexture(w, sw->source,
					       &sw->context->texture[1], 0, 0);
...
static void updateWindowSvgContext(CompWindow * w, SvgSource * source)
{
	int x1, y1, x2, y2;
       ...
       CompActionState state, CompOption * option, int nOption)
{
	...
	if (w) {
		...
		if (data)
			svg =
			    rsvg_handle_new_from_data((guint8 *) data,
						      strlen(data), &error);
...
static void svgWindowMoveNotify(CompWindow * w, int dx, int dy, Bool immediate)
{
	SVG_SCREEN(w->screen);
	SVG_WINDOW(w);
...
static void
svgWindowResizeNotify(CompWindow * w, int dx, int dy, int dwidth, int dheight)
{
	SVG_SCREEN(w->screen);
	SVG_WINDOW(w);
	...
	if (strcmp(pluginName, "zoom") == 0) {
		...
		if (s && output == 0) {
			...
			if (strcmp(eventName, "in") == 0) {
				ss->zoom.x1 =
				    getIntOptionNamed(option, nOption, "x1", 0);
				ss->zoom.y1 =
				    getIntOptionNamed(option, nOption, "y1", 0);
				ss->zoom.x2 =
				    getIntOptionNamed(option, nOption, "x2", 0);
				ss->zoom.y2 =
				    getIntOptionNamed(option, nOption, "y2", 0);
...
static Bool
readSvgFileToImage(const char *file, int *width, int *height, void **data)
{
	cairo_surface_t *surface;
	FILE *fp;
	GError *error = NULL;
...
static CompOption *svgGetDisplayOptions(CompPlugin * plugin,
...
static Bool svgInitScreen(CompPlugin * p, CompScreen * s)
{
	SvgScreen *ss;
...
static void svgFiniScreen(CompPlugin * p, CompScreen * s)
{
	SVG_SCREEN(s);
...
static Bool svgInitWindow(CompPlugin * p, CompWindow * w)
{
	SvgWindow *sw;
...
static void svgFiniWindow(CompPlugin * p, CompWindow * w)
{
	SVG_WINDOW(w);
...
static CompBool svgInitObject(CompPlugin * p, CompObject * o)
{
...
static void svgFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *svgGetObjectOptions(CompPlugin * plugin,
...
static CompMetadata *svgGetMetadata(CompPlugin * plugin)
{
	return &svgMetadata;
}
