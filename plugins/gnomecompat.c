...
static void
panelAction(CompDisplay * d, CompOption * option, int nOption, Atom actionAtom)
{
	Window xid;
	CompScreen *s;
	XEvent event;
...
static CompBool gnomeInitDisplay(CompPlugin * p, CompDisplay * d)
{
	GnomeDisplay *gd;
	int opt, index;
	...
	gd->opt[opt].value.action.priv.val =
	    GNOME_DISPLAY_OPTION_SCREENSHOT_CMD;
...
static void gnomeFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	GNOME_DISPLAY(d);
...
static CompOption *gnomeGetDisplayOptions(CompPlugin * p,
...
static CompBool gnomeInitObject(CompPlugin * p, CompObject * o)
{
...
static void gnomeFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompOption *gnomeGetObjectOptions(CompPlugin * p,
					 CompObject * o, int *count)
{
...
gnomeSetObjectOption(CompPlugin * p,
		     CompObject * o, const char *name, CompOptionValue * value)
{
...
static Bool gnomeInit(CompPlugin * p)
{
...
static void gnomeFini(CompPlugin * p)
{
	freeDisplayPrivateIndex(displayPrivateIndex);
	compFiniMetadata(&gnomeMetadata);
}

static CompMetadata *gnomeGetMetadata(CompPlugin * p)
{
