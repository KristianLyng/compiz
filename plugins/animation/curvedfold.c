...
getObjectZ(Model * model,
	   ...
	   float sinForProg, float relDistToCenter, float curveMaxAmp)
{
	return -(sinForProg *
		 (1 - pow(pow(2 * relDistToCenter, 1.3), 2)) *
		 curveMaxAmp * model->scale.x);
}
...
fxCurvedFoldModelStepObject(CompWindow * w,
			    ...
			    Object * object,
			    float forwardProgress,
			    float sinForProg, float curveMaxAmp)
{
	ANIM_WINDOW(w);
...
void fxCurvedFoldModelStep(CompWindow * w, float time)
{
	...
	float curveMaxAmp =
	    0.4 * pow((float)WIN_H(w) / w->screen->height, 0.4) * animGetF(w,
	...
	for (i = 0; i < model->numObjects; i++, object++)
		fxCurvedFoldModelStepObject
		    (w,
		     model, object, forwardProgress, sinForProg, curveMaxAmp);
}
