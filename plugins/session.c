...
static void sessionFreeWindowListItem(SessionWindowList * item)
{
	if (item->clientId)
		free(item->clientId);
...
static void sessionAddWindowListItem(SessionWindowList * item)
{
...
static void sessionRemoveWindowListItem(SessionWindowList * item)
{
	SessionWindowList *run;
...
static char *sessionGetUtf8Property(CompDisplay * d, Window id, Atom atom)
{
	Atom type;
	int format;
...
static char *sessionGetTextProperty(CompDisplay * d, Window id, Atom atom)
{
	XTextProperty text;
	char *retval = NULL;
...
static char *sessionGetWindowTitle(CompWindow * w)
{
...
static Bool sessionGetIsEmbedded(CompDisplay * d, Window id)
{
	Atom type;
	int format;
	unsigned long nitems;
	...
	result =
	    XGetWindowProperty(d->display, id, sd->embedInfoAtom, 0L, 65536,
...
static char *sessionGetClientLeaderProperty(CompWindow * w, Atom atom)
{
	Window clientLeader;
	...
	if (!clientLeader) {
		...
		while (window && window->transientFor) {
			...
			window =
			    findWindowAtScreen(w->screen, window->transientFor);
...
static int sessionGetIntForProp(xmlNodePtr node, char *prop)
{
	xmlChar *temp;
	int num;
...
static char *sessionGetStringForProp(xmlNodePtr node, char *prop)
{
	xmlChar *text;
	char *retval = NULL;
...
static Bool sessionWindowIsOnAllViewports(CompWindow * w)
{
...
static Bool isSessionWindow(CompWindow * w)
{
	...
	if (matchEval
	    (&sd->opt[SESSION_DISPLAY_OPTION_IGNORE_MATCH].value.match, w))
		return FALSE;

	return TRUE;
}

static void addIntegerPropToNode(xmlNodePtr node, const char *name, int value)
{
...
static void sessionAddWindowNode(CompWindow * w, xmlNodePtr rootNode)
{
	...
	string =
	    sessionGetTextProperty(w->screen->display, w->id, sd->roleAtom);
	if (string) {
	...
	if (childNode) {
		...
		height =
		    (w->saveMask & CWHeight) ? w->saveWc.height : w->
		    serverHeight;
	...
	if (w->state & MAXIMIZE_STATE) {
		...
		if (childNode) {
			if (w->state & CompWindowStateMaximizedVertMask)
				xmlNewProp(childNode, BAD_CAST "vert",
					   BAD_CAST "yes");
			if (w->state & CompWindowStateMaximizedHorzMask)
				xmlNewProp(childNode, BAD_CAST "horiz",
					   BAD_CAST "yes");
		}
	}
...
static void saveState(CompDisplay * d, const char *clientId)
{
	char *filename;
	...
	if (mkdir(filename, 0700) == 0 || errno == EEXIST) {
		...
		if (mkdir(filename, 0700) == 0 || errno == EEXIST) {
			...
			ctx =
			    xmlSaveToFilename(filename, NULL, XML_SAVE_FORMAT);
		}
	}
...
static Bool sessionMatchWindowClass(CompWindow * w, SessionWindowList * info)
{
...
static Bool sessionReadWindow(CompWindow * w)
{
	CompDisplay *d = w->screen->display;
	XWindowChanges xwc;
	unsigned int xwcm = 0;
	...
	for (cur = sc->windowList; cur; cur = cur->next) {
		if (clientId && cur->clientId
		    && strcmp(clientId, cur->clientId) == 0) {
...
static void readState(xmlNodePtr root)
{
	...
	for (cur = root->xmlChildrenNode; cur; cur = cur->next) {
		...
		for (attrib = cur->xmlChildrenNode; attrib;
		     attrib = attrib->next) {
			if (xmlStrcmp(attrib->name, BAD_CAST "geometry") == 0) {
				item->geometryValid = TRUE;
				item->geometry.x =
				    sessionGetIntForProp(attrib, "x");
				item->geometry.y =
				    sessionGetIntForProp(attrib, "y");
				item->geometry.width =
				    sessionGetIntForProp(attrib, "width");
				item->geometry.height =
				    sessionGetIntForProp(attrib, "height");
			}
...
static void loadState(CompDisplay * d, char *previousId)
{
	xmlDocPtr doc;
	xmlNodePtr root;
	char *filename;
...
static void sessionWindowAdd(CompScreen * s, CompWindow * w)
{
	if (!w->attrib.override_redirect && w->attrib.map_state == IsViewable)
		sessionReadWindow(w);
}

static void sessionHandleEvent(CompDisplay * d, XEvent * event)
{
	CompWindow *w = NULL;
	unsigned int state;
		    ...
		    CompOption * arguments, unsigned int nArguments)
{
	...
	if (event == CompSessionEventSaveYourself) {
		...
		if (saveSession && clientId) {
			...
			object =
			    compObjectFind(&c->base, COMP_OBJECT_TYPE_DISPLAY,
					   NULL);
			if (object) {
...
static Bool sessionWindowAddTimeout(void *closure)
{
...
static CompOption *sessionGetDisplayOptions(CompPlugin * p,
...
static CompOption *sessionGetObjectOptions(CompPlugin * plugin,
...
static void sessionFini(CompPlugin * p)
{
	freeCorePrivateIndex(corePrivateIndex);
	compFiniMetadata(&sessionMetadata);
}

static Bool sessionInitCore(CompPlugin * p, CompCore * c)
{
	SessionCore *sc;
...
static void sessionFiniCore(CompPlugin * p, CompCore * c)
{
...
static int sessionInitDisplay(CompPlugin * p, CompDisplay * d)
{
	SessionDisplay *sd;
	char *prevClientId;
...
static void sessionFiniDisplay(CompPlugin * p, CompDisplay * d)
{
	SESSION_DISPLAY(d);
...
static CompBool sessionInitObject(CompPlugin * p, CompObject * o)
{
...
static void sessionFiniObject(CompPlugin * p, CompObject * o)
{
...
static CompMetadata *sessionGetMetadata(CompPlugin * plugin)
{
