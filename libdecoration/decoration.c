...
decor_quads_to_property(long *data,
			...
			int min_width,
			int min_height, decor_quad_t * quad, int nQuad)
{
...
int decor_property_get_version(long *data)
{
	return (int)*data;
}
...
decor_property_to_quads(long *data,
			...
			decor_extents_t * input,
			decor_extents_t * max_input,
			int *min_width, int *min_height, decor_quad_t * quad)
{
	int i, n, flags;
...
add_blur_boxes(long *data,
	       BoxPtr box,
	       int n_box, int width, int height, int gravity, int offset)
{
	int x1, y1, x2, y2;
	int more_gravity;
	int n = n_box;
...
decor_set_vert_quad_row(decor_quad_t * q,
			...
			int height,
			int splitY,
			int splitGravity, double x0, double y0, int rotation)
{
	int nQuad = 0;
...
decor_set_horz_quad_line(decor_quad_t * q,
			 ...
			 int gravity,
			 int width,
			 int splitX, int splitGravity, double x0, double y0)
{
	int nQuad = 0;
...
decor_set_lSrS_window_quads(decor_quad_t * q,
			    decor_context_t * c, decor_layout_t * l)
{
	...
	n = decor_set_vert_quad_row(q,
				    ...
				    lh,
				    splitY,
				    0, l->left.x1, l->left.y1, l->rotation);
	...
	n = decor_set_vert_quad_row(q,
				    ...
				    rh,
				    splitY,
				    0, l->right.x1, l->right.y1, l->rotation);
				...
				decor_context_t * c, decor_layout_t * l)
{
	...
	n = decor_set_horz_quad_line(q,
				     ...
				     splitX, 0, l->bottom.x1, l->bottom.y1);
...
decor_set_lSrStXbS_window_quads(decor_quad_t * q,
				decor_context_t * c,
				decor_layout_t * l, int top_stretch_offset)
{
	int splitX, n, nQuad = 0;
	...
	n = decor_set_horz_quad_line(q,
				     ...
				     l->top.x2 - l->top.x1,
				     -top_right,
				     GRAVITY_EAST, l->top.x1, l->top.y1);
...
decor_set_lSrStSbX_window_quads(decor_quad_t * q,
				decor_context_t * c,
				decor_layout_t * l, int bottom_stretch_offset)
{
	int splitX, n, nQuad = 0;
	...
	n = decor_set_horz_quad_line(q,
				     ...
				     l->bottom.x2 - l->bottom.x1,
				     -bottom_right,
				     GRAVITY_EAST, l->bottom.x1, l->bottom.y1);
				...
				int bottom_stretch_offset)
{
	...
	n = decor_set_vert_quad_row(q,
				    ...
				    -left_bottom,
				    GRAVITY_SOUTH,
				    l->left.x1, l->left.y1, l->rotation);
	...
	n = decor_set_vert_quad_row(q,
				    ...
				    -right_bottom,
				    GRAVITY_SOUTH,
				    l->right.x1, l->right.y1, l->rotation);
...
static int error_handler(Display * xdisplay, XErrorEvent * event)
{
	errors++;
	return 0;
}
...
static void set_picture_transform(Display * xdisplay, Picture p, int dx, int dy)
{
...
set_picture_clip(Display * xdisplay,
		 Picture p,
		 int width,
		 int height, int clipX1, int clipY1, int clipX2, int clipY2)
{
	XRectangle clip[4];
...
static void set_no_picture_clip(Display * xdisplay, Picture p)
{
	XRectangle clip;
...
static XFixed *create_gaussian_kernel(double radius,
				      double sigma, double alpha, int *r_size)
{
	XFixed *params;
...
decor_shadow_t *decor_shadow_create(Display * xdisplay,
				    Screen * screen,
				    int width,
				    int height,
				    int left,
				    ...
				    decor_context_t * c,
				    decor_draw_func_t draw, void *closure)
{
	...
	XRenderComposite(xdisplay,
			 PictOpSrc,
			 src, dst, tmp, 0, 0, 0, 0, 0, 0, d_width, d_height);
	...
	XRenderComposite(xdisplay,
			 PictOpSrc,
			 src, tmp, dst, 0, 0, 0, 0, 0, 0, d_width, d_height);
	...
	if (opacity != (1 << 16)) {
		...
		XRenderComposite(xdisplay,
				 ...
				 dst,
				 None,
				 tmp, 0, 0, 0, 0, 0, 0, d_width, d_height);
...
void decor_shadow_destroy(Display * xdisplay, decor_shadow_t * shadow)
{
	shadow->ref_count--;
	if (shadow->ref_count)
		return;
...
void decor_shadow_reference(decor_shadow_t * shadow)
{
	shadow->ref_count++;
}
...
decor_draw_simple(Display * xdisplay,
		  Pixmap pixmap,
		  Picture picture,
		  int width, int height, decor_context_t * c, void *closure)
{
...
decor_get_default_layout(decor_context_t * c,
			 int width, int height, decor_layout_t * layout)
{
	width = MAX(width, c->left_corner_space + c->right_corner_space);
	height = MAX(height, c->top_corner_space + c->bottom_corner_space);
		      ...
		      int width, int height, decor_layout_t * layout)
{
	...
	if (width >= (height + 2)) {
		...
		if (((width * 2) + 3) <= (height + 2)) {
			...
			layout->bottom.pad =
			    PAD_TOP | PAD_BOTTOM | PAD_RIGHT | PAD_LEFT;
...
decor_fill_picture_extents_with_shadow(Display * xdisplay,
				       ...
				       Picture picture, decor_layout_t * layout)
{
	...
	if (w > 0) {
		...
		if (sw != w) {
			...
			XRenderSetPictureTransform(xdisplay, shadow->picture,
						   &t);
		}

		/* top */
		XRenderComposite(xdisplay, PictOpSrc, shadow->picture, 0,
				 picture, sx, 0, 0, 0, layout->top.x1 + left,
				 layout->top.y1, w, context->top_space);

		/* bottom */
		XRenderComposite(xdisplay, PictOpSrc, shadow->picture, 0,
				 picture, sx,
				 shadow->height - context->bottom_space, 0, 0,
				 layout->bottom.x1 + left, layout->bottom.y1, w,
				 context->bottom_space);
	...
	if (layout->rotation) {
		...
		/* left top */
		XRenderComposite(xdisplay, PictOpSrc, shadow->picture, 0,
				 picture, 0, 0, 0, 0, layout->left.x1,
		...
		/* right top */
		XRenderComposite(xdisplay, PictOpSrc, shadow->picture, 0,
				 picture, 0, 0, 0, 0, layout->right.x1,
		...
		/* left top */
		XRenderComposite(xdisplay, PictOpSrc, shadow->picture, 0,
				 picture, 0, context->top_space, 0, 0,
				 layout->left.x1, layout->left.y1,
				 context->left_space, top - context->top_space);

		/* right top */
		XRenderComposite(xdisplay, PictOpSrc, shadow->picture, 0,
				 picture, shadow->width - context->right_space,
				 context->top_space, 0, 0, layout->right.x1,
				 layout->right.y1, context->right_space,
				 top - context->top_space);
	}
	...
	if (layout->rotation) {
		...
		XRenderComposite(xdisplay, PictOpSrc, shadow->picture, 0,
				 picture, 0, 0, 0, 0,
				 layout->left.x2 - (bottom -
						    context->bottom_space),
				 layout->left.y1,
				 bottom - context->bottom_space,
				 context->left_space);
		...
		/* left bottom */
		XRenderComposite(xdisplay, PictOpSrc, shadow->picture, 0,
				 picture, 0, shadow->height - bottom, 0, 0,
				 layout->left.x1,
				 layout->left.y2 - (bottom -
						    context->bottom_space),
				 context->left_space,
				 bottom - context->bottom_space);
		...
		XRenderComposite(xdisplay, PictOpSrc, shadow->picture, 0,
				 picture, shadow->width - context->right_space,
				 shadow->height - bottom, 0, 0,
				 layout->right.x1,
				 layout->right.y2 - (bottom -
						     context->bottom_space),
				 context->right_space,
				 bottom - context->bottom_space);
	}
	...
	if (h > 0) {
		...
		if (layout->rotation) {
			...
			/* left */
			XRenderComposite(xdisplay, PictOpSrc, shadow->picture,
					 0, picture, 0, 0, 0, 0,
					 layout->left.x1 + (top -
							    context->top_space),
					 layout->left.y1, h,
			...
			t.matrix[0][2] =
			    (shadow->width - context->right_space) << 16;
...
static void
_decor_pad_border_picture(Display * xdisplay, Picture dst, decor_box_t * box)
{
	int x1, y1, x2, y2;
	...
	if (box->pad & PAD_LEFT) {
		XRenderComposite(xdisplay, PictOpSrc, dst, None, dst,
				 x1, y1, 0, 0, x1 - 1, y1, 1, y2 - y1);
	}

	if (box->pad & PAD_RIGHT) {
		XRenderComposite(xdisplay, PictOpSrc, dst, None, dst,
				 x2 - 1, y1, 0, 0, x2, y1, 1, y2 - y1);
	}
}
...
_decor_blend_horz_border_picture(Display * xdisplay,
				 ...
				 int x2,
				 int y2,
				 int dy, int direction, int ignore_src_alpha)
{
	...
	if (ignore_src_alpha) {
		XRenderComposite(xdisplay, PictOpSrc, src, None, dst,
				 xSrc, ySrc, 0, 0, x1, y1, x2 - x1, y2 - y1);
		XRenderFillRectangle(xdisplay, PictOpAdd, dst, &color[2], x1,
				     y1, x2 - x1, y2 - y1);
		gop = PictOpInReverse;
	}
...
_decor_blend_vert_border_picture(Display * xdisplay,
				 ...
				 int y1,
				 int x2,
				 int y2, int direction, int ignore_src_alpha)
{
	XRenderColor color[3] = {
		{0xffff, 0xffff, 0xffff, 0xffff},
		{alpha, alpha, alpha, alpha},
	...
	if (layout->rotation) {
		...
		while (nBox--) {
			...
			XUnionRectWithRegion(&rect, rotated_region,
					     rotated_region);
	...
	if (ignore_src_alpha) {
		if (layout->rotation) {
			...
			XRenderFillRectangle(xdisplay, PictOpAdd, dst,
					     &color[2], x1, y1, x2 - x1,
					     y2 - y1);
...
decor_blend_border_picture(Display * xdisplay,
			   ...
			   Region region,
			   unsigned short alpha,
			   int shade_alpha, int ignore_src_alpha)
{
...
decor_acquire_dm_session(Display * xdisplay,
			 int screen,
			 const char *name,
			 int replace_current_dm, Time * timestamp)
{
	XEvent event;
	...
	if (current_dm_sn_owner != None) {
		...
		XSelectInput(xdisplay, current_dm_sn_owner,
			     StructureNotifyMask);
	}
...
void decor_set_dm_check_hint(Display * xdisplay, int screen)
{
...
convert_property(Display * xdisplay,
		 Window w, Atom target, Atom property, Time dm_sn_timestamp)
{
	...
	if (event->xselectionrequest.target == multiple_atom) {
		if (event->xselectionrequest.property != None) {
			...
			if (XGetWindowProperty(xdisplay,
					       ...
					       atom_pair_atom, &type, &format,
					       &num, &rest, &data) != Success)
				return;
...
int
decor_handle_selection_clear(Display * xdisplay, XEvent * xevent, int screen)
{
	Atom dm_sn_atom;
	char buf[128];
