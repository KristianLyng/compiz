...
Bool compInitMetadata(CompMetadata * metadata)
{
...
Bool compInitPluginMetadata(CompMetadata * metadata, const char *plugin)
{
	char str[1024];
...
static xmlDoc *readXmlFile(const char *path, const char *name)
{
	char *file;
...
Bool compAddMetadataFromFile(CompMetadata * metadata, const char *file)
{
	char *home;
	Bool status = FALSE;
...
Bool compAddMetadataFromString(CompMetadata * metadata, const char *string)
{
	xmlDoc **d, *doc;
...
static int readPluginXmlCallback(void *context, char *buffer, int length)
{
	CompIOCtx *ctx = (CompIOCtx *) context;
	int offset = ctx->offset;
	int i, j;

	i = compReadXmlChunk("<compiz><plugin name=\"", &offset, buffer,
			     length);
	i += compReadXmlChunk(ctx->name, &offset, buffer + i, length - i);
	i += compReadXmlChunk("\">", &offset, buffer + i, length - i);

	if (ctx->nDisplayOInfo) {
		i += compReadXmlChunk("<display>", &offset, buffer + i,
				      length - i);

		for (j = 0; j < ctx->nDisplayOInfo; j++)
			i += compReadXmlChunkFromMetadataOptionInfo(&ctx->
								    displayOInfo
								    [j],
								    &offset,
								    buffer + i,
	...
	if (ctx->nScreenOInfo) {
		i += compReadXmlChunk("<screen>", &offset, buffer + i,
				      length - i);
...
initXPathFromMetadataPath(CompXPath * xPath,
			  CompMetadata * metadata, const xmlChar * path)
{
	xmlXPathObjectPtr obj;
	xmlXPathContextPtr ctx;
	int i;
...
initXPathFromMetadataPathElement(CompXPath * xPath,
				 ...
				 const xmlChar * path, const xmlChar * element)
{
	char str[1024];
...
static void finiXPath(CompXPath * xPath)
{
	xmlXPathFreeObject(xPath->obj);
	xmlXPathFreeContext(xPath->ctx);
}

static CompOptionType getOptionType(char *name)
{
...
static void initBoolValue(CompOptionValue * v, xmlDocPtr doc, xmlNodePtr node)
{
	xmlChar *value;
...
initIntValue(CompOptionValue * v,
	     CompOptionRestriction * r, xmlDocPtr doc, xmlNodePtr node)
{
	xmlChar *value;
...
static void initColorValue(CompOptionValue * v, xmlDocPtr doc, xmlNodePtr node)
{
	xmlNodePtr child;
...
initActionValue(CompDisplay * d,
		CompOptionValue * v,
		CompActionState state, xmlDocPtr doc, xmlNodePtr node)
{
	...
	for (child = node->xmlChildrenNode; child; child = child->next) {
		...
		if (value) {
			...
			for (i = 0; i < SCREEN_EDGE_NUM; i++)
				if (strcasecmp((char *)value, edgeToString(i))
				    == 0)
...
initMatchValue(CompDisplay * d,
	       CompOptionValue * v, Bool helper, xmlDocPtr doc, xmlNodePtr node)
{
	xmlChar *value;
...
initListValue(CompDisplay * d,
	      ...
	      Bool helper, xmlDocPtr doc, xmlNodePtr node)
{
	xmlNodePtr child;
	...
	for (child = node->xmlChildrenNode; child; child = child->next) {
		...
		if (value) {
			...
			case CompOptionTypeBool:
				initBoolValue(&value[v->list.nValue], doc,
					      child);
				break;
			case CompOptionTypeInt:
				initIntValue(&value[v->list.nValue], r, doc,
					     child);
				break;
			case CompOptionTypeFloat:
				initFloatValue(&value[v->list.nValue], r, doc,
					       child);
				break;
			case CompOptionTypeString:
				initStringValue(&value[v->list.nValue], r, doc,
						child);
				break;
			case CompOptionTypeColor:
				initColorValue(&value[v->list.nValue], doc,
					       child);
				break;
			case CompOptionTypeAction:
				initActionValue(d, &value[v->list.nValue],
						state, doc, child);
				break;
			case CompOptionTypeKey:
				initKeyValue(d, &value[v->list.nValue], state,
					     doc, child);
				break;
			case CompOptionTypeButton:
				initButtonValue(d, &value[v->list.nValue],
						state, doc, child);
				break;
			case CompOptionTypeEdge:
				initEdgeValue(d, &value[v->list.nValue], state,
					      doc, child);
				break;
			case CompOptionTypeBell:
				initBellValue(d, &value[v->list.nValue], state,
					      doc, child);
				break;
			case CompOptionTypeMatch:
				initMatchValue(d, &value[v->list.nValue],
					       helper, doc, child);
			default:
				break;
			}
...
static char *stringFromMetadataPathElement(CompMetadata * metadata,
					   const char *path,
					   const char *element)
{
	char str[1024];
...
boolFromMetadataPathElement(CompMetadata * metadata,
			    const char *path,
			    const char *element, Bool defaultValue)
{
	Bool value = FALSE;
	char *str;
...
initIntRestriction(CompMetadata * metadata,
		   CompOptionRestriction * r, const char *path)
{
	char *value;
...
initActionState(CompMetadata * metadata,
		CompOptionType type, CompActionState * state, const char *path)
{
	...
	*state = CompActionStateAutoGrab;
	...
	if (type == CompOptionTypeEdge) {
		...
		noEdgeDelay =
		    stringFromMetadataPathElement(metadata, path, "nodelay");
		if (noEdgeDelay) {
...
initOptionFromMetadataPath(CompDisplay * d,
			   CompMetadata * metadata,
			   CompOption * option, const xmlChar * path)
{
	...
	case CompOptionTypeInt:
		...
		initIntValue(&option->value, &option->rest, defaultDoc,
			     defaultNode);
		break;
	case CompOptionTypeFloat:
		initFloatRestriction(metadata, &option->rest, (char *)path);
		initFloatValue(&option->value, &option->rest, defaultDoc,
			       defaultNode);
		break;
	case CompOptionTypeString:
	...
	case CompOptionTypeAction:
		...
		initActionValue(d, &option->value, state, defaultDoc,
				defaultNode);
		break;
	case CompOptionTypeKey:
		initActionState(metadata, option->type, &state, (char *)path);
	...
	case CompOptionTypeButton:
		initActionState(metadata, option->type, &state, (char *)path);
		initButtonValue(d, &option->value, state, defaultDoc,
				defaultNode);
		break;
	case CompOptionTypeEdge:
		initActionState(metadata, option->type, &state, (char *)path);
		initEdgeValue(d, &option->value, state, defaultDoc,
			      defaultNode);
		break;
	case CompOptionTypeBell:
		initActionState(metadata, option->type, &state, (char *)path);
		initBellValue(d, &option->value, state, defaultDoc,
			      defaultNode);
		break;
	case CompOptionTypeMatch:
		helper =
		    boolFromMetadataPathElement(metadata, (char *)path,
						"helper", FALSE);
		initMatchValue(d, &option->value, helper, defaultDoc,
			       defaultNode);
		break;
	case CompOptionTypeList:
		value =
		    stringFromMetadataPathElement(metadata, (char *)path,
						  "type");
		if (value) {
...
static void
finiScreenOptionValue(CompScreen * s, CompOptionValue * v, CompOptionType type)
{
	int i;
	...
	case CompOptionTypeList:
		for (i = 0; i < v->list.nValue; i++)
			finiScreenOptionValue(s, &v->list.value[i],
					      v->list.type);
	default:
		break;
	}
...
void compFiniScreenOption(CompScreen * s, CompOption * o)
{
...
compInitScreenOptionsFromMetadata(CompScreen * s,
				  ...
				  CompOption * opt, int n)
{
	int i;

	for (i = 0; i < n; i++) {
		if (!compInitScreenOptionFromMetadata
		    (s, m, &opt[i], info[i].name)) {
...
Bool
compSetScreenOption(CompScreen * s, CompOption * o, CompOptionValue * value)
{
...
finiDisplayOptionValue(CompDisplay * d,
		       CompOptionValue * v, CompOptionType type)
{
	CompScreen *s;
	int i;
	...
	case CompOptionTypeList:
		for (i = 0; i < v->list.nValue; i++)
			finiDisplayOptionValue(d, &v->list.value[i],
					       v->list.type);
	default:
		break;
	}
...
void compFiniDisplayOption(CompDisplay * d, CompOption * o)
{
				   ...
				   CompOption * opt, int n)
{
	...
	for (i = 0; i < n; i++) {
		if (!compInitDisplayOptionFromMetadata
		    (d, m, &opt[i], info[i].name)) {
...
Bool
compSetDisplayOption(CompDisplay * d, CompOption * o, CompOptionValue * value)
{
	if (isActionOption(o)) {
...
char *compGetShortPluginDescription(CompMetadata * m)
{
	char str[1024];
...
char *compGetLongPluginDescription(CompMetadata * m)
{
	char str[1024];
...
char *compGetShortScreenOptionDescription(CompMetadata * m, CompOption * o)
{
	char str[1024];
...
char *compGetLongScreenOptionDescription(CompMetadata * m, CompOption * o)
{
	char str[1024];
...
char *compGetShortDisplayOptionDescription(CompMetadata * m, CompOption * o)
{
	char str[1024];
...
char *compGetLongDisplayOptionDescription(CompMetadata * m, CompOption * o)
{
	char str[1024];
				       ...
				       int *offset, char *buffer, int length)
{
	...
	if (info->type) {
		i += compReadXmlChunk("\" type=\"", offset, buffer + i,
				      length - i);
		i += compReadXmlChunk(info->type, offset, buffer + i,
				      length - i);
	}

	if (info->data) {
		i += compReadXmlChunk("\">", offset, buffer + i, length - i);
		i += compReadXmlChunk(info->data, offset, buffer + i,
				      length - i);
		i += compReadXmlChunk("</option>", offset, buffer + i,
				      length - i);
	} else {
		i += compReadXmlChunk("\"/>", offset, buffer + i, length - i);
	}
