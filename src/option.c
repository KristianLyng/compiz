...
void compInitOption(CompOption * o)
{
		...
		case CompOptionTypeButton:
			...
			if (action->type & CompBindingTypeEdgeButton) {
				if (action->button.button ==
				    v.action.button.button
				    && action->button.modifiers ==
				    v.action.button.modifiers
...
static char *stringAppend(char *s, const char *a)
{
	char *r;
	int len;
...
static unsigned int
bindingStringToEdgeMask(CompDisplay * d, const char *binding)
{
