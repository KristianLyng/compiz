...
typedef void (*GLProgramParameter4dvProc) (GLenum target,
					   GLuint index, const GLdouble * data);
...
static void workaroundsAddToFullscreenList(CompWindow * w)
{
...
static void workaroundsRemoveFromFullscreenList(CompWindow * w)
{
				 ...
				 GLfloat x, GLfloat y, GLfloat z, GLfloat w)
{
	...
	ws = GET_WORKAROUNDS_SCREEN(currentScreen,
				    GET_WORKAROUNDS_DISPLAY(currentScreen->
							    display));
...
static void workaroundsUpdateParameterFix(CompScreen * s)
{
...
static char *workaroundsGetWindowRoleAtom(CompWindow * w)
{
...
static void workaroundsRemoveSticky(CompWindow * w)
{
...
static void workaroundsUpdateSticky(CompWindow * w)
{
	WORKAROUNDS_WINDOW(w);
...
static void updateUrgencyState(CompWindow * w)
{
	Bool urgent;
...
static void workaroundsFixupFullscreen(CompWindow * w)
{
	Bool isFullSize;
	int output;
	BoxPtr box;
...
static void workaroundsDoFixes(CompWindow * w)
{
	CompDisplay *d = w->screen->display;
...
workaroundsDisplayOptionChanged(CompDisplay * d,
				CompOption * opt, WorkaroundsDisplayOptions num)
{
	CompScreen *s;
	CompWindow *w;
...
static void workaroundsHandleEvent(CompDisplay * d, XEvent * event)
{
	CompWindow *w;
...
static Bool workaroundsInitDisplay(CompPlugin * plugin, CompDisplay * d)
{
	...
	workaroundsSetStickyAlldesktopsNotify(d,
					      workaroundsDisplayOptionChanged);
	workaroundsSetAlldesktopStickyMatchNotify(d,
						  workaroundsDisplayOptionChanged);
	workaroundsSetAiglxFragmentFixNotify(d,
					     workaroundsDisplayOptionChanged);
...
static CompBool workaroundsInitObject(CompPlugin * p, CompObject * o)
{
...
static void workaroundsFiniObject(CompPlugin * p, CompObject * o)
{
